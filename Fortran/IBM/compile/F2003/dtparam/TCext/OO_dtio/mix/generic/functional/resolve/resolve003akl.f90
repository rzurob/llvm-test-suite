...
!*  ===================================================================
!*
!*  TEST CASE NAME             : resolve003akl
!*
!*  PROGRAMMER                 : David Forster (derived from resolve003a by Robert Ma)
...
!*                                        - for unformatted I/O with multiple extended array
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
!* ===================================================================
!*
...
program resolve003akl
   ...
   open ( 1, file = 'resolve003.1', form='unformatted', access='sequential' )
   ...
   read ( 1, iostat = stat, iomsg = msg )             b1
   if ( ( stat /= 0 ) .or. ( msg /= 'dtioreadg' ) )      error stop 9_4

   select type ( b1 )
      type is ( gen3(*,*,*) ) ! tcx: (*,*,*)
         if ( ( b1(1)%c /='abc' ) .or. ( b1(1)%d /= 'def' ) .or. ( b1(1)%e /= 'ghi' )  .or. &
              ( b1(2)%c /='jkl' ) .or. ( b1(2)%d /= 'mno' ) .or. ( b1(2)%e /= 'pqr' ) )  error stop 10_4
      class default
         error stop 11_4
   end select
