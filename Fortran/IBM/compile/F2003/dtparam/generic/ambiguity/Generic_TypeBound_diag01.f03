...
!*
      ...
      CLASS(NextGen(4,:,4,4)) FUNCTION foo2(Obj)
      CLASS(NextGen(4,*,4,4)) :: Obj ! Must be type "uncompatible" with CLASS(Base(4,*)) to be valid
      POINTER  :: foo2

      ALLOCATE (foo2, source= Obj)
      IF ( .NOT. ASSOCIATED(foo2)) ERROR STOP 30

      END FUNCTION foo2

      END MODULE Mod1
!*
      PROGRAM Generic_TypeBound_diag01

! If dtv is of type NextGen, both calls to foo1 and foo2 are valid

      END PROGRAM Generic_TypeBound_diag01
