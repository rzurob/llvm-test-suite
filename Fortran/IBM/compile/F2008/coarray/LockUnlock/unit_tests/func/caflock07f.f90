...
module stack_manager
  ...
  contains
    subroutine get_task(job)
      ...
      lock(dimen_lock[-3, 2], stat=i1, errmsg=c1, acquired_lock=l)
      job = stack(stack_size)
      stack_size = stack_size - 1
      runlock(dimen_lock[-3, 2], stat=i2, errmsg=c2)
	...
	subroutine put_task(job,image)
      ...
      lock(dimen_lock[image, image + 1], stat=i1, errmsg=c1, &
        acquired_lock=l)
      stack_size[image] = stack_size[image] + 1
      stack(stack_size[image])[image] = job
      runlock(dimen_lock[image, image + 1], stat=i2, errmsg=c2)
