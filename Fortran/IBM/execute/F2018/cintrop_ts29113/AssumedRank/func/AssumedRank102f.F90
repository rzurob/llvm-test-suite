...
module mod
   ...
   contains
   subroutine sub_mod(AAA, BBB, CCC, DDD)
     use iso_c_binding
     integer :: i, j, k
     integer(c_int) :: AAA(:), BBB(-N+1:,0:), CCC(:,:,:), DDD(:,:,:,:,:,:,:,:,:,:)
...
!---------- internal subprogram ----------
   call sub_int(A, B, C, D)

!---------- module subprogram ----------
   call sub_mod(A, B, C, D)

contains
  subroutine sub_int(AA, BB, CC, DD)
     integer(c_int) :: AA(:), BB(-N+1:,0:), CC(:,:,:), DD(:,:,:,:,:,:,:,:,:,:)
