...
!* ===================================================================
!*
!* TEST CASE TITLE              : AssumedRank301f.f
!*
...
!*                                - allocatable
!*                                - deallocate on the C-side and reallocate with different bounds 
...
interface c_check
    subroutine check_int(arg) bind(c)
        import :: c_int
        integer(c_int), allocatable, intent(inout) :: arg(..) 
    end
    subroutine check_short(arg) bind(c)
        import :: c_short
        integer(c_short), allocatable, intent(inout) :: arg(..) 
    end
    subroutine check_complex(arg) bind(c)
        import :: c_float_complex
        complex(c_float_complex), allocatable, intent(inout) :: arg(..) 
    end
    subroutine check_float(arg) bind(c)
        import :: c_float
        real(c_float), allocatable, intent(inout) :: arg(..) 
    end
end interface
...
if( size(a1)       /=                               11 ) ERROR STOP 123
if( any(shape(a1)  /=                            [11]) ) ERROR STOP 124
if( any(lbound(a1) /=                            [-5]) ) ERROR STOP 125
if( any(ubound(a1) /=                             [5]) ) ERROR STOP 126
if( any(a1         /=                  [(i, i=-5, 5)]) ) ERROR STOP 127
...
if( size(r2)       /=                                4 ) ERROR STOP 135
if( any(shape(r2)  /=                           [2,2]) ) ERROR STOP 136
if( any(lbound(r2) /=                         [10,11]) ) ERROR STOP 137
if( any(ubound(r2) /=                         [11,12]) ) ERROR STOP 138

if(                                .not. allocated(s3) ) ERROR STOP 140
if(                            .not. is_contiguous(s3) ) ERROR STOP 141
if( rank(s3)       /=                                3 ) ERROR STOP 142
if( size(s3)       /=                               90 ) ERROR STOP 143
if( any(shape(s3)  /=                        [10,3,3]) ) ERROR STOP 144
if( any(lbound(s3) /=                        [0,0,-1]) ) ERROR STOP 145
if( any(ubound(s3) /=                         [9,2,1]) ) ERROR STOP 146
...
if( size(z1)       /=                                3 ) ERROR STOP 165
if( any(shape(z1)  /=                             [3]) ) ERROR STOP 166
if( any(lbound(z1) /=                             [1]) ) ERROR STOP 167
if( any(ubound(z1) /=                             [3]) ) ERROR STOP 168

end program AssumedRank301f
