! *********************************************************************
!* ===================================================================
!*
!* DATE                         : October 27, 2013
!* ORIGIN                       : AIX Complier Development
!*
!* PRIMARY FUNCTIONS TESTED     : C Interop: Assumed rank dummy argument
!* SECONDARY FUNTIONS TESTED    :
!*
!* REQUIRED COMPILER OPTIONS    :
!*                               (use -D_DEBUG for a debug version)
!*
!* DESCRIPTION                  : Calling a BIND(C) procedure defined in C from Fortran
!*                                - array is rank 0, 1, 2, 3
!*                                - allocatable
!*                                - Verify value / Change value on the C-side
!*                                - Generic resolution based on allocatable vs. pointer
!*
!* Actua1 Argument:
!*
!* Dummy Argument:
!*
!* ===================================================================
!*  REVISION HISTORY
!*
!*  MM/DD/YY:  Init:  Comments:
!* ===================================================================
!*
!234567890123456789012345678901234567890123456789012345678901234567890
program AssumedRank303f
use, intrinsic :: iso_c_binding
implicit none

interface c_check
    subroutine check_alloc(arg) bind(c)
        import :: c_int
        integer(c_int), allocatable :: arg(..)
    end
    subroutine check_ptr(arg) bind(c)
        import :: c_int
        integer(c_int), pointer :: arg(..)
    end
end interface

integer        :: st, i, j, k
character(200) :: msg
integer(c_int), allocatable :: a0, a1(:), a3(:,:,:)
integer(c_int), pointer :: p0, p1(:), p3(:,:,:)
integer(c_int), target  :: t0

! Allocate all the allocatable arrays
a0 = -1

allocate(a1(10), stat=st, errmsg=msg)
if( st .NE. 0 ) then
   print *, msg
   ERROR STOP 1
endif
a1 = [(i**2, i=1, 10)]

allocate(a3(-4:5,-2:3,0:0), stat=st, errmsg=msg)
if( st .NE. 0 ) then
   print *, msg
   ERROR STOP 2
endif
do i = lbound(a3,1), ubound(a3,1), 1
   do j = lbound(a3,2), ubound(a3,2), 1
      do k = lbound(a3,3), ubound(a3,3), 1
         a3(i,j,k)= i + j + k
      end do
   end do
end do

! Allocate all the allocatable arrays
t0 = -1
p0 => t0

allocate(p1(10), stat=st, errmsg=msg)
if( st .NE. 0 ) then
   print *, msg
   ERROR STOP 1
endif
p1 = [(i**2, i=1, 10)]

allocate(p3(-4:5,-2:3,0:0), stat=st, errmsg=msg)
if( st .NE. 0 ) then
   print *, msg
   ERROR STOP 2
endif
do i = lbound(p3,1), ubound(p3,1), 1
   do j = lbound(p3,2), ubound(p3,2), 1
      do k = lbound(p3,3), ubound(p3,3), 1
         p3(i,j,k)= i + j + k
      end do
   end do
end do

! Verify alllocation status, rank, shape, size, bounds and contiguity in Fortran
if(                      .not. allocated(a0) ) ERROR STOP 10
if( a0             /=                     -1 ) ERROR STOP 11
if(                      .not. allocated(a1) ) ERROR STOP 20
if(                  .not. is_contiguous(a1) ) ERROR STOP 21
if( rank(a1)       /=                      1 ) ERROR STOP 22
if( size(a1)       /=                     10 ) ERROR STOP 23
if( any(shape(a1)  /=                  [10]) ) ERROR STOP 24
if( any(lbound(a1) /=                   [1]) ) ERROR STOP 25
if( any(ubound(a1) /=                  [10]) ) ERROR STOP 26
if( any(a1         /=     [(i**2, i=1, 10)]) ) ERROR STOP 27
if(                      .not. allocated(a3) ) ERROR STOP 50
if(                  .not. is_contiguous(a3) ) ERROR STOP 51
if( rank(a3)       /=                      3 ) ERROR STOP 52
if( size(a3)       /=                     60 ) ERROR STOP 53
if( any(shape(a3)  /=              [10,6,1]) ) ERROR STOP 54
if( any(lbound(a3) /=             [-4,-2,0]) ) ERROR STOP 55
if( any(ubound(a3) /=               [5,3,0]) ) ERROR STOP 56
do i = lbound(a3,1), ubound(a3,1), 1
   do j = lbound(a3,2), ubound(a3,2), 1
      do k = lbound(a3,3), ubound(a3,3), 1
         if( a3(i,j,k)     /=    (i + j + k) ) ERROR STOP 57
      end do
   end do
end do

! Verify alllocation status, rank, shape, size, bounds and contiguity in C
call c_check(a0)
call c_check(a1)
call c_check(a3)
call c_check(p0)
call c_check(p1)
call c_check(p3)

! Verify again
if(                                .not. allocated(a0) ) ERROR STOP 110
if( a0             /=                              -99 ) ERROR STOP 111

if(                                .not. allocated(a1) ) ERROR STOP 120
if(                            .not. is_contiguous(a1) ) ERROR STOP 121
if( rank(a1)       /=                                1 ) ERROR STOP 122
if( size(a1)       /=                               10 ) ERROR STOP 123
if( any(shape(a1)  /=                            [10]) ) ERROR STOP 124
if( any(lbound(a1) /=                             [1]) ) ERROR STOP 125
if( any(ubound(a1) /=                            [10]) ) ERROR STOP 126
if( any(a1         /=                  [(i, i=-5, 4)]) ) ERROR STOP 127

if(                                .not. allocated(a3) ) ERROR STOP 140
if(                            .not. is_contiguous(a3) ) ERROR STOP 141
if( rank(a3)       /=                                3 ) ERROR STOP 142
if( size(a3)       /=                               60 ) ERROR STOP 143
if( any(shape(a3)  /=                        [10,6,1]) ) ERROR STOP 144
if( any(lbound(a3) /=                       [-4,-2,0]) ) ERROR STOP 145
if( any(ubound(a3) /=                         [5,3,0]) ) ERROR STOP 146

end program AssumedRank303f
