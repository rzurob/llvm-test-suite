...
void sub_1(CFI_cdesc_t * a)
{
  ...
  const char * src = "AAABBBCCCDDDEEEFFFGGGHHH";
#if _DEBUG
  ...
  // for assumed_size dummy, based on TS, the extent of last dim should be -1 
  assert(a->dim[1].extent == -1);
  assert(a->dim[1].sm == a->dim[0].extent*a->dim[0].sm);

  char *address;
  CFI_index_t subscripts[2];
  // we have to specify the numeric value for the extent of last dimension, since a->dim[0].extent = -1 
  for (int j=a->dim[1].lower_bound; j < 4 ; j++)
  {
    subscripts[1] = j;
