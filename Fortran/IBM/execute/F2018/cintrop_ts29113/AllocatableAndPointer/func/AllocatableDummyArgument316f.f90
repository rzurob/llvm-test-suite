...
subroutine associate_new_obj(src) bind(C)
    ...
    if ( lbound(src,1) /= 0 )  ERROR STOP 60
    if ( ubound(src,1) /= 3 )  ERROR STOP 61
    if ( size(src)     /= 4 )  ERROR STOP 62
    do i = 1, size(src)
        if ( .not. precision_x8(src(i), ((i+1)*2.0,(i+1)*2.0+1.0) )) ERROR STOP 63
    end do

    ptr(1:4) => src
    if (   lbound(ptr,1) /= 1 )  ERROR STOP 64
    if (   ubound(ptr,1) /= 4 )  ERROR STOP 65
    if (   size(ptr)     /= 4 )  ERROR STOP 66
    do i = 1, size(ptr)
        if ( .not. precision_x8(ptr(i), src(i-1) )) ERROR STOP 67
    end do
