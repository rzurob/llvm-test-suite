...
!*  ============================================================================
!*
!*  TEST CASE NAME             : F2008/value/array/func/valuearray_f221.f
!*
...
!*                         		for passing an array section with a step of 2 
!*								of different types as actual argument
...
subroutine sub1_lg(arg)
	...
	if (size(arg) .ne. 4) error stop 511
	if ( any(lbound(arg) .ne. 1)) error stop 512
	if ( any(ubound(arg) .ne. 4)) error stop 513
	if (rank(arg) .ne. 1) error stop 514
	if (any(shape(arg) .ne. 4)) error stop 515	
	arg = .false.
end subroutine	
