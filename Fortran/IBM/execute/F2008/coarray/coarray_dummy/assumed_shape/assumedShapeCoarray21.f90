...
!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray21.f
!*  TEST CASE TITLE            : 
!*
...
module asc
   ...
   subroutine copyData(A, image, next_image)
      real, dimension(:,:,:), intent(inout) :: A[1, 1, 1, *]
...
program main
   ...
   real, dimension(SIZE, SIZE, SIZE), save :: A[1, 1, 1, *]
   real, dimension(SIZE, SIZE, SIZE), save :: results[1, 1, 1, *]
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      do x = 1, s1
         do y = 1, s2
            do z = 1, s3
               A(x,y,z)[1, image, 1, 1] = mod(x + image * next_image, SIZE) + 1
            end do
         end do
      end do
