...
!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray14.f
!*  TEST CASE TITLE            : 
!*
...
module asc
   ...
   subroutine verifyCopy(A, image, left_image, right_image)
      ...
      S = SIZE(A)
      do i = 1, S/3
         if (iachar(A(i)[left_image]) .NE. iachar(A(i)[image])) then
            print *,A(i)[left_image]," .NE. ",A(i)[image]
            ERROR STOP 101
         else if (iachar(A(i+S-S/3)[right_image]) .NE. iachar(A(i+S-S/3)[image])) then
...
program main
   ...
   implicit none
   integer, parameter :: SIZE = 51
   integer x, add, chA, chZ, images, image, left_image, right_image

   character, dimension(SIZE), save :: A[*]
   ...
   if (mod(image, 3) .EQ. 0) then
      ...
      !
      chA = iachar('A') + image - 1
      add = chA
      do x = 1, SIZE
         if (mod(add, chZ) .EQ. 0) then
            add = chA
         end if
         add = add + 1
         A(x)[image] = achar(add)
