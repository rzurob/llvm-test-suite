...
!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray27.f
!*  TEST CASE TITLE            : 
!*
...
!*
!*  Use of coarrays as drived type in a module. Uses different coranks and different shapes.
...
program main
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      do row = 1, SIZE
         ...
         p(row)%y = row + 3.23
         p(row)%z = image * 1.52
         p(row)%dir = image * 2.52
         p1(row)[image]%x = row + 2.23
         p1(row)[image]%y = row + 3.23
         p1(row)[image]%z = image * 1.52
         p1(row)[image]%dir = image * 2.52
         do col = 1, SIZE
            do x1 = 1, SIZE
               p2(row,col,x1)[image]%x = row + x1 + 1.23
               p2(row,col,x1)[image]%y = col + x1 + 0.23
               p2(row,col,x1)[image]%z = image * 0.52 + x1
               p2(row,col,x1)[image]%dir = image * 1.52 + x1
            end do
         end do
      end do
