!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray25.f
!*
!*  DATE                       : 2011-02-18
!*
...
program main
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      do row = 1, SIZE
         do col = 1, SIZE
            p(row,col)%x = row + 2.23
            p(row,col)%y = col + 3.23
            p(row,col)%z = image * 1.52
            p(row,col)%dir = image * 2.52
            p1(row,col)[image]%x = row + 2.23
            p1(row,col)[image]%y = col + 3.23
            p1(row,col)[image]%z = image * 1.52
            p1(row,col)[image]%dir = image * 2.52
            do x1 = 1, SIZE
               do y1 = 1, SIZE
                  p2(row,col,x1,y1)[image]%x = row + x1 + y1 + 1.23
                  p2(row,col,x1,y1)[image]%y = col + x1 + y1 + 0.23
                  p2(row,col,x1,y1)[image]%z = image * 0.52 + x1 + y1
                  p2(row,col,x1,y1)[image]%dir = image * 1.52 + x1 + y1
               end do
            end do
         end do
      end do
