...
program main
   ...
   complex, dimension(SIZE, SIZE, SIZE), save :: A[*]
   complex, dimension(SIZE, SIZE, SIZE), save :: results[*]
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      do x = 1, s1
         do y = 1, s2
            do z = 1, s3
               A(x,y,z)[image] = ( mod(x+y+z+image*next_image, SIZE)+1.3, mod(x+y+z+image*next_image, SIZE)+0.3 )
            end do
         end do
      end do
