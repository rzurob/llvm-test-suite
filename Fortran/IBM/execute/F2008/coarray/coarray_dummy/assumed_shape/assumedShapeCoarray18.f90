...
!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray18.f
!*  TEST CASE TITLE            : 
!*
...
program main
   ...
   complex, dimension(SIZE), save :: A[*]
   complex, dimension(SIZE), save :: results[*]
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      !
      do x = 1, SIZE
         A(x)[image] = ( mod(x+image*next_image, SIZE) + 1.2, mod(x+image*next_image, SIZE) + 0.2 )
      end do
