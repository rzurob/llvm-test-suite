...
!*  ============================================================================
!*
!*  TEST CASE NAME             : assumedShapeCoarray20.f
!*  TEST CASE TITLE            : 
!*
...
module asc
   ...
   subroutine copyData(A, image, next_image)
      real, dimension(:), intent(inout) :: A[1, 1, *]
...
program main
   ...
   real, dimension(SIZE), save :: A[1, 1 ,*]
   real, dimension(SIZE), save :: results[1, 1 ,*]
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      !
      do x = 1, SIZE
         A(x)[image, 1, 1] = mod(x + image * next_image, SIZE) + 1
      end do
