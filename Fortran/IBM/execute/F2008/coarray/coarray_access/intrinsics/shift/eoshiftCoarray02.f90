...
!*  ============================================================================
!*
!*  TEST CASE NAME             : eoshiftCoarray02.f
!*  TEST CASE TITLE            : 
!*
...
program main
   ...
   integer, dimension(SIZE, SIZE, SIZE), save, codimension[SIZE, *] :: X_I

   real B_R
   real, dimension(SIZE), save, codimension[SIZE/2, *] :: M_R
   real, dimension(SIZE, SIZE, SIZE), save, codimension[SIZE/2, SIZE/2, SIZE/2, *] :: X_R

   complex B_C
   complex, dimension(SIZE), save, codimension[SIZE, *] :: M_C
   complex, dimension(SIZE, SIZE, SIZE), save, codimension[SIZE/4, SIZE/4, SIZE/4, *] :: X_C
   ...
   !
   B_I = -this_image()
   M_I = eoshift(M_I, SHIFT=SIZE/2, BOUNDARY=B_I, DIM=1)
   do row = (SIZE/2)+1, SIZE 
   ...
   !
   B_R = -this_image()*1.5
   M_R = eoshift(M_R, SHIFT=SIZE/2, BOUNDARY=B_R, DIM=1)
   do row = (SIZE/2)+1, SIZE
   ...
   M_C = eoshift(M_C, SHIFT=SIZE/2, BOUNDARY=B_C, DIM=1)
   do row = (SIZE/2)+1, SIZE
