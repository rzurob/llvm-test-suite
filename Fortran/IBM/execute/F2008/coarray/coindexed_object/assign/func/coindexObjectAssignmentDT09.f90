!*  ============================================================================
!*
!*  TEST CASE NAME             : coindexObjectAssignmentDT09.f
!*
!*  DATE                       : 2011-03-11
!*
...
!*
!*  Uses corank of 1.
!*  Testing derived type co-arrays with parameters. Two levels of nesting is used.
...
program main
   ...
   if (image .EQ. 1) then
      ...
      print *,image,":M3:",M3
      Mtn[1]%M(1) = M1
      call Mtn[1]%M(1)%compare(M1)
   else if (image .EQ. 2) then
      Mtn[2]%M(2) = M2
      call Mtn[2]%M(2)%compare(M2)
   else
      Mtn[image]%M(3) = M3
      call Mtn[image]%M(3)%compare(M3)
   end if
