...
module coa
   ...
   subroutine verifyImagesProcessed(A, A1, A2, images_processesed, image, SIZE)
      ...
      if (image .LT. images_processesed) then
         ...
         do row = 1, SIZE
            do col = 1, SIZE
               if (A2(row,col)%x .NE. -300 .AND. A2(row,col)%y .NE. -300 .AND. A2(row,col)%z .NE. -300 .AND. A2(row,col)%radius .NE. -300 .AND. A2(row,col)%color .NE. -300) ERROR STOP 31
            end do
            if (A(row) .NE. -300) ERROR STOP 102
            if (A1(row)%x .NE. -300 .AND. A1(row)%y .NE. -300 .AND. A1(row)%z .NE. -300 .AND. A1(row)%radius .NE. -300 .AND. A1(row)%color .NE. -300)ERROR STOP 32
         end do
      end if
