...
!*  ============================================================================
!*
!*  TEST CASE NAME             : coindexObjectAssignmentDT07.f
!*  TEST CASE TITLE            : 
!*
...
!*  will be ignored.
!*  Global assignment using intrinsic type real.
!*
!*
!* ============================================================================
   ...
   subroutine verifyImagesProcessed(A, A1, A2, images_processesed, image, SIZE)
      ...
      if (image .LT. images_processesed) then
         do row = 1, SIZE
            ...
            if (A(row) .NE. row+1.5) then
               print *,image,":",A(row)," .NE. ",row+1.5
               ERROR STOP 101
            end if
         ...
         do row = 1, SIZE
            do col = 1, SIZE
               if (A2(row,col)%x .NE. -300.0 .AND. A2(row,col)%y .NE. -300.0 .AND. A2(row,col)%z .NE. -300.0 .AND. A2(row,col)%radius .NE. -300.0 .AND. A2(row,col)%color .NE. -300.0) ERROR STOP 31
            end do
            if (A(row) .NE. -300.0) ERROR STOP 102
            if (A1(row)%x .NE. -300.0 .AND. A1(row)%y .NE. -300.0 .AND. A1(row)%z .NE. -300.0 .AND. A1(row)%radius .NE. -300.0 .AND. A1(row)%color .NE. -300.0)ERROR STOP 32
         end do
      end if
   ...
   if (image .EQ. 1) then
      ...
      do x1 = 1, limit_X
         do y1 = 1, limit_Y
            do row = 1, SIZE
               do col = 1, SIZE
                  A2(row,col)[x1,y1]%x = row + 1.5
                  A2(row,col)[x1,y1]%y = col + 1.5
                  A2(row,col)[x1,y1]%z = row + col + 1.5
                  A2(row,col)[x1,y1]%radius = row + col + 2 + 1.5
                  A2(row,col)[x1,y1]%color = col + 2 + 1.5
               end do
               A(row)[x1,y1] = row + 1.5
               A1(row)[x1,y1]%x = row + 1.5
               A1(row)[x1,y1]%y = col + 1.5
               A1(row)[x1,y1]%z = row + col + 1.5
               A1(row)[x1,y1]%radius = row + col + 1 + 1.5
               A1(row)[x1,y1]%color = col + 1 + 1.5
            end do
            i = i + 1
         end do
      end do
