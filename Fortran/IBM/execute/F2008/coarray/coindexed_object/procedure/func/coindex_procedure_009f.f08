...
program coindex_procedure_009f
    implicit none
    integer, save ::caf1[4,*], caf2(10)[4,*]
    real(4), save ::caf3(10)[4,*]
    real(4) sum1, sum2
    ...
    m = image_index(caf1,[2,3])
    if ( funcp1(caf1[2,3]) .ne. m*2) then
        error stop 1
    end if

    i = 5
    m = image_index(caf2, [2,3])
    if ( funcp1(caf2(i)[2,3]) .ne. i*m*2) then
        error stop 2
    end if

    if (funcp2(caf2(:)[2,3]) .ne. 10) then
        error stop 3
    end if
    ...
    do i=1,10
        sum2 = caf3(i)[2,3] + sum2
    end do
    call funcp3(caf3(:)[2,3], sum1)
