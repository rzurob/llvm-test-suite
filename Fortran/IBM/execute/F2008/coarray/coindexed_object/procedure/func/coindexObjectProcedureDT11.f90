...
!*  ============================================================================
!*
!*  TEST CASE NAME             : coindexObjectProcedureDT11.f
!*  TEST CASE TITLE            : 
!*
...
program main
   ...
   if (mod(image, 2) .EQ. 0) then
      ...
      !
      do row = 1, SIZE
         p(row)%x = (row + 0.5, image + 0.5)
         p(row)%y = (row  + 1.5, image + 1.5)
         p(row)%z = (row + 2.5, image + 2.5)
         p1(row)[image]%x = (row + 0.5, image + 0.5)
         p1(row)[image]%y = (row + 1.5, image + 1.5)
         p1(row)[image]%z = (row + 2.5, image + 2.5)
         do col = 1, SIZE
            do x1 = 1, SIZE
               p2(row,col,x1)[image]%x = (row + col + 0.5, x1 + image + 0.5)
               p2(row,col,x1)[image]%y = (row + col + 1.5, x1 + image + 1.5)
               p2(row,col,x1)[image]%z = (row + col + 2.5, x1 + image + 2.5)
            end do
         end do
      end do
