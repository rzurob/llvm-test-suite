...
!*  ===================================================================
!*
!*  TEST CASE NAME             : dummyArg003.f
!*  TEST CASE TITLE            : Test the dummy arg character array coarray with different corank and cobound
!*                               
!*  PROGRAMMER                 : Ke Wen Lin 
!*  DATE                       : March 28, 2011 
!*  ORIGIN                     : Compiler Development, IBM CDL
!*
!*  PRIMARY FUNCTIONS TESTED   : Test the dummy arg character array coarray with different corank and cobound
!*                              
...
!*  -----------
!*  The testcase aim to 
!*  1. test the dummy arg character array coarray with different corank and cobound
!*  -----------
!*  
...
program dummyArg003
  ...
  contains 
	...
	! here, the corank and cobound is different from that of the coarray variables.
	subroutine sub(co_str1, co_str2, co_str3, char1, char2, im)
		character(len=10), dimension(ArrSize), intent(inout) :: co_str1[-4:*]
		character(len=10), dimension(ArrSize,*), intent(inout) :: co_str2[1:1,-3:*]
		character(len=10), dimension(:,:,:), intent(inout) :: co_str3[0:2,1:*]
		character(len=1), intent(in) :: char1[*], char2[*]
		
		integer, intent(in) :: im
	
		integer :: i, j, k, m, n, l
		integer :: lo1(1), up1(1), lo2(2), up2(2), lo3(2), up3(2)
		...
		if(any (lo1 .NE. [-4]) .OR. any(lo2 .NE. [1,-3]) .OR. any(lo3 .NE. [0,1])) then
			error stop 11
		end if
	    ...
	    do j = lo3(2), up3(2)
			do i = lo3(1), up3(1)
				do m = 1, ArrSize
					do n = 1, ArrSize
						do l = 1, ArrSize
							co_str3(m,n,l)[i,j] = repeat(char1[im],10)
						end do
					end do 
				end do
			end do
	...
	function func(co_str1, co_str2, co_str3, str1, str2)
		character(len=10), dimension(:), intent(in) :: co_str1[0:2,*]
		character(len=10), dimension(ArrSize,ArrSize), intent(in) :: co_str2[*]
		character(len=10), dimension(ArrSize,ArrSize,*), intent(in) :: co_str3[-1:0,-2:-1,-3:*]
		character(len=10), intent(in) :: str1, str2
		
		logical :: func
		
		integer :: i, j, k, m, n, l
		integer :: lo1(2), up1(2), lo2(1), up2(1), lo3(3), up3(3)
		
		lo1 = lcobound(co_str1); lo2 = lcobound(co_str2); lo3 = lcobound(co_str3);
		up1 = ucobound(co_str1); up2 = ucobound(co_str2); up3 = ucobound(co_str3);
		
		if(any(lo1 /= [0,1]) .OR. any(lo2 .NE. [1]) .OR. any(lo3 .NE. [-1,-2,-3])) then
			error stop 12
		end if
