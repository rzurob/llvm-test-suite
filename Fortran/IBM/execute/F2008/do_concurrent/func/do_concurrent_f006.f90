      ...
      program main
        ...
        if ( .false. ) then
          ...
          do concurrent (i=m:25:m, j=1:m:1, k=m:25:4)
            p_i_res(i/5) = func2(-1*p_i_arr(i/5))
            call sub1()
          end do

          if ( .not. precision_r4(i_res,i_arr) ) then
        ...
        do concurrent (i = 1:5, j = 1:5, (mod(100.0e0,p_i_arr2(i)) > p_value) .and. (mod(100.0e0,p_i_arr(j)) < 30.6e0))
          call sub1()
        ...
        do concurrent (i = 1:10, p_lvar .eqv. .true.)
          p_i_res3(i) = func1(p_i_arr4(i))
          call sub1()
          do concurrent (j = 1:10, func3(100.0d0,p_i_arr3(j)) > 0.154d2)
            call sub1()
            p_i_res4(j) = func1(p_i_arr4(j))
          end do
        end do
        ...
        do concurrent (i = 1:5, j = 1:5, (func2(-1.0e0*p_i_arr(i)) > 12.0e0) .and. (func2(-1.0e0*p_i_arr(j)) < 50.0e0))
          p_i_res2(i,j) = func3(100.5d0,real(i*j,8))
          call sub1()
          do concurrent (k = 1:5, l = 1:5, (abs(-1.0e0*p_i_arr(k)) > 26.5e0) .and. (abs(-1.0e0*p_i_arr(l)) < 40.0e0))
            p_i_res5(k,l) = func1(p_i_arr4(k))
            call sub1()
          end do
        end do
        ...
        do concurrent (i = 1:m*2, p_lvar .eqv. .true.)
          ...
          do concurrent (j = 100:1000:100, (abs(-1.0d0*p_i_arr3(j/100)) >= 30.0d0) .and. (abs(-1.0d0*p_i_arr3(j/100)) <= 80.0d0))
            ...
            do concurrent (k = 1:5, l = 1:5)
              call sub1()
              p_i_res2(k,l) = func3(100.5d0,real(k*l,8))
            end do
          end do
        end do
