        ...
        do x = 1,5
          do y = 1,5
            if ( .not. precision_x8(dtComplexType%comp8(x,y),comp8_result(x,y)) ) then
              print *, "block with do concurrent with DTP mask and a block inside produces incorrect result"
              print *, "x: ", x
              print *, "y: ", y
          ...
          do concurrent (int1 = 1:3, dtLogicalType%log1 .eqv. .true.)
            ...
            block
              ...
              do concurrent (int4 = 100:300:100, (dtLogicalType%log2 .eqv. .false.) .and. (dtMixType%char4(int4/100) .eq. "pass"))
                block
                  real*8 :: tmp
                  tmp = real(int4,8)*0.5d0
                  dtRealType%real8(int4/100) = tmp
                  do concurrent (int2 = 1:3, int4_2 = 3:5, int8 = 100:300:100, dtLogicalType%log4 .eqv. .false.)
                    block
                      complex*16 :: tmp2 = (2.0q0,5.0q0)
                      dtComplexType%comp16(int2,int4_2-2,int8/100) = tmp2*(int2,1.0q0)
                    end block
                  end do
                end block
              end do
