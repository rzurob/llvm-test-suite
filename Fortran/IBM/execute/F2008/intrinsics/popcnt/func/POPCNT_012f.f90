...
!*  DRIVER STANZA              :
!*
!*  DESCRIPTION                : Test that array argument in POPCNT() may 
...
!******** 1 dimension ******************************************	
	...
	arr4=[1,2,3,4,5]
	res4=POPCNT([arr4(-1), arr4(0), arr4(1), arr4(2), arr4(3)])
		
	if (any(res1 /= 2)) ERROR STOP 1
	if (any(res2 /= [1,1,2,1,2])) ERROR STOP 2
	if (size1 == size2) ERROR STOP 3
	if (any(res4 /= [1,1,2,1,2])) ERROR STOP 4
	
!****************************************************************

!******** 2 dimensions ******************************************	
	
	verify5=reshape([1,1,2,1,2,2,3,1,2,2,3,2], [3,4])
	...
	verify7=reshape([1,1,2,1,2,2], [3,2])
	arr7=reshape([(i,i=1,6)], [3,2])
	res7=reshape(POPCNT([ arr7(-10,8), arr7(-9,8), arr7(-8,8), arr7(-10,9), &
		...
		do y=1,3 
			do z=1,4 
				do t=1,2
					...
					res8(x,y,z,t) = POPCNT(arr8(x,y,z,t))
					verify8(x,y,z,t) = POPCNT(x*y*z*t)
				end do	
			end do
		end do
	end do
		...
		do y=-2,-1 
			do z=10,11 
				do t=-1,1
					...
					res10(x,y,z,t) = POPCNT(arr10(x,y,z,t))
					verify10(x,y,z,t) = POPCNT(x*y*z*t)
				end do	
			end do
		end do
	end do
...
!*************** Dummy arrays ***********************************
	...
	call sub1(POPCNT(arr2), POPCNT(arr6), POPCNT(arr8), 5)

!****************************************************************

!*************** Assumed size ***********************************

	call sub2(POPCNT(arr4), POPCNT(arr6), POPCNT(arr8))
...
!****************************************************************
			...
			do y=1,N2 
				do z=1,N3 
					do t=1,N4
						...
						res9(x,y,z,t) = POPCNT(arr9(x,y,z,t))
					end do	
				end do
			end do
	    end do
	...
	subroutine sub1(x,y,z,dim1)
		...
		if (size(POPCNT(x)) /= 5) ERROR STOP 12
		if (size(POPCNT(y)) /= 15) ERROR STOP 13
		if (size(POPCNT(z)) /= 48) ERROR STOP 14
	...
	subroutine sub2(x,y,z)
		...
		if (size(POPCNT(x(-1:3))) /= 5) ERROR STOP 15
		if (size(POPCNT(y(1:3,-1:3))) /= 15) ERROR STOP 16
		if (size(POPCNT(z(-1:0,1:3,-2:1,-5:-4))) /= 48) ERROR STOP 17
		
	end subroutine sub2
	
end program POPCNT_012f
