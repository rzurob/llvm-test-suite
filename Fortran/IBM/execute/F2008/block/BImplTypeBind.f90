...
!*  "BIND") have a type prescribed by the IMPLICIT statement.
!*  A BIND statement can only be applied in the spec part of a module, so we
!*  can't introduce variables via a BIND statement *in the BLOCK*, but we can
...
module mod
  ...
  bind(c) :: b1, b6, i3, d4, s5, s7

end module mod

program BImplTypeBind
