!#######################################################################
! *********************************************************************
! %START
! %MAIN: YES
! %PRECMD: rm -f *.mod
! %COMPOPTS: -qfree=f90
! %GROUP: fconstr042a.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
! %POSTCMD:
! %END
! *********************************************************************
!*  ===================================================================
!*
!*  DATE                       : 03/08/2004
!*
!*  PRIMARY FUNCTIONS TESTED   :
!*  SECONDARY FUNCTIONS TESTED :
!*
!*  DESCRIPTION                : structure constructor (in DATA statement)
!*
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
!* ===================================================================
!*
!*  REVISION HISTORY
!*
!*  MM/DD/YY:  Init:  Comments:
!* ===================================================================
!23456789012345678901234567890123456789012345678901234567890123456789012

module m
    type base
        integer*4 :: id
    end type

    type (base) :: b1_m(10)
    integer*4 :: i_m(10)

    DATA (i_m(i), i=1,5), (b1_m(i), i=2,6) /4*1, 10, 2*base(id = 100), &
            3*base(-1)/

    data b1_m(1), (b1_m(i), i=7,10) /base(1), base(7), base(8), base(id =9), base(id=10)/

end module

program fconstr042a
use m
    type (base) b1(6)

    data b1(1), (b1(i+1), i=1,3), (b1(i),i=5,6) /base(id=1), base(2), 1*base(3), &
            0*base(id=-1), 1*base(id=4), base(5), base(6), 0*base(-1)/

    !! verify b1
    do i=1, 6
        if (b1(i)%id /= i) error stop 1_4
    end do

    !! verify b1_m
    if (any (b1_m(2:3)%id /= 100)) error stop 2_4

    if (any (b1_m(4:6)%id /= -1)) error stop 3_4

    if (b1_m(1)%id /= 1) error stop 4_4

    do i = 2, 5
        if (b1_m(i+5)%id /= i+5) error stop 5_4
    end do

    !! verify i_m
    if (any (i_m(1:5) /= (/1, 1, 1, 1, 10/))) error stop 6_4

end
