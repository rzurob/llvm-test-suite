...
module m
    ...
    type (child), pointer :: c1_m => null()
    type (thirdGeneration), pointer :: t1_m(:) => null()
    ...
    contains
        ...
        subroutine initializeModuleData
            ...
            t1_m(1) = thirdGeneration(id=2, value=2.0, name='t1_m', isSet=.true.)
            t1_m(2) = thirdGeneration(child = child(3, 3.0, name='t1_m2'), &
...
use m
    ...
    type (thirdGeneration), pointer :: t1

    allocate (t1)

    t1 = thirdGeneration (3, value = 3.0, name = 't1', isSet=.true.)
