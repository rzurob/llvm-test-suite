...
use m
    ...
    if (.not. isSeq1Valid (c1%s1, 1, 2)) error stop 1_4
    if (.not. isSeq1Valid (c1%base%s1, 1, 2)) error stop 2_4

    if ((c1%base%id /= 100) .or. (c1%id /= c1%base%id)) error stop 3_4

    if (c1%name /= 'test data c1') error stop 4_4

    if (.not. isSeq1Valid (b1%s1, -1, -2)) error stop 5_4
...
logical function isSeq1Valid (s, intVal1, intVal2)
    ...
    type (seq1), intent(in) :: s
    integer, intent(in) :: intVal1, intVal2
