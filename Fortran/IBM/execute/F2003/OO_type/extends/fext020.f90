...
module m1
    ...
    type, extends (oldBase) :: secondChild
        logical*1 :: flag = .false.
    end type

    type (secondChild), save :: c1_m
    type (newChild), save :: nc1_m
end module

program fext020
    use m1

    type (newBase) :: c1      !<-- actually a child type
    type (oldBase) :: b1      !<-- base type
    type (newChild) :: nc1    !<-- thirdGeneration
    type (secondChild) :: s1  !<-- a second child type
    ...
    ! nc1%id, nc1%base%id, nc1%child%id, nc1%child%base%id
