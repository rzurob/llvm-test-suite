...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  IF (Param%I             .NE. 3   )                      ERROR STOP 11
  IF (ABS(Param%R-5_8*ACOS(0.54030231_8)) .GE. 1.e-5  )   ERROR STOP 12

  IF (T1%Comp%I          .NE. 3   )                       ERROR STOP 21
  IF (ABS(T1%Comp%R-5_8*ACOS(0.54030231_8)) .GE. 1.e-5 )  ERROR STOP 22

  IF (ANY(LBOUND(Arr1)   .NE. (/3/)) )     ERROR STOP 31
  IF (ANY(UBOUND(Arr1)   .NE. (/5/)) )     ERROR STOP 32

  IF (ANY(Arr1%Comp%I    .NE. (/(NINT(ACOS(I/5.0)),    I=3, 5)/) ))         ERROR STOP 41

  IF (.NOT. ALL(precision_R4(Arr1%Comp%R, (/(ACOS(I/5.001),    &
          I=NINT(3*ACOS(0.54030231_8)), NINT(5*ACOS(0.54030231_8)))/) )))   ERROR STOP 42

  IF (ANY(LBOUND(Arr2)   .NE. (/NINT(ACOS(Param%R))/)  ))             ERROR STOP 51
  IF (ANY(UBOUND(Arr2)   .NE. (/NINT(3*ACOS(Param%R))/) ))            ERROR STOP 52

  IF (ANY(Arr2%I    .NE. INT(ACOS(0.54030231_8)) ))                   ERROR STOP 61
  IF (ANY(ABS(Arr2%R +ACOS(0.54030231_8)).GE. 1.E-6  ))               ERROR STOP 62
