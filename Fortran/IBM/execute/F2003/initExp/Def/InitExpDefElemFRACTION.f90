...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  IF ( KIND(R40)                                .NE. 4              )      STOP 11
  IF ( ANY(TRANSFER(R40, i4Quiet_N_NaN, 8)      .NE. i4Quiet_N_NaN  ) )    STOP 12

  IF ( KIND(R41)                                .NE. 4                  )  STOP 13
  IF ( ANY(TRANSFER(R41, i4Signaling_N_NaN, 8)  .NE. i4Signaling_N_NaN  )) STOP 14

  IF ( KIND(R42)                                .NE. 4             )       STOP 15
  IF ( ANY( TRANSFER(R42, i4N_Infinity, 8)      .NE. i4N_Infinity  ) )     STOP 16


  IF ( KIND(R43)     .NE. 4                           )      STOP 17
  IF ( ANY( R43      .NE. FRACTION(r4N_DeNormalized)  ) )    STOP 18

  IF ( KIND(R44)     .NE. 4                           )      STOP 19
  IF ( ANY( R44      .NE. FRACTION(r4N_Zero)          ) )    STOP 20

  IF ( KIND(R45)     .NE. 4                           )      STOP 21
  IF ( ANY( R45      .NE. FRACTION(r4P_Zero)          ) )    STOP 22

  IF ( KIND(R46)     .NE. 4                           )      STOP 23
  IF ( ANY( R46      .NE. FRACTION(r4p_DeNormalized)  ) )    STOP 24

  IF ( KIND(R47)                              .NE. 4             )        STOP 25
  IF ( ANY( TRANSFER(R47, i4P_Infinity, 8)    .NE. i4P_Infinity  ) )      STOP 26

  IF ( KIND(R48)                              .NE. 4                  )   STOP 27
  IF ( ANY(TRANSFER(R48, i4Signaling_P_NaN, 8).NE. i4Signaling_P_NaN  ) ) STOP 28

  IF ( KIND(R49)                              .NE. 4              )       STOP 29
  IF ( ANY( TRANSFER(R49, i4Quiet_P_NaN, 8)   .NE. i4Quiet_P_NaN  ) )     STOP 30



  IF ( KIND(R80)                                .NE. 8              )      STOP 31
  IF ( ANY(TRANSFER(R80, i8Quiet_N_NaN, 8)      .NE. i8Quiet_N_NaN  ) )    STOP 32

  IF ( KIND(R81)                                .NE. 8                  )  STOP 33
  IF ( ANY(TRANSFER(R81, i8Signaling_N_NaN, 8)  .NE. i8Signaling_N_NaN  )) STOP 34

  IF ( KIND(R82)                                .NE. 8             )       STOP 35
  IF ( ANY( TRANSFER(R82, i8N_Infinity, 8)      .NE. i8N_Infinity  ) )     STOP 36


  IF ( KIND(R83)     .NE. 8                           )      STOP 37
  IF ( ANY( R83      .NE. FRACTION(r8N_DeNormalized)  ) )    STOP 38
  IF ( KIND(R84)     .NE. 8                           )      STOP 39
  IF ( ANY( R84      .NE. FRACTION(r8N_Zero)          ) )    STOP 40

  IF ( KIND(R85)     .NE. 8                           )      STOP 41
  IF ( ANY( R85      .NE. FRACTION(r8P_Zero)          ) )    STOP 42

  IF ( KIND(R86)     .NE. 8                           )      STOP 43
  IF ( ANY( R86      .NE. FRACTION(r8p_DeNormalized)  ) )    STOP 44

  IF ( KIND(R87)                              .NE. 8             )        STOP 45
  IF ( ANY( TRANSFER(R87, i8P_Infinity, 8)    .NE. i8P_Infinity  ) )      STOP 46

  IF ( KIND(R88)                              .NE. 8                  )   STOP 47
  IF ( ANY(TRANSFER(R88, i8Signaling_P_NaN, 8).NE. i8Signaling_P_NaN  ) ) STOP 48

  IF ( KIND(R89)                              .NE. 8              )       STOP 49
  IF ( ANY( TRANSFER(R89, i8Quiet_P_NaN, 8)   .NE. i8Quiet_P_NaN  ) )     STOP 50
