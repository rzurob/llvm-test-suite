...
!-----------    ASSOCIATE  with  INTEGER  expressions   ----------------
           ...
           associate ( dertype => der_type%i1 , arg => 3 , arg1 => d_t%i1 )
                  dertype = dertype + arg
                  if(dertype .ne. arg1)then
                  error stop 10
                  endif
           end associate
      
           associate ( dertype => der_type%i2 , arg2 => d_t%i2 )
                  dertype = dertype*2 
                  if(dertype .ne. arg2)then
                  error stop 11
                  endif
           end associate

           associate ( dertype => der_type%i4 , argo => 3)
                  dertype = dertype - argo
                  if(dertype .ne. d_t%i4)then
                  error stop 12
                  endif
           end associate

           associate ( dertype => der_type%i8 , arg3 => d_t%i8 )
                  dertype = dertype + 3
                  if(dertype .ne. arg3)then
                  error stop 13
                  endif
           end associate
...
!-----------   ASSOCIATE with REAL expressions ----------------
           ...
           associate ( dertype => der_type%r4 , arg4 => d_t%r4 )
                  dertype = dertype*2 
                  if (.not.precision_r4(dertype,arg4)) then
                  error stop 14
                  endif
           end associate

           associate ( dertype => der_type%r8 , arg8 => 2)
                  dertype = dertype * arg8 
                  if (.not.precision_r8(dertype,d_t%r8)) then
                  error stop 15
                  endif
           end associate

           associate ( dertype => der_type%r16 , arg16 => 800.0 )
                  dertype = dertype + arg16  
...
!-----------   ASSOCIATE with CHARACTER expressions ----------------
           ...
           associate ( dertype => der_type%c , argb => d_t%c )
                  if(dertype .ne. argb)then
                  error stop 17
                  endif
           end associate
...
!-----------   ASSOCIATE with BYTE expressions ----------------
           ...
           associate ( dertype => der_type%b , ar1 => 2)
                  dertype = dertype + ar1  
...
!-----------   ASSOCIATE with COMPLEX expressions ----------------
           ...
           associate ( dertype => der_type%x , arx => (1.0,2.0) )
                  dertype = dertype + arx
           ...
           associate ( dertype => der_type%x8 , arx8 => (3.0E0,4.0E0))
                  dertype = dertype + arx8
