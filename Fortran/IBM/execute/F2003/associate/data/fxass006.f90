...
!-----------    ASSOCIATE  with  INTEGER  expressions   ----------------
           ...
           associate ( dertype => der_type%i8 )
                  dertype = dertype + 3
                  if(dertype .ne. der_type%i8)then
                  error stop 13
                  endif
           end associate
...
!-----------   ASSOCIATE with COMPLEX expressions ----------------
           ...
           associate ( dertype => der_type%x )
                  ...
                  if (.not. precision_x8(dertype,der_type%x)) then
                  error stop 19
                  endif
           end associate
...
!-----------   ASSOCIATE with DERIVED TYPES  -----------------------
           associate ( dtype => der_type )
                  if(dtype%i1 .ne. der_type%i1) error stop 30
                  if(dtype%i2 .ne. der_type%i2) error stop 31
                  if(dtype%i4 .ne. der_type%i4) error stop 32
                  if(dtype%i8 .ne. der_type%i8) error stop 33
                  if(dtype%l1 .neqv. der_type%l1) error stop 34
                  if(dtype%l2 .neqv. der_type%l2) error stop 35
                  if(dtype%l4 .neqv. der_type%l4) error stop 36
                  if (.not. precision_r4(dtype%r4,der_type%r4)) error stop 37
                  if (.not. precision_r8(dtype%r8,der_type%r8)) error stop 38
                  if (.not. precision_r6(dtype%r16,der_type%r16)) error stop 39
                  if (.not. precision_x8(dtype%x,der_type%x)) error stop 40
                  if (.not. precision_x8(dtype%x8,der_type%x8)) error stop 41
                  if (.not. precision_x6(dtype%x16,der_type%x16)) error stop 42
                  if(dtype%b .ne. der_type%b) error stop 43
                  if(dtype%c .ne. der_type%c) error stop 44
           end associate
