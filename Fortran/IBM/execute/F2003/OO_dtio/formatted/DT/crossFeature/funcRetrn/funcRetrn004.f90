...
module m
   ...
   contains
      ...
      class(base) function genArray(dtv)
         class(base), intent(in) :: dtv
         allocatable :: genArray(:)
         allocate( genArray(4), source = (/ base(100.0), base(101.0), base(102.0), base(103.0) /) )
      end function
...
use m
   ...
   interface
      class(child) function genArrayExt()
         import child
   ...
   open (1, file = 'funcRetrn004.1', form='formatted', access='sequential' )
   ...
   contains
      ...
      class(base) function genArrayInt()
         pointer :: genArrayInt(:,:)
         allocate ( genArrayInt(2,2), source = reshape ( source = (/ child(301.0, 311.0), child(302.0, 312.0), &
