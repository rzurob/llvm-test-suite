...
use m
   ...
   type(base)                :: b3(2,2) = reshape( source = (/ base(131.0), base(132.0) , base(133.0), base(134.0) /), shape = (/2,2/) )

   type(child), allocatable  :: c1(:)
   class(child), pointer     :: c2(:)
   type(child)               :: c3(3) = (/ child(231.0, 232.0), child(233.0, 234.0), child(235.0, 236.0) /)

   open (1, file = 'associate002.1', form='formatted', access='direct', recl=200 )
   ...
   allocate ( c1(2), source = (/ child ( 211.0 , 212.0 ), child(213.0, 214.0) /) )
   allocate ( c2(3), source = (/ child ( 221.0 , 222.0 ), child ( 223.0 , 224.0 ), child ( 225.0 , 226.0 ) /) )
