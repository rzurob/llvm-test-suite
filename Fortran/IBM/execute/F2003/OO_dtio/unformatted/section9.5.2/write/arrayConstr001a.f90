...
program arrayConstr001a
   ...
   open (unit = 1, file ='arrayConstr001a.data', form='unformatted', access='sequential')
   
   ! I/O operations
   
   write (1, iostat=stat, iomsg=msg )      reshape( source = (/ base1('abc'), base1('def'), base1('ghi'), base1('jkl') /), shape = (/2,2/) )
   write (1, iostat=stat, iomsg=msg )      reshape( source = (/ (base2(i),i = 4,7) /), shape=(/1,4/) )
