...
program array001
   ...
   allocate ( b1(5), source = (/ ( base(x=1.0,y=2.0), i=1,5) /) )
   allocate ( b2(2,2), source = reshape ( source = (/ ( base(x=3.0,y=4.0),i=1,4) /), shape = (/2,2/)))
   b3       = (/ ( base(x=5.0,y=6.0), i=1,3) /) 
   
   ! I/O operations   
   
   open ( u1%get(), file = 'array001.data', form='unformatted', access='sequential' )   !<- open a external file with unit #2
   
   inquire ( iolength = length1 )  b1           !<- output list is a single array item
   
   if ( length1 /= 40 )            error stop 1_4
