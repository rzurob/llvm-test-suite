...
!*                                    - Dummy Argument being polymorphic assumed-size array entity with formatted i/o
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
!* ===================================================================
!*
...
module m
   ...
   contains
      ...
      subroutine myread ( dtv )
         ...
         read ( 1, "(4(1x,DT))", iostat = stat, iomsg = msg) dtv(1:2,1:2)
         select type ( dtv )
            type is ( base )
...
program dummyArg006
   ...
   deallocate ( b1, b2 )
   allocate ( b1(3:6), source = (/ child ( 'ABC', 101 ), child ( 'DEF', 102 ), child ( 'GHI', 103 ), child ( 'JKL', 104 ) /) )
   allocate ( b2(2,2), source = reshape ( source = (/ child ( 'abc', 201 ), child ( 'def', 202 ), child ( 'ghi', 203 ), child ( 'jkl', 204 ) /), shape = (/2,2/) ) )
