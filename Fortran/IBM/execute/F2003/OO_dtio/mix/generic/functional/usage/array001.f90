...
program array001
   ...
   write ( 1, "(4(DT),/,3(DT))", iostat = stat, iomsg = msg ) c1, c2((/1,2,3/))
   ...
   read ( 1, "(6(1X,DT))", iostat = stat, iomsg = msg )                  b1, b2(1:3)
   ...
   read ( 1, "(4(DT),/,3(DT))", iostat = stat, iomsg = msg )  c1(1:2,1:2), c2
   ...
   read ( 1, "(2(1x,DT))", iostat = stat, iomsg = msg )                  b1
   if ( ( stat /= 0 ) .or. ( msg /= 'dtioreadc' ) )           error stop 8_4

   select type ( b1 )
      type is ( child )
         if ( ( b1(1)%c /= 'mno' ) .or. ( b1(1)%i /= 105 )   .or. &
              ( b1(2)%c /= 'pqr' ) .or. ( b1(2)%i /= 106 ) )  error stop 9_4
   end select
