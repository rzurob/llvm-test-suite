...
use m
    ...
    print *, c2%compare ((/(child1(i, 'test'), i=1,10)/))
    print *, all(c2%compare ((/(child1(i, 'test'), i=1,10)/)))

    if (.not. all (c2%compare ((/(child1(i, 'test'), i=1,10)/)))) error stop 1_4

    !! WE'RE TESTING THAT THE NEXT STATEMENT FAILS
    if (all (c2%compare ((/(child1(i, 'test'), i=1,10)/)))) error stop 2_4
end
