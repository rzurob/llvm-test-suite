...
program iosign042
    ...
    open (1,file="iosign042.1",form='formatted',access='stream', &
          sign='plus')
...
12  format(A4,2(X,F7.4))
    ...
    open (1,file="iosign042.1",form='formatted',position='append',&
          access='stream',sign='suppress')

    write (1,*) "\nTest 2\n"

13  format(A4,2(X,F5.2))
14  format(A4,2(X,F6.3))
15  format(A4,2(X,F7.4))

16  format(A4,2(X,F5.2))
17  format(A4,2(X,F6.3))
18  format(A4,2(X,F7.4))
    ...
    open (1,file="iosign042.1",form='formatted',position='append',&
          access='stream',sign='processor_defined')

    write (1,*) "\nTest 3\n"

19  format(A4,2(X,F5.2))
20  format(A4,2(X,F6.3))
21  format(A4,2(X,F7.4))
