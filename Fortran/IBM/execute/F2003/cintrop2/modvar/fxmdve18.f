...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdve18 cxmdve18 
! %COMPOPTS:   
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INT_FAST32_T , DIMENSION
!*
!*  DESCRIPTION                :
...
module mod
  USE ISO_C_BINDING
  integer(C_INT_FAST32_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INT_FAST32_T):: X,Y
    integer(C_INT_FAST32_T):: TX
    TX = X; X = Y; Y = TX
