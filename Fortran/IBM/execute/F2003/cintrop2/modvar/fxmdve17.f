...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/runint16.sh fxmdve17 cxmdve17 
! %COMPOPTS:   
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
module mod
  USE ISO_C_BINDING
  integer(C_INT_FAST16_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INT_FAST16_T):: X,Y
    integer(C_INT_FAST16_T):: TX
    TX = X; X = Y; Y = TX
