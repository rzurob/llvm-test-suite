...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdve06 cxmdve06
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INTPTR_T , DIMENSION
!*
!*  DESCRIPTION                :
...
module mod
  USE ISO_C_BINDING
  integer(C_INTPTR_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INTPTR_T):: X,Y
    integer(C_INTPTR_T):: TX
    TX = X; X = Y; Y = TX
