...
{ 
  ...
  if ( testf(a) && testf(ab) && testf(B) 
       && testd(c) && testl(D) && testf(dA)  
       && testd(cc) && testl(DD) )
      exit(60);
      
  for ( i = 0; i < 10; i++)
     if ( testf(a1[i]) &&  testf(b1[i]) 
          &&  testd(c1[i]) &&  testl(d1[i])  )
        exit(61);
        
  for ( i = 0; i <  10; i++ )
    for (j = 0; j < 10; j++)
     if ( testf(a2[i][j]) && testf(B2[i][j]) 
          && testd(c2[i][j]) && 
            testl(d2[i][j])  )
         exit(62);
  ...
  if ( testf(a - 1.0f) && testf(ab- 1.0f) && testf(B- 1.0f) 
       && testd(c - 1.0) && testl(D - 1.0l) && testf(dA- 1.0f)  
