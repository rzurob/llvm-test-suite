...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdvg06 cxmdvg06 
! %COMPOPTS:   -qmixed
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
!*  ===================================================================
!*
!*  TEST CASE NAME             : fxmdvg06.f 
!*  TEST CASE TITLE            : bind(c) attribute/statement
!*
!*  PROGRAMMER                 : Kan Tian
!*  DATE                       : Sep 14,2002 
!*
!*  PRIMARY FUNCTIONS TESTED   :test integer(c_intptr_t) with bind(c) 
!*                              attribute/statement 
...
module mod
  ...
  integer(c_intptr_t), bind(c,name="xaa") :: fx=1

  integer(c_intptr_t):: fy =2 
  bind(c,name="Xaa") :: fy
...
program testmix
  use mod
  logical ::result
  integer(c_intptr_t)::basval1,basval2
