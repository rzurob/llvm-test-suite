...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdve07 cxmdve07 
! %COMPOPTS:   
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INTMAX_T , DIMENSION
!*
!*  DESCRIPTION                :
...
module mod
  USE ISO_C_BINDING
  integer(C_INTMAX_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INTMAX_T):: X,Y
    integer(C_INTMAX_T):: TX
    TX = X; X = Y; Y = TX
