...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdvc15 cxmdvc15 
! %COMPOPTS:   
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
module mod
  USE ISO_C_BINDING
  integer(C_INT_LEAST64_T),dimension(0:5,0:4,0:3), bind(c) :: c3darray
end module mod

subroutine fsub()
  use mod
  integer(C_INT_LEAST64_T),dimension(0:5,0:4,0:3) :: f3darray
  integer::i,j,k
  logical::res
  ...
  DO i = 0, 3, 1
     Do j=0,4,1
        Do k=0,5,1
           f3darray(k,j,i)=100*(i+1)+10*(j+1)+k
