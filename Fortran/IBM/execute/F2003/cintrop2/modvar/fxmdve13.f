...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdve13 cxmdve13 
! %COMPOPTS:   
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INT_LEAST16_T , DIMENSION
!*
!*  DESCRIPTION                :
...
module mod
  USE ISO_C_BINDING
  integer(C_INT_LEAST16_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INT_LEAST16_T):: X,Y
    integer(C_INT_LEAST16_T):: TX
    TX = X; X = Y; Y = TX
