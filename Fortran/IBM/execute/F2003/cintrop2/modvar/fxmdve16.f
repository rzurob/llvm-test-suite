...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdve16 cxmdve16
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
module mod
  USE ISO_C_BINDING
  integer(C_INT_FAST8_T), DIMENSION(5),bind(c) :: A
...
CONTAINS
  ...
  SUBROUTINE EXCHANGE (X,Y)
    USE ISO_C_BINDING
    integer(C_INT_FAST8_T):: X,Y
    integer(C_INT_FAST8_T):: TX
    TX = X; X = Y; Y = TX
