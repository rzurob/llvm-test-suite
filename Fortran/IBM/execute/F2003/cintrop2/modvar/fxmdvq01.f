...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/fxmdvn01.sh fxmdvq01 cxmdvq01
! %COMPOPTS: -qfree=f90
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*                              : are initialized in Fortran main(), then call
!*                              : C subroutine csub(). In C subroutine, the
!*                              : value of variables are checked. If is not
!*                              : equal to the initialized values in Fortran,
!*                              : test case exit with non_zero number. Then,
!*                              : C subroutine calls a Fortran subroutine
!*                              : fsub() to modify the value of the variables.
!*                              : Then csub() checks the variables to verify
!*                              : the modification. After that, csub() change
!*                              : the value of the variables again and return
!*                              : to Fortran mian().  values of the variables
!*                              : are checked in Fortran main() to verify value
!*                              : have been changed in C subroutine.
!*                              : The C subroutine is complied by xlc under AIX,
