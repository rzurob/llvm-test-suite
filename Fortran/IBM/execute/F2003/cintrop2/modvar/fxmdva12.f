...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdva12 cxmdva12
! %COMPOPTS:  
! %GROUP: redherring.f 
! %VERIFY:
! %STDIN:
...
module mod
  ...
  integer(C_INT_LEAST8_T), bind(c) :: x1

  ! Use of bind(c) statement before variable declaration
  bind(c) :: x2
  integer(C_INT_LEAST8_T) :: x2

  integer(C_INT_LEAST8_T), bind(c,name="x3") :: fx3

  ! Use of bind(c) statement after  variable declaration
  integer(C_INT_LEAST8_T) :: fx4
  bind(c,name="x4") :: fx4 
...
CONTAINS
  ...
  function assert4(n1,n2)
    INTEGER(C_INT_LEAST8_T), INTENT(IN) :: n1,n2
    LOGICAL assert4
    if (n1 .NE. n2) then
       assert4= .FALSE.
    else
...
subroutine fsub()
  use mod
  integer(C_INT_LEAST8_T) :: basval1,basval2,basval3,basval4
  integer case_id
  LOGICAL ::result
