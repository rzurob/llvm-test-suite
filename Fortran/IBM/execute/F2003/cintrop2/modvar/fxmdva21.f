...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/run.sh fxmdva21 cxmdva21
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
module mod
  ...
  integer(C_SHORT), bind(c) :: x1

  ! Use of bind(c) statement before variable declaration
  bind(c) :: x2
  integer(C_SHORT) :: x2

  integer(C_SHORT), bind(c,name="x3") :: fx3
...
subroutine fsub()
  use mod
  integer(C_SHORT) :: basval1,basval2,basval3,basval4
  integer case_id
  LOGICAL ::result
