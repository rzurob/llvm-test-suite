...
module m
    ...
    function distribute (p1, m, n)
        ...
        type(point), allocatable :: distribute (:,:)
        ...
        allocate(distribute(m,n))

        k = 1

        do j = 1, n
            do i = 1, m
                distribute(i,j) = point(radius*[cos(theta+delta*k), &
                    sin(theta+delta*k)])
...
use m
    ...
    if (.not. ref%equal(point(1.0))) error stop 4_4
    ...
    if (.not. ref%equal(point(1.0))) error stop 6_4
