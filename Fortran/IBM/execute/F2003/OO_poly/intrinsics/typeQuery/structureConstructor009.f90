...
! %PRECMD: rm -f *.mod
! %COMPOPTS: -qfree=f90
! %GROUP: structureConstructor009.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
...
use m
    ...
    if(.NOT. extends_type_of(Base(1), mold1)) error stop 5_4
    if(.NOT. extends_type_of(Child(1, "abc"), mold2)) error stop 6_4
    if(.NOT. same_type_as(Base(1), mold1)) error stop 7_4
    if(.NOT. same_type_as(Child(1, "abc"), mold2)) error stop 8_4
end
