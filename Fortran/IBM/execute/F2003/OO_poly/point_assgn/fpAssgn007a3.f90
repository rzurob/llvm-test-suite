...
use m1
    ...
    interface test
        ...
        use m1
            class (container1), intent(inout) :: c1, c2
        end subroutine

        subroutine testContainer2 (c1, c2)
        use m1
            class (container2), intent(inout) :: c1, c2
        end subroutine
    end interface

    type (container1) :: co1, co2

    type (container2) :: co11, co12
...
use m1
    type (container1), intent(inout) :: c1, c2

    !! initially both pointers are disassociated
    if ((c1%data%typeID() /= 1) .or. (c2%data%typeID() /= 1)) error stop 5_4

    allocate (child :: c2%data)
