  ...
  INTEGER :: i
    ...
    CLASS IS (Child)
      ...
      CLASS IS (Child)
        ...
        IF ( KIND(U%Int) .NE. 1 )              ERROR STOP 35

        IF ( ANY(U%Cplx       .NE. (-8.0_8, 8.0_8) ))   ERROR STOP 35
        IF ( KIND(U%Cplx) .NE. 8 )                      ERROR STOP 35

        IF ( ANY(U%L        .NEQV. .TRUE._8) )   ERROR STOP 35
        IF ( KIND(U%L) .NE. 8 )                  ERROR STOP 35

        IF ( ANY(U%C    .NE. Str) )   ERROR STOP 35
        IF ( LEN(U%C) .NE. 1025 )     ERROR STOP 35
