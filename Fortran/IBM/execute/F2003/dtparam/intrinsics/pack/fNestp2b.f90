...
contains
		...
		function arrayvector_multi(matrixA, matrixB) result (X)
			type(admatrix(4)), intent(in), dimension(3) :: matrixA
			type(admatrix(4)), intent(in), dimension(3) :: matrixB
			real :: X(3)
			do i = 1,3
...
type (admatrix(4)) :: vec(2,3), field2(2,3)
...
real :: res2(6)
	...
	res = pack(vec, mask2)
 	print *, pack(vec, mask2) * pack(vec, mask2)
	do i = 1,3
