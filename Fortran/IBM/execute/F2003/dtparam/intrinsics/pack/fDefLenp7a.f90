...
!*                                Derived type component has allocatable attribute.
!*
module m1
    type dtp(l)
        integer, len :: l
...
type (dtp(:)), allocatable :: matrixA(:,:)    !dimension(4,4)
type (dtp(:)), allocatable :: vec(:), res(:)    !dimension(8)
logical :: mask1(4,4)
integer :: num
num = 1
...
allocate( res(lbound(pack (point, mask1, vec),1):ubound(pack (point, mask1, vec),1)), SOURCE = pack (point, mask1, vec))
