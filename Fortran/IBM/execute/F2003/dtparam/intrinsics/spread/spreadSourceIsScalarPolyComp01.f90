...
!*  5. SOURCE HAS POLYMORPHIC COMPONENT
!*  6. DEFECT 357496
...
program spreadSourceIsScalarPolyComp01
  use m
  implicit none
  ...
  allocate(poly2,source=child(2,1,4,3)(i1=3,c1="a",c2=["00","11","22"]) )

  if(allocated(contain1)) then
     allocate(contain1%poly1,source=poly2)
  end if

  call verify(spread(contain1,1,5))
  deallocate(contain1%poly1)
       ...
       do i=lbound(dt,1),ubound(dt,1)
          ...
          select type(x=>dt(i)%poly1)
             type is(child(2,*,4,*))
                ...
                if(x%c1%len /= 1)                          error stop 17_4
                if(x%c2%len /= 4)                          error stop 18_4
                if(lbound(x%c2,1) /= 2)                    error stop 19_4
                if(ubound(x%c2,1) /= 4)                    error stop 20_4
                if(any(x%c2  /= ["00","11","22"]))         error stop 21_4
                if(x%i1 /= 3)                              error stop 22_4
             type is(base(2,*))
                if(x%k1 /= 2)                              error stop 23_4
                if(x%l1 /= 1)                              error stop 24_4
                if(x%i1 /= 10)                             error stop 25_4
                if(x%c1 /= "x")                            error stop 26_4
             class default
                error stop 100_4
           end select
       end do
