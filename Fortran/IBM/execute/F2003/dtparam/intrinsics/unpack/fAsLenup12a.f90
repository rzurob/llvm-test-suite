...
use m1
type (admatrix(4,:)), allocatable :: res(:,:) !res(2,3) 
type (admatrix(4,:)),allocatable :: field(:,:) !field(2,3) 
type (admatrix(4,:)),allocatable :: vec1(:), vec2(:) !vec(3) 
logical :: mask1(6) = (/.TRUE., .FALSE., .TRUE., .FALSE., .TRUE., .FALSE./)
logical :: mask2(2,3)
real :: num
real :: res2(2,3)
allocate(admatrix(4,3)::vec1(3))
	num = 1.0
	do j = 1,3
		do i = 1,3
			allocate(vec1(j)%row(i)%element, SOURCE = num)
			num = num + 1.0
		end do 
	end do
allocate(vec2(ubound(vec1,1)),SOURCE=vec1)
allocate(admatrix(4,3)::field(2,3))	
	do k =1,2
	do i = 1,3
		do j =1,3
...
allocate(res(ubound(unpacker(vec2,field,mask2),1),ubound(unpacker(vec2,field,mask2),2)), &
	SOURCE = unpacker(vec2,field,mask2))	
