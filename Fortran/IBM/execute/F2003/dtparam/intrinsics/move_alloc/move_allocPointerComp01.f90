...
!*  ===================================================================
!*
!*  TEST CASE NAME             : move_allocPointerComp01.f   
!*  TEST CASE TITLE            :
!*
...
!*  DESCRIPTION
!*  1. SECTION 13.7.82
!*  2. COMPONENTS ARE POINTERS, SOME LENGTH PARAMETER ARE DEFERRED
...
module m
   ...
   type second(l2)
     ...
     type(first(:)),pointer :: first1=>null() 
   end type
   type third(l3)
     integer,len  :: l3
     type(first(:)),pointer  :: first2=>null()
     type(second(:)),pointer :: second1=>null()
   end type
end module

program move_allocPointerComp01
  ...
  if(.not. associated(to1%first2))            error stop 12_4
  if(.not. associated(to1%first2%i1))         error stop 13_4
  if(.not. associated(to1%first2%i2))         error stop 14_4
  if(.not. associated(to1%second1))           error stop 15_4
  if(.not. associated(to1%second1%i3))        error stop 16_4
  if(.not. associated(to1%second1%first1))    error stop 17_4
  if(.not. associated(to1%second1%first1%i1)) error stop 18_4
  if(.not. associated(to1%second1%first1%i2)) error stop 19_4
