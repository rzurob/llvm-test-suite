...
MODULE Mod 
      ...
      SUBROUTINE CheckBase(Arg)
        ...
        IF ( Arg%k1 .NE. 4 ) STOP 11
        IF ( Arg%l1 .NE. 3 ) STOP 12
        IF ( SIZE(Arg%A0) .NE. 3 ) STOP 13
        IF ( LEN(Arg%C0)  .NE. 3 ) STOP 14
        IF ( ANY(Arg%A0   .NE.   -1) ) STOP 15
        IF ( TRIM(Arg%C0) .NE. 'Bas' ) STOP 16
      ...
      SUBROUTINE CheckNextGen(Arg)
        ...
        IF ( Arg%k1 .NE. 4 ) STOP 20
        IF ( Arg%l1 .NE. 3 ) STOP 21
...
PROGRAM DTP_PARAMETER_06
      ...
      SELECT TYPE ( poly )
          CLASS IS (NextGen(4,*,4,*,*))
             IF ( ASSOCIATED(poly%ptr) .nEQV. ASSOCIATED(ntgt%ptr) ) STOP 60
            CALL poly%sub2()
