...
PROGRAM DTP_ACE_11
      ...
      IF ( .NOT. ALLOCATED(c1) ) ERROR STOP 10

      b1 = [Base(4,10) :: Base(4,10)([(I, I=1, 10)]), Base(4,10)([(I+10, I=1, 10)]), Base(4,10)([(I+20, I=1, 10)])]
      IF ( .NOT. ALLOCATED(b1) ) ERROR STOP 11
      IF ( SIZE(b1)    .NE.  3 ) ERROR STOP 12
      DO I = 1, SIZE(b1)
         IF ( SIZE(b1(I)%A1) .NE. 10 ) ERROR STOP 13
      END DO

      c1 = b1
      IF ( .NOT. ALLOCATED(c1%poly) ) ERROR STOP 14
      IF ( SIZE(c1%poly)    .NE.  6 ) ERROR STOP 15
      DO I = 1, SIZE(c1%poly)
         IF ( SIZE(c1%poly(I)%A1) .NE. 10 ) ERROR STOP 16
      END DO
      print*, c1%poly

      c1 = b1(3:1:-2)
      IF ( .NOT. ALLOCATED(c1%poly) ) ERROR STOP 17
      IF ( SIZE(c1%poly)    .NE.  3 ) ERROR STOP 18
      DO I = 1, SIZE(c1%poly)
         IF ( SIZE(c1%poly(I)%A1) .NE. 10 ) ERROR STOP 19
      END DO
      print*, c1%poly
