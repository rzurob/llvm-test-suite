...
PROGRAM DTP_ACE_05
      ...
      J = M
      carr = [Child(K,J,K,J) :: Child(K,J,K,J)(), c1, Child(K,J,K,J)(), c2, Child(K,J,K,J)(R0=0.98)]
      IF ( SIZE(carr) .NE. 5 ) ERROR STOP 30
      DO I = 1, 5
        IF ( SIZE(carr(I)%A01)     .NE.   J ) ERROR STOP 31
        IF ( SIZE(carr(I)%A02)     .NE. J+1 ) ERROR STOP 32
        IF ( SIZE(carr(I)%A2)      .NE.   J ) ERROR STOP 33
        IF ( LBOUND(carr(I)%A01,1) .NE.   1 ) ERROR STOP 34
        IF ( LBOUND(carr(I)%A02,1) .NE.   1 ) ERROR STOP 35
        IF ( LBOUND(carr(I)%A2,1)  .NE.   1 ) ERROR STOP 36
        IF ( UBOUND(carr(I)%A01,1) .NE.   J ) ERROR STOP 37
        IF ( UBOUND(carr(I)%A02,1) .NE. J+1 ) ERROR STOP 38
        IF ( UBOUND(carr(I)%A2,1)  .NE.   J ) ERROR STOP 39
      END DO
      IF ( ANY(carr(1)%A01 .NE. -1) ) ERROR STOP 40
      IF ( ANY(carr(2)%A01 .NE.  1) ) ERROR STOP 41
      IF ( ANY(carr(3)%A01 .NE. -1) ) ERROR STOP 42
