...
!*  ===================================================================
!*
!*  TEST CASE NAME             : F2003/dtparam/ArraySections/arrsec_1_2.f
!*  DATE                       : Jul. 09, 2008
...
!*  DESCRIPTION:
!*
!*  Variation of arrsec_1_1.f with:
!*  - Array section dummy args copied to a static variable which is then passed
!*    as the array section to another subroutines
!*
...
contains
    ...
    subroutine sub1(str,dta)
        ...
        type(base(K,:)), allocatable, save :: sdtarrobj(:)
        if (.not.allocated(sdtarrobj)) then
            allocate(sdtarrobj(N), source=dta)
        end if
        call sub1_doit(str,sdtarrobj)
        dta = sdtarrobj
    end subroutine
