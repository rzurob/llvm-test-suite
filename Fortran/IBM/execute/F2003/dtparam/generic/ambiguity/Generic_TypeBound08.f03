...
!*
      ...
      IF ( base1%size() .NE. size_obj4(base1) ) ERROR STOP 10
      IF ( base1%size() .NE. size(base1%base_arr) ) ERROR STOP 11
      ...
      SELECT TYPE (ptr1)
          TYPE IS (Base(4,*))
           IF (ptr1%size() .NE. size_obj4(ptr1)) ERROR STOP 13
           IF (ptr1%size() .NE. size(ptr1%base_arr)) ERROR STOP 14
      ...
      SELECT TYPE (ptr1)
          TYPE IS (Child(4,*,4,*))
           IF (ptr1%size() .NE. size_obj4(ptr1)) ERROR STOP 17
           IF (ptr1%size() .NE. (size(ptr1%base_arr)+size(ptr1%child_arr)) )ERROR STOP 18

          CLASS DEFAULT
           STOP 19
      END SELECT

      IF ( dtv%size() .NE. size_obj8(dtv) ) ERROR STOP 20
      IF ( dtv%size() .NE. (size(dtv%base_arr)+size(dtv%child_arr)+size(dtv%nextg_arr)) ) ERROR STOP 21
