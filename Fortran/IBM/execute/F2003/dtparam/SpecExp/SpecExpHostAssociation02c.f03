...
PROGRAM SpecExpHostAssociation02c
      ...
      SUBROUTINE Sub12(N, M)
        ...
        TYPE(Child(c1%ptr%k1,c1%ptr%l1,c1%ptr%k1,c1%ptr%l1)) :: Obj
         ...
         IF ( Obj%k2 .NE. N ) ERROR STOP 16
         IF ( Obj%l2 .NE. M ) ERROR STOP 17
         IF ( SIZE(Obj%A1) .NE. M ) ERROR STOP 18
         IF ( LEN(Obj%C1)  .NE. M ) ERROR STOP 19

         ALLOCATE( Base(Obj%k2,Obj%l2) :: Obj%ptr )
         IF ( Obj%ptr%k1 .NE. N ) ERROR STOP 20
         IF ( Obj%ptr%l1 .NE. M ) ERROR STOP 21
         IF ( SIZE(Obj%ptr%A1) .NE. M ) ERROR STOP 22
         IF ( LEN(Obj%ptr%C1)  .NE. M ) ERROR STOP 23
      ...
      SUBROUTINE Sub14(N, M, P)
        ...
        TYPE(Child(KIND(c1%ptr%A1(1)),c1%ptr%A1(1),KIND(c1%ptr%A1(c1%l2)),c1%ptr%A1(c1%l2))) :: Obj
         ...
         IF ( Obj%k1 .NE. N ) ERROR STOP 34
         IF ( Obj%l1 .NE. M ) ERROR STOP 35
         IF ( Obj%k2 .NE. N ) ERROR STOP 36
         IF ( Obj%l2 .NE. P ) ERROR STOP 37
         IF ( SIZE(Obj%A1) .NE. M ) ERROR STOP 38
         IF ( LEN(Obj%C1)  .NE. M ) ERROR STOP 39

         ALLOCATE( Base(Obj%k2,Obj%l2) :: Obj%ptr )
         IF ( Obj%ptr%k1 .NE.   N ) ERROR STOP 40
         IF ( Obj%ptr%l1 .NE.   P ) ERROR STOP 41
         IF ( SIZE(Obj%ptr%A1) .NE. P ) ERROR STOP 42
         IF ( LEN(Obj%ptr%C1)  .NE. P ) ERROR STOP 43
      ...
      SUBROUTINE Sub15(N, M, P)
        ...
        TYPE(Base(KIND(c1%ptr%I1),LEN(c1%ptr%C1))) :: Obj(c1%ptr%I1)
         ...
         IF ( SIZE(Obj) .NE. P ) ERROR STOP 44
         DO I = 1, c1%ptr%I1
            IF ( Obj(I)%k1 .NE. N ) ERROR STOP 45
            IF ( Obj(I)%l1 .NE. M ) ERROR STOP 46
            IF ( SIZE(Obj(I)%A1) .NE. M ) ERROR STOP 47
            IF ( LEN(Obj(I)%C1)  .NE. M ) ERROR STOP 48
         END DO
