...
!* =====================================================================
!*
!*  TEST CASE NAME             : dtpUseAssociated03a
!*
!*  DATE                       : July 13, 2009
!*  ORIGIN                     : AIX Compiler Development,
!*
!*  PRIMARY FUNCTIONS TESTED   : Multiple levels of USE Association with
...
MODULE mBase
    ...
    TYPE tBase(labelSize)
        INTEGER, LEN :: labelSize = LABEL_SIZE

        CHARACTER(labelSize) :: label = 'Blank'
...
MODULE mExt
    ...
    TYPE, EXTENDS(tBase) :: tExt(dataSize)
        INTEGER, LEN :: dataSize = 3

        REAL :: data( dataSize ) = 0.0
...
MODULE mDoubleExt
    ...
    TYPE, EXTENDS(tExt) :: tDoubleExt(length)
        ...
        INTEGER :: array( length,length ) = 0
    END TYPE tDoubleExt


    TYPE(tDoubleExt), SAVE :: DoubleExt
...
MODULE mData
    USE mDoubleExt, DoubleExt => Ext, Ext => Base, Base => DoubleExt

    IMPLICIT NONE

    INTEGER, PARAMETER :: MAX_ELEMENTS = 3

    CHARACTER(Ext%labelSize) :: labelList( MAX_ELEMENTS )
...
MODULE mTestBase
    USE mData, DoubleExt => Base, Ext => Ext, Base => DoubleExt
...
END PROGRAM dtpUseAssociated03a
...
SUBROUTINE TestDoubleExt(realArray, rc)
    ...
    INTEGER :: intArray( Base%length,Base%length,MAX_ELEMENTS )
    INTEGER :: controlIntArray( 2,2,3 )

    TYPE(tDoubleExt(Base%labelSize,Base%dataSize,Base%length))&
    ...
    DO i = 1, 3
        doubleExtTest( (i - 1) ) =&
            tDoubleExt(Base%labelSize,Base%dataSize,Base%length)&
