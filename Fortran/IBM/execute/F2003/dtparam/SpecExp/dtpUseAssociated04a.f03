...
!*                               Associated by a Subprogram
!*  SECONDARY FUNCTIONS TESTED : Where the Derived Types have Default
!*                               Initialization
!*
...
MODULE mBase
    ...
    TYPE tBase(arrayDim1,arrayDim2,arrayKind)
        ...
        INTEGER, KIND :: arrayKind = 16

        REAL(arrayKind) :: array( arrayDim1,arrayDim2 ) = 0.0
    ...
    TYPE(tBase), SAVE, TARGET :: base( 10 )
...
MODULE mVariation1
    ...
    TYPE, EXTENDS(tBase) :: tVariation1(variation1Kind)
        INTEGER, KIND :: variation1Kind = 8

        REAL(variation1Kind) :: variation1Array( arrayDim1,arrayDim2 ) = 0.0
    END TYPE tVariation1


    TYPE(tVariation1(arrayDim1=:,arrayDim2=:)), POINTER :: variation1( : )
...
MODULE mVariation2
    ...
    TYPE, EXTENDS(tBase) :: tVariation2(variation2Kind)
        INTEGER, KIND :: variation2Kind = 4

        INTEGER(variation2Kind) :: variation2Array( arrayDim1,arrayDim2 ) = 0
    END TYPE tVariation2

END MODULE mVariation2


PROGRAM dtpUseAssociated04a
    USE mVariation1
    USE mVariation2
    ...
    ALLOCATE(realArray( (base%arrayDim1 * base%arrayDim2 * SIZE( base )) ), &
            ...
            SOURCE=[ ((1.0_16 / REAL(i, 16)), i = 1,                        &
                           (base%arrayDim1 * base%arrayDim2 * SIZE( base ))) ])
    ...
    DO i = 1, 10
        j = ((i - 1) * 12) + 1
        base( i ) =                                         &
            tBase(arrayKind=base%arrayKind)                 &
...
END PROGRAM dtpUseAssociated04a
...
SUBROUTINE DoubleUse( rc )
    ...
    ALLOCATE(tVariation1(arrayKind=variation1%arrayKind,            &
                    variation1Kind=variation1%variation1Kind) ::    &
    ...
    DO i = 1, base%arrayDim2
        ...
        variation1( i )%variation1Array = REAL(variation1( i )%array, 8)
    END DO
