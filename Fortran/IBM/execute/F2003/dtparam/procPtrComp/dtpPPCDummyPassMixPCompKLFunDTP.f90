...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpPPCDummyPassMixPCompKLFunDTP
!*
!*  DATE                       : 2009-03-23
!*
!*  PRIMARY FUNCTIONS TESTED   : Procedure Pointers as Components
!*
!*  SECONDARY FUNCTIONS TESTED : reference to pass function with mixed dummy args, K+L, DTP result, ref via dummy arg, interface via other components
!*
...
!*  The interface for the procedure pointers comes from an interface already
!*  established for other procedure pointer components, which must be explicit.
!*
!* ============================================================================
!234567890123456789012345678901234567890123456789012345678901234567890123456789

module dtpPPCDummyPassMixPCompKLFunDTPmod
  ...
  type dt (k,l)
     ...
     procedure (f1), pointer, pass :: x1 => null()  ! explicit from interface below
     procedure (f2), pointer, pass :: x2 => null()  ! explicit from interface below
     procedure (cat), pointer, pass :: x4 => null() ! explicit from module procedure below

     procedure (x1), pointer, pass :: p1 => null()  ! derives from procedure component
     procedure (x2), pointer, pass :: p2 => null()  ! ditto
     procedure (x4), pointer, pass :: p4 => null()  ! ditto
  end type dt
...
contains
  ...
  ! We use pointers to the "poison" functions to initialise components which should not be invoked.
  ! (Since there are some components which are merely there to test the ability of the compiler
  ! to use their interface to define the interface for another pointer component, we want to rule
  ! out the possibility that the reference is being used too directly.)
  type(dt(1,40)) function poison1(this,i,ch)
    class(dt(1,*)), intent(in) :: this
    integer(1), intent(in)  :: i
    character(*), intent(in) :: ch
    poison1 = dt(1,40)("",0)
    stop 2
  end function poison1

  type(dt(2,40)) function poison2(this,factor)
    class(dt(2,*)), intent(in) :: this
    integer(2), intent(in)  :: factor
    poison2 = dt(2,40)("",0)
    stop 3
  end function poison2

  type(dt(4,40)) function poison4(this,that)
    class(dt(4,*)), intent(in) :: this, that
    poison4 = dt(4,40)("",0)
    stop 4
  end function poison4
...
end module dtpPPCDummyPassMixPCompKLFunDTPmod


program dtpPPCDummyPassMixPCompKLFunDTP

  use dtpPPCDummyPassMixPCompKLFunDTPmod
  implicit none
  ...
  t1a_h = dt(1,0)("",127,x1=poison1,x2=poison2,x4=poison4,p1=add,p2=dup,p4=cat)
  t1b_h = dt(1,0)("",-127,x1=poison1,x2=poison2,x4=poison4,p1=sub,p2=shrink,p4=cat)
  t2a_h = dt(2,4)("abcd",32000,x1=poison1,x2=poison2,x4=poison4,p1=add,p2=dup,p4=cat)
  t2b_h = dt(2,4)("efgh",-12345,x1=poison1,x2=poison2,x4=poison4,p1=sub,p2=shrink,p4=cat)
  t1c_h = dt(1,3)("ijk",1,x1=poison1,x2=poison2,x4=poison4,p1=add,p2=dup,p4=cat)
  t1d_h = dt(1,3)("lmn",-1,x1=poison1,x2=poison2,x4=poison4,p1=sub,p2=shrink,p4=cat)
  t1p_h => t1c_h
  allocate(t4p_h, source=dt(4,5)("opqrs",123454321,x1=poison1,x2=poison2,x4=poison4,p1=add,p2=dup,p4=cat))
  ...
  allocate(t1p_h, source=dt(1,5)("opqrs",0,x1=poison1,x2=poison2,x4=poison4,p1=sub,p2=shrink,p4=cat))
...
end program dtpPPCDummyPassMixPCompKLFunDTP
