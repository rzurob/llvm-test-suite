...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpPPCDummyPassMixPCompKLSub
!*
!*  DATE                       : 2009-03-23
!*
!*  PRIMARY FUNCTIONS TESTED   : Procedure Pointers as Components
!*
!*  SECONDARY FUNCTIONS TESTED : pointer to PASS subroutines from DTP with K+L param with other args of mixed types via dummy arg, interface via other components
!*
...
!*  ADAPTED FROM               : dtpPPCDummyPassMixInterfaceKLSub (<-dtpPPCLocalPassMixInterfaceKLSub<-dtpPPCLocalNPassDTPInterfaceKLSub<-dtpPPCBasicNoPassSubroutine)
!*
!*  DESCRIPTION
!*
...
!*  The interface is declared via procedure pointer components which are declared
!*  earlier in the type and which are themselves defined explicitly either by
!*  reference to an abstract interface or to a module procedure.
!*
!* ============================================================================
!234567890123456789012345678901234567890123456789012345678901234567890123456789

module dtpPPCDummyPassMixPCompKLSubmod
  ...
  type dt (k,l)
     ...
     procedure (s1), pointer, pass     :: x1 => null()  ! explicit from interface below
     procedure (s1), pointer, pass(a2) :: x2 => null()  ! explicit from interface below
     procedure (s1), pointer, pass(a4) :: x4 => null()  ! explicit from interface below
     procedure (s5), pointer, pass     :: xR => null()  ! explicit from module procedure below
     procedure (s6), pointer, pass     :: xCI => null() ! explicit from module procedure below

     procedure (x1), pointer, pass     :: p1 => null()
     procedure (x2), pointer, pass(a2) :: p2 => null()
     procedure (x4), pointer, pass(a4) :: p4 => null()
     procedure (xR), pointer, pass     :: pR => null()
     procedure (xCI), pointer, pass    :: pCI => null()
  end type dt
...
contains
  ...
  subroutine s5(this,r1)
    ...
    real(4), intent(in) :: r1
    stop 2 ! this should never be actually called
  end subroutine s5
  ...
  subroutine s6(this,ch,i1)
    ...
    stop 3 ! this should never be actually called
  end subroutine s6
  ...
  subroutine disp1x(this,that,that2)
    class(dt(1,*)), intent(in) :: this
    class(dt(2,*)), intent(in) :: that
    class(dt(4,*)), intent(in) :: that2
    stop 4  ! this should never be actually called
  end subroutine disp1x

end module dtpPPCDummyPassMixPCompKLSubmod


program dtpPPCDummyPassMixPCompKLSub

  use dtpPPCDummyPassMixPCompKLSubmod
  implicit none
  ...
  t1a_h = dt(1,0)("",127,disp1x,disp1x,disp1x,s5,s6,disp1,disp1,disp1,dispR,dispCI)
  t1b_h = dt(1,0)("",-127,disp1x,disp1x,disp1x,s5,s6,disp1a,disp1a,disp1a,dispRa,dispCI)
  t2a_h = dt(2,4)("abcd",32000,disp1x,disp1x,disp1x,s5,s6,disp1,disp1,disp1,dispR,dispCI)
  t2b_h = dt(2,4)("efgh",-12345,disp1x,disp1x,disp1x,s5,s6,disp1a,disp1a,disp1a,dispRa,dispCI)
  t1c_h = dt(1,3)("ijk",1,disp1x,disp1x,disp1x,s5,s6,disp1,disp1,disp1,dispR,dispCI)
  t1d_h = dt(1,3)("lmn",-1,disp1x,disp1x,disp1x,s5,s6,disp1a,disp1a,disp1a,dispRa,dispCI)
  t1p_h => t1c_h
  allocate(t4ap_h, source=dt(4,5)("opqrs",123454321,disp1x,disp1x,disp1x,s5,s6,disp1,disp1,disp1,dispR,dispCI))
  allocate(t4bp_h, source=dt(4,5)("tuvwx",123454321,disp1x,disp1x,disp1x,s5,s6,disp1a,disp1a,disp1a,dispRa,dispCIa))
...
end program dtpPPCDummyPassMixPCompKLSub
