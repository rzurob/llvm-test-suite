...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  if ((res == einval) .or. (res == enomem)) error stop 199

  IF ( ASSOCIATED(T) ) ERROR STOP 10
  ...
  if ((res == eagain) .or. (res == einval) .or. (res == enomem)) error stop 100

  ! Verification
  IF ( .NOT. ASSOCIATED(T) ) ERROR STOP 11
  IF ( SIZE( T )  .NE. N   ) ERROR STOP 12
  if (Mutex /= 1) error stop 13
  ...
  if ((res == eagain) .or. (res == einval) .or. (res == enomem)) error stop 101

  res = f_pthread_join (Thread2)
  if ((res == eagain) .or. (res == einval) .or. (res == enomem)) error stop 102
