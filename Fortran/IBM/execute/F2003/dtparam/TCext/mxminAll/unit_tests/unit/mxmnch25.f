...
! ftcx_dtp -qk /tstdev/F2003/mxminAll/unit_tests/unit/mxmnch25.f
...
!234567890123456789012345678901234567890123456789012345678901234567890
      ...
      if (maxval(adt%aa(:,:,2:3)) /= 'ibm') error stop 1
      if (maxval(adt%aa(:,1:2,:)) /= 'usa') error stop 2
      if (maxval(adt%aa(2:2,:,:)) /= 'jan') error stop 3

      allocate(xx(3,2))
      xx=maxval(adt%aa(:,:,2:3),dim=1,mask=adt%aa(:,:,2:3) .ne. 'x')
      ...
      allocate(xx(2,4))
      xx=maxval(adt%aa(:,1:2,:),dim=2,mask=adt%aa(:,1:2,:) .ne. 'a')
      ...
      if (minval(adt%aa(:,2:3,1:4:2)) /= 'bel') error stop 19
      if (minval(adt%aa(:,1:3:2,:)) /= 'bel') error stop 20
      if (minval(adt%aa(1:1,:,:)) /= 'bel') error stop 21

      allocate(xx(3,2))
      xx=minval(adt%aa(:,:,2:3),dim=1,mask=adt%aa(:,:,2:3) .ne. 'x')
      if (xx(1,1) /= '~') error stop 22
      if (xx(2,1) /= 'can') error stop 23
      if (xx(3,1) /= '~') error stop 24
      if (xx(1,2) /= 'ibm') error stop 25
      if (xx(2,2) /= '~') error stop 26
      if (xx(3,2) /= '~') error stop 27
      deallocate(xx)

      allocate(xx(2,4))
      xx=minval(adt%aa(:,1:2,:),dim=2,mask=adt%aa(:,1:2,:) .ne. 'a')
      if (xx(1,1) /= '~') error stop 28
      if (xx(2,1) /= 'jan') error stop 29
      if (xx(1,2) /= 'can') error stop 30
      if (xx(2,2) /= '~') error stop 31
      if (xx(1,3) /= '~') error stop 32
      if (xx(2,3) /= 'ibm') error stop 33
      if (xx(1,4) /= 'usa') error stop 332
      if (xx(2,4) /= '~') error stop 333
      deallocate(xx)

      allocate(xx(1,3))
      xx = minval(adt%aa(2:2,:,:),dim=3)
