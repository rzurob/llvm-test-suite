...
!*  ===================================================================
!*
!*  TEST CASE NAME             : ffinal024ikl2.f
!*  TEST CASE NAME             : type-bound procedure ffinal024ikl2
!*
!*  PROGRAMMER                 : David Forster (derived from ffinal024i by Catherine Sun)
...
use m1
   type dt (kdt_1,kdt_2) ! kdt_1,kdt_2=4,4
      ...
      type(child(kdt_2,:)), pointer :: dt_c(:,:)  ! tcx: (kdt_2,:)
   end type
end module
...
use m2, only : dt
   ...
   allocate(t1(2))
   allocate(t1(1)%dt_b) 
   allocate(child(4,7):: t1(1)%dt_c(2,2)) ! tcx: child(4,7)
   allocate(t1(2)%dt_b)
   allocate(child(4,7):: t1(2)%dt_c(2,2)) ! tcx: child(4,7)
