...
!*  ===================================================================
!*
!*  TEST CASE NAME             : ffinal024gkl2.f
!*  TEST CASE NAME             : type-bound procedure ffinal024gkl2
!*
!*  PROGRAMMER                 : David Forster (derived from ffinal024g by Catherine Sun)
...
use m1
   type dt (kdt_1,kdt_2,ldt_1) ! kdt_1,kdt_2,ldt_1=4,4,7
      ...
      type(child(kdt_2,ldt_1)), pointer :: dt_c  ! tcx: (kdt_2,ldt_1)
   end type
end module
...
use m2, only : dt
   class(base(4)), intent(inout) :: arg1 ! tcx: (4)
   class(dt(4,4,7)), allocatable  :: t1(:)   ! tcx: (4,4,7)
   allocate(t1(2))
   allocate(t1(1)%dt_b, t1(2)%dt_b)
   allocate(t1(1)%dt_c, t1(2)%dt_c)
