...
!*  ===================================================================
!*
!*  TEST CASE NAME             : ffinal024ckk3.f
!*  TEST CASE NAME             : type-bound procedure ffinal024ckk3
!*
...
module m2
use m1
   type dt (kdt_1,kdt_2,kdt_3) ! kdt_1,kdt_2,kdt_3=8,4,4
      integer, kind :: kdt_1,kdt_2,kdt_3
      type(base(kdt_2)) ::  dt_b  ! tcx: (kdt_2)
      type(child(kdt_2,kdt_3)) :: dt_c  ! tcx: (kdt_2,kdt_3)
   end type
end module
...
use m2, only : dt
   class(base(4)), intent(inout) :: arg1 ! tcx: (4)
   class(dt(4,4,8)), pointer  :: t1(:, :)   ! tcx: (8,4,4)
   allocate(t1(2,2))
   deallocate(t1)
end subroutine 
...
! type: dt - added parameters (kdt_1,kdt_2,kdt_3) to invoke with (8,4,4) / declare with (8,4,4) - 1 changes
