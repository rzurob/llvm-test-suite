...
use m, only: point
    ...
    integer*4 function numOfGoodCompPoint3D (p)
        class (point3D(4)), intent (in) :: p ! tcx: (4)
    ...
    subroutine invalidatePoint3D (p3)
        type (point3D(4)), intent(inout) :: p3 ! tcx: (4)
...
use m1
    ...
    type (point3D(4)), target :: p3d ! tcx: (4)

    class (point(4)), pointer :: p_ptr ! tcx: (4)

    p3d = point3D(4) (1.0, 2.0, 3.0) ! tcx: (4)
...
! type: point3D - added parameters () to invoke with (4) / declare with (4) - 6 changes
