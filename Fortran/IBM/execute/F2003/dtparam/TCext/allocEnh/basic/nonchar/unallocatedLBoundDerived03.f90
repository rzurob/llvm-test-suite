...
! ftcx_dtp -qnol -qreuse=base /tstdev/F2003/allocEnh/basic/nonchar/unallocatedLBoundDerived03.f
...
MODULE mD2
    ...
    TYPE, EXTENDS(tD) :: tD2    ! (4)
        INTEGER(K1), POINTER :: d2IP
    END TYPE tD2
END MODULE mD2
...
PROGRAM unallocatedLBoundDerived03
    ...
    INTERFACE
        SUBROUTINE CheckIt( dA )
            USE mD2
            TYPE(tD2(4)), ALLOCATABLE :: dA( :,:,: )
        END SUBROUTINE CheckIt
    END INTERFACE

    INTEGER, TARGET :: iT( 100 ) = (/ (i, i = 0, 99) /)

    TYPE(tD2(4)), ALLOCATABLE :: d2Alloc( :,:,: )

    TYPE(tD2(4)) :: d2Array( 0:99,0:99,0:99 )
