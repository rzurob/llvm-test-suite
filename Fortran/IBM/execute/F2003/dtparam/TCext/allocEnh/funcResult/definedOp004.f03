...
! ftcx_dtp -qk -qnol /tstdev/F2003/allocEnh/funcResult/definedOp004.f
...
module m
    ...
    function concatStrings (s1, s2)
        type(string(4)), intent(in) :: s1(:), s2(:)

        type(string(4)) concatStrings(max(size(s1), size(s2)))
...
use m
    type(string(4)), allocatable :: s1(:), s2(:), s3(:)

    s1 = (/(string(4)(null()), string(4)(genString(i)), i=1,100)/)

    s2 = (/(string(4)(genString(i*2)), string(4)(null()), string(4)('XYZ'), i=1,20)/)
