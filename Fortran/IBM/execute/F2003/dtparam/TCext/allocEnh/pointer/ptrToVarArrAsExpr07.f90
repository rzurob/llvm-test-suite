...
! ftcx_dtp -ql -qnodefaultpv -qdeferredlp -qreuse=self /tstdev/F2003/allocEnh/pointer/ptrToVarArrAsExpr07.f
...
!*  ===================================================================
!*
!*  TEST CASE TITLE            : ptrToVarArrAsExpr07 - variable is a POINTER
!*
...
MODULE mDerived
    ...
    TYPE :: tDerived(N2,K2)    ! (20,4)
        INTEGER, KIND                 :: K2
        INTEGER, LEN                  :: N2
        INTEGER(K2)                   :: d
        TYPE(tBase(:,K2)),ALLOCATABLE :: tB( : )
    END TYPE tDerived
...
PROGRAM ptrToVarArrAsExpr07
    ...
    TYPE(tDerived(:,4)), POINTER :: dArrPtr( : )
    TYPE(tDerived(:,4)), ALLOCATABLE, TARGET :: dArr( : )


    ALLOCATE(dArr( 10 ), SOURCE=[ (tDerived(20,4)((11 - i),NULL()), i = 1, 10) ])

    DO i = 1, 10
        ALLOCATE(tBase(20,4) :: dArr( i )%tB( dArr( i )%d ) )
