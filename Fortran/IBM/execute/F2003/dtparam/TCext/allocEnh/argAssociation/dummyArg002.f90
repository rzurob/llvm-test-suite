...
! ftcx_dtp -qck -ql -qdeferredlp /tstdev/F2003/allocEnh/argAssociation/dummyArg002.f
...
module m
    type base(n1,k1)    ! (20,4)
        integer, kind        :: k1
        integer, len         :: n1
        integer(k1), pointer :: ids(:) => null()
    end type

    type, extends(base) :: child(k2,n2)    ! (20,4,1,7)
        integer, kind                          :: k2
        integer, len                           :: n2
        character(kind=k2,len=n2), allocatable :: name
    end type
...
use m
    ...
    allocate (b2(10), source=(/(child(20,4,1,7)(null(), 'xlftest'), i=1,10)/))
