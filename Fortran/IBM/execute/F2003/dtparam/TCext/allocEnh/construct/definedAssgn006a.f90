...
! ftcx_dtp -qck -qk -qnol -qnodefaultpv /tstdev/F2003/allocEnh/construct/definedAssgn006a.f
...
module m
    ...
    elemental type(string(4)) function concatString (s, c)
...
module n
use m
    type employee(k3,n1)    ! (4,2)
        integer, kind                 :: k3
        integer, len                  :: n1
        type(number(k3))              :: ID
        type(string(k3))              :: name(n1)
        type(string(k3)), allocatable :: address(:)
    end type
end module

program definedAssgn006a
use n
    type(employee(4,:)), allocatable :: p1(:)
    type(number(4)) num1
    type(string(4)) name(2)
    ...
    p1 = (/(employee(4,2)(number(4)(i), name // achar(65+mod(i,26)), &
        (/string(4)('8200 Warden Ave.'), string(4)('Markham')/)), i=1,100)/)
