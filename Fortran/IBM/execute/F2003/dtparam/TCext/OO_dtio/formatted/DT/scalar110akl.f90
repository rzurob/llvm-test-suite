...
!*  ===================================================================
!*
!*  TEST CASE NAME             : scalar110akl
!*
!*  PROGRAMMER                 : David Forster (derived from scalar110a by Robert Ma)
...
!*                                        derived type variable with polymorphic component (read)
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
!* ===================================================================
!*
...
use m
   ...
   open (1, file = 'scalar110akl.1', form='formatted', access='sequential' )
   ...
   read ( 1, fmt, iostat = stat, iomsg = msg )                 ( b1, b2, i= 4,5 )
   if ( ( stat /= 0 ) .or. ( msg /= 'dtioread' ) ) error stop 1_4

   select type ( g => b1%d )
     type is ( cdata(4,4) )
        if (( g%i /= 101 ) .or. ( g%j /= 102 ) .or. ( b1%k /= 1001 ) ) error stop 2_4
        if (( b2%d%i /= 103 ) .or. ( b2%k /= 1002 ) ) error stop 3_4
     class default
        error stop 4_4
   end select

   read ( 1, "( DT'_b3-1'(5,6,7),:,1X, DT'_b3-2'(6,7,8))", iostat = stat, iomsg = msg )   ( b3, i = 3,5)
