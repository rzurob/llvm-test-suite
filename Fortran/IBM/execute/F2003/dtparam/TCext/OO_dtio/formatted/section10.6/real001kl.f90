...
!*  ===================================================================
!*
!*  TEST CASE NAME             : real001kl
!*
!*  PROGRAMMER                 : David Forster (derived from real001 by Robert Ma)
...
module m1
   type threereals (kr1,kr2,kr3) ! kr1,kr2,kr3=4,8,16
      integer, kind :: kr1,kr2,kr3
      real(kr1)  :: c1 = 0.0
      real(kr2)  :: c2 = 0.0
      real(kr3) :: c3 = 0.0
   end type  
...
program real001kl
   ...
   interface write(formatted)
      subroutine writeformatted (dtv, unit, iotype, v_list, iostat, iomsg)
         import threereals
         class(threereals(4,8,16)), intent(in) :: dtv ! tcx: (4,8,16)
   ...
   interface read(formatted)
      subroutine readformatted (dtv, unit, iotype, v_list, iostat, iomsg)
         import threereals
         class(threereals(4,8,16)), intent(inout) :: dtv ! tcx: (4,8,16)
         integer,  intent(in) :: unit
         character(*), intent(in) :: iotype
         integer, intent(in)     :: v_list(:)         
         integer,  intent(out) :: iostat
   ...
   class(threereals(4,8,16)), allocatable :: f1 ! tcx: (4,8,16)
   class(threereals(4,8,16)), pointer     :: f2 ! tcx: (4,8,16)
   ...
   allocate (f1, source = threereals(4,8,16)(1.0,2.0,3.0))                                          !<- basic test  ! tcx: (4,8,16)
   allocate (f2, source = threereals(4,8,16)(tiny(f2%c1),1.3E1,102.6D-1))                           !<- basic test  ! tcx: (4,8,16)

      
   open (unit = 1, file ='real001kl.1', form='formatted', access='sequential')
   open (unit = 2, file ='real001kl.2', form='formatted', access='stream', status='replace')   
...
! type: threereals - added parameters (kr1,kr2,kr3) to invoke with (4,8,16) / declare with (4,8,16) - 9 changes
