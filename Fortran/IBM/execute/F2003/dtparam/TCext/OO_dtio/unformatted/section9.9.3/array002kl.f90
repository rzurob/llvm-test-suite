...
program array002kl
   ...
   allocate ( b2(2,2), source = reshape ( source=(/(child(4,8,3,2)(x=3,y=4.0,z='ftn', a=5, b=6.0, c=(7.0,8.0)),i=1,4)/), shape= (/2,2/) )) ! tcx: (4,8,3,2)
   allocate ( base(4,8,3):: b3(3) ) ! tcx: base(4,8,3)
   allocate ( child(4,8,3,2) :: b4(2,2) ) ! tcx: (4,8,3,2)

   ! I/O operations

   open ( u1%get(), file = 'array002kl.data', form='unformatted', access='sequential' )   !<- open a external file with unit #2
...
! type: child - added parameters (kchild_1) to invoke with (4,8,3,2) / declare with (4,8,*,2) - 6 changes
