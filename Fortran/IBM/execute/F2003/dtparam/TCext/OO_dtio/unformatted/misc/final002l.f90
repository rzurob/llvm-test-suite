...
!*  ===================================================================
!*
!*  TEST CASE NAME             : final002l
!*
!*  PROGRAMMER                 : David Forster (derived from final002 by Robert Ma)
...
contains
   ...
   subroutine finalchild(dtv)
      ...
      if(dtv%lbase_1 /= 3) then
         print *, 'finalize child: dtv%lbase_1=', dtv%lbase_1, '(error: unexpected length)'
      else
   ...
   subroutine finalchildrank1(dtv)
      ...
      !! Note: This has been patched to avoid potential output problems
      if(dtv%lbase_1/=3) then
         print *, 'finalize child rank=1: dtv%lbase_1=', dtv%lbase_1, size(dtv), '(error: unexpected length)'
      else
...
program final002l
   ...
   open (unit = 3, file ='final002l.3', form='unformatted', access='sequential')

   ! allocation of variables

   allocate (c1, source = child(3)('abc', 'ABC') )           !<- finalized ! tcx: (3)
   allocate (c2, source = child(3)('def', 'DEF') )           !<- finalized ! tcx: (3)
   allocate (c3, source = child(3)('ghi', 'GHI') )           !<- finalized ! tcx: (3)
   allocate (c4(3), source = (/ c1, child(3)('jkl', 'JKL') , c3 /) )   !<- finalize 1 child scalar ! tcx: (3)
   ...
   allocate(child(3):: c1, c2, c3, c4(3) ) ! tcx: child(3)
   rewind 3
...
! type: child - added parameters () to invoke with (3) / declare with (*) - 13 changes
