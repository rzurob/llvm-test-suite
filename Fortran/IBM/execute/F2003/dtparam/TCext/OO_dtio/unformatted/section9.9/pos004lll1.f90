...
module m1
   ...
   type, extends(base) :: child (lchild_1) ! lchild_1=1
      integer, len :: lchild_1
      character(lchild_1) :: cc(lbase_2)
   contains 
...
program pos004lll1
   ...
   allocate ( b1, source = child(1,3,1) ( (/'a','b','c'/), (/'A','B','C'/) ) ) ! tcx: (1,3,1)
   allocate ( b2, source = child(1,3,1) ( (/'d','e','f'/), (/'D','E','F'/) ) ) ! tcx: (1,3,1)
   allocate ( b3, source = child(1,3,1) ( (/'g','h','i'/), (/'G','H','I'/) ) ) ! tcx: (1,3,1)
   allocate ( b4(3), source = (/ b1, b2, b3 /) ) 
   ! I/O operations
   
   open ( 1, file = 'pos004lll1.data', form='unformatted', access='stream' )
...
! type: child - added parameters (lchild_1) to invoke with (1,3,1) / declare with (*,*,*) - 6 changes
