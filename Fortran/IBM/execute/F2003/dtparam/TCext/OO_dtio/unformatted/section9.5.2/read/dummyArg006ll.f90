...
program dummyArg006ll
   ...
   type(child(3,3)) :: c1 = child(3,3)('abc','def') ! tcx: (3,3) ! tcx: (3,3)
   type(child(:,:)), pointer :: c2 ! tcx: (:,:)
   type(child(:,:)), allocatable :: c3 ! tcx: (:,:)

   allocate(base(3):: b2, b3 ) ! tcx: base(3)
   allocate(child(3,3):: c2, c3 ) ! tcx: child(3,3)

   cc1 = 'ABCabcdef'
   cc2 = 'DEFghijkl'
   cc3 = 'GHImnopqr'

   open (unit = 1, file ='dummyArg006ll.data', form='unformatted', access='sequential')
