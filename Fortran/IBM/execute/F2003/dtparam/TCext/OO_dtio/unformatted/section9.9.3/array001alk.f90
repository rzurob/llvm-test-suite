...
program array001alk
   ...
   allocate ( b1(5), source = (/ ( base(0)(), i=1,5) /) ) ! tcx: (0)
   allocate ( b2(2,2), source = reshape ( source = (/ ( base(0)(),i=1,4) /), shape = (/2,2/))) ! tcx: (0)
   b3       = (/ ( base(0)(), i=1,3) /)  ! tcx: (0)

   ! I/O operations

   open ( u1%get(), file = 'array001alk.data', form='unformatted', access='sequential' )   !<- open a external file with unit #2
...
! type: base - added parameters (lbase) to invoke with (0) / declare with (*) - 12 changes
