...
!*  ===================================================================
!*
!*  TEST CASE NAME             : pos003kk
!*
!*  PROGRAMMER                 : David Forster (derived from pos003 by Robert Ma)
...
module m1
   type, abstract :: base (kbase) ! kbase=1
      integer, kind :: kbase
   end type
...
program pos003kk
   ...
   class(base(1)), allocatable :: b1, b2 ! tcx: (1)
   character(200) :: msg1 = ''
   integer :: stat1
   ...
   allocate ( b1, source = child(1,2)() ) ! tcx: (1,2)
   allocate ( b2, source = child(1,2)() ) ! tcx: (1,2)
   
   ! I/O operations
   
   
   open ( 1, file = 'pos003kk.data', form='unformatted', access='stream' )
...
! type: base - added parameters (kbase) to invoke with (1) / declare with (1) - 5 changes
! type: child - added parameters (kchild) to invoke with (1,2) / declare with (1,2) - 4 changes
