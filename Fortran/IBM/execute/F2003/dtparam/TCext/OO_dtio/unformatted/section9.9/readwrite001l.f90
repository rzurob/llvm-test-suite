...
!*  ===================================================================
!*
!*  TEST CASE NAME             : readwrite001l
!*
!*  PROGRAMMER                 : David Forster (derived from readwrite001 by Robert Ma)
...
program readwrite001l
   ...
   if ( readwrite1 /= 'UNKNOWN' )                            error stop 101_4

   inquire ( file='readwrite001l.f', read = readwrite1 )    !<- file not connected to any units
   ...
   open (myunit1, file='readwrite001l.1', form='unformatted', access='sequential'      , action='read', status='old')
   open (myunit2, file='readwrite001l.2', form='unformatted', access='direct', recl=3  , action='write' )
   open (myunit3, file='readwrite001l.3', form='unformatted', access='stream'          , action='readwrite' )
   
   if ( ( .not. isRead(myunit1) ) .or. ( isWrite(myunit1) ) .or. ( isReadWrite(myunit1) ) )                                          error stop 4_4
   if ( ( isRead('readwrite001l.2') ) .or. ( .not. isWrite('readwrite001l.2') ) .or. ( isReadWrite('readwrite001l.2') ) )               error stop 5_4
   if ( ( .not. isRead('readwrite001l.3') ) .or. ( .not. isWrite('readwrite001l.3') ) .or. ( .not. isReadWrite('readwrite001l.3') ) )   error stop 6_4
