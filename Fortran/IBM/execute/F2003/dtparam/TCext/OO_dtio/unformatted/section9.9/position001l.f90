...
program position001l
   ...
   if ( getPositionByUnit(myunit1)         /= 'aUNDEFINED' )   error stop 101_4  !<- disconnected unit
   if ( getPositionByFile('position001l.f') /= 'aUNDEFINED' )   error stop 2_4  !<- disconnected file

   open (myunit1, file='position001l.1', form='unformatted', access='sequential', status='replace' )
   open (myunit2, file='position001l.2', form='unformatted', access='direct', recl=3, status='replace' )
   open (myunit3, file='position001l.3', form='unformatted', access='stream', position='rewind', status='replace' )

   if ( getPositionByUnit(myunit1) /= 'aASIS' )                error stop 3_4   !<- default position is ASIS
   if ( getPositionByFile('position001l.2') /= 'aUNDEFINED' )   error stop 4_4   !<- direct access is always undefined in position
   ...
   if ( getPositionByFile('position001l.3') /= 'aREWIND' )      error stop 14_4
