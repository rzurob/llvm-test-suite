...
module m1
   type, abstract :: base (lbase_1,base_2) ! lbase_1,base_2=1,3
      integer, len :: lbase_1,base_2
      character(lbase_1) :: c(base_2)
   contains
      procedure, pass :: getc
...
program pos004l
   ...
   allocate ( b1, source = child(1,3) ( (/'a','b','c'/), (/'A','B','C'/) ) ) ! tcx: (1,3)
   allocate ( b2, source = child(1,3) ( (/'d','e','f'/), (/'D','E','F'/) ) ) ! tcx: (1,3)
   allocate ( b3, source = child(1,3) ( (/'g','h','i'/), (/'G','H','I'/) ) ) ! tcx: (1,3)
   allocate ( b4(3), source = (/ b1, b2, b3 /) ) 
   ! I/O operations
   
   open ( 1, file = 'pos004l.data', form='unformatted', access='stream' )
...
! type: child - added parameters () to invoke with (1,3) / declare with (*,*) - 6 changes
