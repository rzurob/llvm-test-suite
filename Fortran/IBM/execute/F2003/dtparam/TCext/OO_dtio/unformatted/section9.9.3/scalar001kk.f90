...
module m1
   ...
   interface write(unformatted)
      subroutine writeUnformatted (dtv, unit, iostat, iomsg)
         import base
         class(base(4,4)), intent(in) :: dtv ! tcx: (4,4)
         integer,  intent(in) :: unit
         integer,  intent(out) :: iostat
         character(*),  intent(inout) :: iomsg
      end subroutine
...
contains
   ...
   real(4) function getX(a)
      class(base(4,4)), intent(in) :: a ! tcx: (4,4)
      getX = a%x
   end function
   ...
   subroutine setX(a, i)
      class(base(4,4)), intent(inout) :: a ! tcx: (4,4)
      real(4) :: i
      a%x = i
   end subroutine
...
program scalar001kk
   ...
   type(base(4,4)), allocatable :: b1 ! tcx: (4,4)
   type(base(4,4)), pointer     :: b2 ! tcx: (4,4)
   ...
   allocate ( b1, source = base(4,4)(x=1.0,y=2.0) ) ! tcx: (4,4)
   allocate ( b2, source = base(4,4)(x=3.0,y=4.0) ) ! tcx: (4,4)
   b3       =    base(4,4)(5.0, 6.0) ! tcx: (4,4)

   ! I/O operations

   open ( u1%get(), file = 'scalar001kk.data', form='unformatted', access='sequential' )   !<- open a external file with unit #2

   inquire ( iolength = length1 )  b1           !<- output list is a single scalar item

   if ( length1 /= 8 )             error stop 101_4
...
! type: base - added parameters (kbase_1,kbase_2) to invoke with (4,4) / declare with (4,4) - 16 changes
