...
module m
    ...
    type :: emptybase (leb) ! leb=0
       integer, len :: leb
    end type

end module

program zerosized001ll
use m
   ...
   interface read(unformatted)
      ...
      use m
         class(emptybase(*)), intent(inout) :: dtv ! tcx: (*)
         integer, intent(in) :: unit
         integer, intent(out) :: iostat
         character(*), intent(inout) :: iomsg
      end subroutine
   ...
   interface write(unformatted)
      ...
      use m
         class(emptybase(*)), intent(in) :: dtv ! tcx: (*)
         integer, intent(in) :: unit
         integer, intent(out) :: iostat
         character(*), intent(inout) :: iomsg
      end subroutine
   ...
   class(emptybase(:)), allocatable :: e1 ! tcx: (:)
   class(emptybase(:)), pointer     :: e2 ! tcx: (:)
   type (emptybase(0))              :: e3 ! tcx: (0)

   ! allocation of variables

   allocate (base(0):: b1, b2 ) ! tcx: base(0)
   allocate (emptybase(0):: e1, e2 ) ! tcx: emptybase(0)

   open(1, file='zerosized001ll.data', access='sequential', form='unformatted')
...
! type: emptybase - added parameters (leb) to invoke with (0) / declare with (*) - 9 changes
