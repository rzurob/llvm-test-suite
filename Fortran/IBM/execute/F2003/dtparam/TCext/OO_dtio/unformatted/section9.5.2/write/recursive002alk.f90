...
!*  ===================================================================
!*
!*  TEST CASE NAME             : recursive002alk
!*
...
module m
   ...
   interface write(unformatted)
      subroutine writeunformattedll(dtv, unit, iostat, iomsg )
         import linkedlist
         class(linkedlist(1)), intent(in) :: dtv ! tcx: (1)
...
program recursive002alk
   ...
   character(200) :: msg = ''
   class(linkedlist(1)), pointer :: ll ! tcx: (1)

   allocate ( ll )
   allocate ( ll%head, source = base(3) (c='abc')) ! tcx: (3)
   allocate ( ll%head%next, source = base(3)(c='def') ) ! tcx: (3)
   allocate ( ll%head%next%next, source = base(3) (c='ghi') ) ! tcx: (3)
   allocate ( ll%head%next%next%next, source = base(3) (c='jkl') ) ! tcx: (3)
   allocate ( ll%head%next%next%next%next, source = base(3) (c='mno') ) ! tcx: (3)

   open (1, file = 'recursive002alk.1', form='unformatted', access='stream' )
...
! type: linkedlist - added parameters (kll) to invoke with (1) / declare with (1) - 3 changes
