...
contains
   ...
   function getNewChild(c)
      type(child(3)) :: getNewChild ! tcx: (3)
      character(6) :: c
...
program funcRetrn004l
   ...
   open (unit = 1, file ='funcRetrn004l.data', form='unformatted', access='sequential')
...
function getNewChildAlloc(c)
   ...
   type(child(:)), allocatable :: getNewChildAlloc ! tcx: (:)
   character(6) :: c
   allocate(getNewChildAlloc, source = child(3)(c(1:3),c(4:6)) ) ! tcx: (3)
 end function
