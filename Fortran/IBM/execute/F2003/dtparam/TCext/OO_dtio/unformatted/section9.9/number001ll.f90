...
!*  ===================================================================
!*
!*  TEST CASE NAME             : number001ll
!*
!*  PROGRAMMER                 : David Forster (derived from number001 by Robert Ma)
...
program number001ll
   ...
   if ( getNumberByFile('number001ll.disconnected') /= -1 )      error stop 18_4  !<- inquire by file with a file that is not connected
   ! these two lines are commented out from the resolution of 297250 if ( getNumberByFile('number001ll.stdin')  /= INPUT_UNIT  )   error stop 19_4  !<- inquire by file with a file that is of standard in
   !if ( getNumberByFile('number001ll.stdout') /= OUTPUT_UNIT )   error stop 20_4  !<- inquire by file with a file that is of standard out  
   ...
   inquire ( file = 'number001ll.1', number = number1 )
   if ( number1 /= -1 ) error stop 21_4
   inquire ( myunit2 , number = number1 )
   if ( number1 /= 2  ) error stop 22_4  
   
   inquire ( file = 'number001ll.stdin', number = number1 )
