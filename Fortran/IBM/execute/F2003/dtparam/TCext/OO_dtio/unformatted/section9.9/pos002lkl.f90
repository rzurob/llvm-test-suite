...
module m1
   ...
   type, extends(child) :: gen3 (lgen3) ! lgen3=0
      integer, len :: lgen3
      ! empty
   end type
...
program pos002lkl
   ...
   allocate ( b2, source = gen3(3,4,0)( 'def', 102 ) ) ! tcx: (3,4,0)
   allocate ( b3, source = gen3(3,4,0)( 'ghi', 103 ) ) ! tcx: (3,4,0)
   allocate ( b4(2,2), source = reshape ( source=(/ b2, b3, b2, b3 /), shape=(/2,2/) ) )

   ! I/O operations

   open ( 1, file = 'pos002lkl.data', form='unformatted', access='stream' )
...
! type: gen3 - added parameters (lgen3) to invoke with (3,4,0) / declare with (*,4,*) - 2 changes
