...
!*  ===================================================================
!*
!*  TEST CASE NAME             : name001l
!*
!*  PROGRAMMER                 : David Forster (derived from name001 by Robert Ma)
...
program name001l
   ...
   if ( getNameByUnit(myunit1) /= 'DNE' )     error stop 101_4
   ...
   open (myunit1, file='name001l.1', form='unformatted')
   open (myunit2, access='direct', recl=3)
   open (myunit3, file='name001l.3', form='unformatted', access='stream' )
   
   if ( getNameByUnit(myunit1) /= 'name001l.1' )       error stop 7_4
   if ( getNameByUnit(myunit2) /= 'fort.2' )          error stop 8_4   !<- unit 2 connected to file fort.2 by default
   if ( getNameByUnit(myunit3) /= 'name001l.3' )       error stop 9_4
   
   if ( getNameByFile('name001l.1') /= 'name001l.1' )   error stop 10_4
   if ( getNameByFile('fort.2') /= 'fort.2' )         error stop 11_4
   if ( getNameByFile('name001l.3') /= 'name001l.3' )   error stop 12_4
      
   ! I/O operations
   
   write (myunit1, iostat=stat1, iomsg=msg1 ) b1
   if (( stat1 /= 0 ) .or. ( msg1 /= 'name001l.1' ) ) error stop 13_4
   ...
   if (( stat1 /= 0 ) .or. ( msg1 /= 'name001l.3' ) ) error stop 15_4
   
   rewind myunit1
   
   read  (myunit1, iostat=stat1, iomsg=msg1 ) b2
   if (( stat1 /= 0 ) .or. ( msg1 /= 'name001l.1' ) ) error stop 16_4
   ...
   if (( stat1 /= 0 ) .or. ( msg1 /= 'name001l.3' ) ) error stop 18_4
   ...
   if ( getNameByFile('name001l.disconnected') /= 'name001l.disconnected' ) error stop 22_4  !<- inquire by file with a file that is not connected
   !  these two lines are commented out due to the resolution of defect 297250   if ( getNameByUnit(INPUT_UNIT)  /= 'name001l.stdin' )                   error stop 23_4  !<- inquire by file with a file that is of standard in

   !if ( getNameByUnit(OUTPUT_UNIT) /= 'name001l.stdout' )                  error stop 24_4  !<- inquire by file with a file that is of standard out        
