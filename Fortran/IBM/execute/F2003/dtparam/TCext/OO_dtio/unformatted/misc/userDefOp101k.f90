...
!*  ===================================================================
!*
!*  TEST CASE NAME             : userDefOp101k
!*
!*  PROGRAMMER                 : David Forster (derived from userDefOp101 by Robert Ma)
...
program userDefOp101k
   use m

   class(base(4)), allocatable :: b1, b2, b3, b4 ! tcx: (4)

   allocate ( child(4) :: b1, b2 ) ! tcx: (4)
   allocate ( child(4) :: b3 ) ! tcx: (4)

   open ( 1, file='userDefOp101k.1', form='unformatted', access='sequential' )
   ...
   select type ( a )
      ...
      type is ( child(4) ) ! tcx: (4)
         select type ( b )
            type is ( child(4) ) ! tcx: (4)
               read ( 1, iostat = stat, iomsg = msg ) A, B
               allocate ( myAdd, source = child(4) ( i1= ( a%i1+b%i1 ), i2= ( a%i2+b%i2 ) ) ) ! tcx: (4)
            class default
               error stop 14_4
         end select
   end select
...
! type: child - added parameters () to invoke with (4) / declare with (4) - 11 changes
