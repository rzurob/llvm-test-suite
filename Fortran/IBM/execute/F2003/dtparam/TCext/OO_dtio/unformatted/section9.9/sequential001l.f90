...
!*  ===================================================================
!*
!*  TEST CASE NAME             : sequential001l
!*
!*  PROGRAMMER                 : David Forster (derived from sequential001 by Robert Ma)
...
program sequential001l
   ...
   if ( isSequential(myunit1)  /= 'UNKNOWN' )                  error stop 101_4
   if ( isSequential('fort.2') /= 'UNKNOWN' )                  error stop 2_4
   if ( isSequential('sequential001l.f') /= 'UNKNOWN' )         error stop 3_4   
   
   open (myunit1, file='sequential001l.1', form='unformatted', access='sequential'     )
   open (myunit2, file='sequential001l.2', form='unformatted', access='direct', recl=3 )
   open (myunit3, file='sequential001l.3', form='unformatted', access='stream' )

   if ( isSequential(myunit1)           /= 'YES' )             error stop 4_4
   if ( isSequential('sequential001l.2') /= 'NO' )              error stop 5_4
