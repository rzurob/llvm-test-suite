...
program funcRetrn003ll
   ...
   allocate(b3, source = child(3,3)('jkl','mno')) ! tcx: (3,3)

   open (unit = 1, file ='funcRetrn003ll.data', form='unformatted', access='sequential')
...
contains
   function bar()
      ...
      allocate(bar, source = child(3,3)('pqr','stu') ) ! tcx: (3,3)
   end function
end program
...
use m1
   class(base(:)), allocatable :: foo ! tcx: (:)
   allocate(foo, source =  child(3,3)('vwx','xyz') ) ! tcx: (3,3)
end function
