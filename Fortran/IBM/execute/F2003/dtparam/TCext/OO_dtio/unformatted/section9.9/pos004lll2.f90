...
!*  ===================================================================
!*
!*  TEST CASE NAME             : pos004lll2
!*
!*  PROGRAMMER                 : David Forster (derived from pos004 by Robert Ma)
...
program pos004lll2
   ...
   allocate ( b1, source = child(1,3,3) ( (/'a','b','c'/), (/'A','B','C'/) ) ) ! tcx: (1,3,3)
   allocate ( b2, source = child(1,3,3) ( (/'d','e','f'/), (/'D','E','F'/) ) ) ! tcx: (1,3,3)
   allocate ( b3, source = child(1,3,3) ( (/'g','h','i'/), (/'G','H','I'/) ) ) ! tcx: (1,3,3)
   allocate ( b4(3), source = (/ b1, b2, b3 /) ) 
   ! I/O operations
   
   open ( 1, file = 'pos004lll2.data', form='unformatted', access='stream' )
...
! type: base - added parameters (lbase_1,base_2) to invoke with (1,3) / declare with (*,*) - 8 changes
! type: base - added parameters (lchild_1) to invoke with (1,3,3) / declare with (*,*,*) - 8 changes
! type: base - added parameters (lbase_1,lbase_2) to invoke with (1,3) / declare with (*,*) - 8 changes
! type: child - added parameters (lchild_1) to invoke with (1,3,3) / declare with (*,*,*) - 6 changes
