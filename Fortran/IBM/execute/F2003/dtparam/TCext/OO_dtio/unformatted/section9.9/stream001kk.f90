...
!*  ===================================================================
!*
!*  TEST CASE NAME             : stream001kk
!*
!*  PROGRAMMER                 : David Forster (derived from stream001 by Robert Ma)
...
program stream001kk
   ...
   allocate ( b1, source = child(4,4)(x=1.0,y=2.0) ) ! tcx: (4,4)
   allocate ( b2, source = child(4,4)(x=3.0,y=4.0) ) ! tcx: (4,4)
   ...
   open ( u1%get(), file = 'stream001kk.data', form='unformatted', access='stream' )   !<- open a external file with unit #2
   
   if ( .not. u1%isStream() )      error stop 2_4
   ...
   if ( (.not. precision_r4(b1%getx(),3.0) ) .or. (.not. precision_r4(b1%gety(),4.0) ) ) error stop 7_4
   if ( (.not. precision_r4(b2%getx(),1.0) ) .or. (.not. precision_r4(b2%gety(),2.0) ) ) error stop 8_4
...
use m1, only: base, child, unit, u1
   ...
   if ( .not. u1%isStream() )   error stop 9_4
...
use m1, only: base, child, unit, u1
   ...
   if ( .not. u1%isStream() )   error stop 10_4
