...
!*  =================================================================== 
!*
!*  TEST CASE NAME             : fdtio507_1kl
!*
!*  PROGRAMMER                 : David Forster (derived from fdtio507_1 by Jim Xia)
!*  DATE                       : 2007-08-14 (original: 11/19/2004)
...
!*
!*  DESCRIPTION                : DTIO on generics (direct access for unformatted
!                               DTIO; list items are scalars)
...
use m
    ...
    read (1, rec=2, iostat=stat, iomsg=err) b1(1), b1(2), b1(3)
    ...
    if (allocated (b1(1)%d1) .or. (.not. allocated (b1(2)%d1)) .or. &
        (.not. allocated (b1(3)%d1)))  error stop 3_4

    if ((lbound(b1(2)%d1,1) /= 1) .or. (ubound(b1(2)%d1,1) /= 2)) error stop 4_4

    if ((lbound(b1(3)%d1,1) /= 0) .or. (ubound(b1(3)%d1,1) /= 0)) error stop 5_4

    if (.not. precision_r4 (b1(2)%d1(1), 1.0)) error stop 6_4

    if (.not. precision_r4 (b1(2)%d1(2), 2.0)) error stop 7_4

    if (.not. precision_r4 (b1(3)%d1(0), 0.0)) error stop 8_4
...
use m
    ...
    errmsg = ''
    allocate (r1(0:0), source=(/0.0e0_4/))
    ...
    write (1, iostat=stat, iomsg=errmsg, rec=2) b1(1), b1(2), b1(3)
