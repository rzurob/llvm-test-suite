!*  ===================================================================
!*
!*  TEST CASE NAME             : structConstr003al
!*
...
module m
   ...
   type, extends(base) :: child
      character(lbase1), pointer :: i => null()
      contains
...
program structConstr003al
   ...
   open ( 1, file = 'structConstr003al.1', form='unformatted', access='sequential' )

   write ( 1, iostat = stat, iomsg = msg )                   base(3)('abc') ! tcx: (3)
   if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteb' ) )         error stop 1_4

   write ( 1, iostat = stat, iomsg = msg )      child(3)('def') ! tcx: (3)
   if ( ( stat /= 0 ) .or. ( msg /= 'dtiowritec' ) )         error stop 2_4

   write ( 1, iostat = stat, iomsg = msg )    gen3(3)('ghi') ! tcx: (3)
   if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteg' ) )         error stop 3_4

   write ( 1, iostat = stat, iomsg = msg )      child(3)('jkl', aaa ) ! tcx: (3)
   if ( ( stat /= 0 ) .or. ( msg /= 'dtiowritec' ) )         error stop 4_4

   write ( 1, iostat = stat, iomsg = msg )      gen3(3)('mno', bbb, zoo ) ! tcx: (3)
   if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteg' ) )         error stop 5_4

   write ( 1, iostat = stat, iomsg = msg )    ( child(3) (zoo,aaa), gen3(3)('cat',bbb, zoo),i=-1,1 ) ! tcx: (3) ! tcx: (3)
...
! type: gen3 - added parameters () to invoke with (3) / declare with (*) - 4 changes
