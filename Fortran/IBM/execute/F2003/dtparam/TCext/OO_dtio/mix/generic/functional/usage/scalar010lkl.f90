...
program scalar010lkl
   ...
   open ( 1, file = 'scalar010lkl.1', form='formatted', access='sequential' )

   allocate ( b1, source = child(3,4) ( 'abc', 101 ) ) ! tcx: (3,4)
   allocate ( c1, source = gen3(3,4,3)  ( 'def', 102, 'DEF' ) ) ! tcx: (3,4,3)

   write ( 1, *, iostat = stat, iomsg = msg ) b1, c1

   deallocate ( b1 )
   allocate ( b1, source = gen3(3,4,3) ( 'ghi', 103, 'GHI' ) ) ! tcx: (3,4,3)
   ...
   deallocate ( b1 )
   allocate ( b1, source = gen3(3,4,3) ( ) ) ! tcx: (3,4,3)
...
! type: gen3 - added parameters (lgen3_1) to invoke with (3,4,3) / declare with (*,4,*) - 8 changes
