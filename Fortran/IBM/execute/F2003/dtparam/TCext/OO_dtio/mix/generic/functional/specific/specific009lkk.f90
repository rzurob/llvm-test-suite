...
module m
   ...
   contains
      ...
      subroutine writeg (dtv, unit, iotype, v_list, iostat, iomsg)
         class(gen3(*,4,4)), intent(in) :: dtv ! tcx: (*,4,4)
         integer, intent(in) :: unit
         character(*), intent(in) :: iotype
         integer, intent(in)  :: v_list(:)
         integer, intent(out) :: iostat
      ...
      subroutine readg (dtv, unit, iotype, v_list, iostat, iomsg)
         class(gen3(*,4,4)), intent(inout) :: dtv ! tcx: (*,4,4)
         integer, intent(in) :: unit
         character(*), intent(in) :: iotype
         integer, intent(in)  :: v_list(:)
         integer, intent(out) :: iostat
...
program specific009lkk
   ...
   type(gen3(3,4,4))                :: g2 ! tcx: (3,4,4)
   type(gen3(3,4,4)), parameter     :: g3 = gen3(3,4,4)('mno',501,502) ! tcx: (3,4,4) ! tcx: (3,4,4)

   open ( 1, file = 'specific009lkk.1', form='formatted', access='sequential' )

   allocate ( b1, source = gen3(3,4,4)('abc',101, 102)) ! tcx: (3,4,4)
   allocate ( c1, source = gen3(3,4,4)('def',201, 202)) ! tcx: (3,4,4)
  allocate ( g1, source = gen3(3,4,4)('ghi',301, 302)) ! tcx: (3,4,4)

   g2 = gen3(3,4,4)('jkl',401,402) ! tcx: (3,4,4)
   ...
   deallocate ( b1, c1, g1 )
   allocate ( gen3(3,4,4) :: b1, c1, g1 ) ! tcx: (3,4,4)
