...
!*                                    -  selector is a array entity with unformatted i/o
...
program associate002akl
   ...
   open ( 1, file = 'associate002akl.1', form='unformatted', access='sequential' )

   associate ( a => b1((/1,2,3/)), b => b2, c => c1, d => c2(3:1:-1) )

      write ( 1, iostat = stat, iomsg = msg )      a, b  !<- reversion
      if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteb' ) )       error stop 1_4

      associate ( e => c, f => d(3:1:-1) )
         write ( 1, iostat = stat, iomsg = msg )   e, f
   ...
   call associate2 ( b1, (/ c1(1:2,1), c2(1) /) )
   ...
   associate ( aa => b1, bb => b2, cc => c1, dd => c2 )
      ...
      read ( 1, iostat = stat, iomsg = msg )      aa, bb
      if ( ( stat /= 0 ) .or. ( msg /= 'dtioreadb' ) )       error stop 4_4

      associate ( ee => cc, ff => dd )
         read ( 1, iostat = stat, iomsg = msg )   ee, ff
...
!   associate ( g => b1, h => (/ c1(1:2,1), c2(1) /) )
   subroutine associate2 ( g, h)
    ...
    type(child(*,4)), intent(in) :: h(:)
      write ( 1,  iostat = stat, iomsg = msg )      g, h
...
!   associate ( g => b1, h => c2 )
   subroutine associate4 ( g, h )
    ...
    type(child(*,4)), intent(inout) :: h(:)
      read ( 1, iostat = stat, iomsg = msg )      g,h
