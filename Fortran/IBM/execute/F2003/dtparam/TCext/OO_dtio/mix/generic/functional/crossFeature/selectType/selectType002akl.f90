...
!*  ===================================================================
!*
!*  TEST CASE NAME             : selectType002akl
!*
...
!*                                    -  selector is a polymorphic array entity with unformatted i/o
...
program selectType002akl
   ...
   open ( 1, file = 'selectType002akl.1', form='unformatted', access='direct', recl = 30 )

   select type ( g => b1 )
      class is ( base(*)) ! tcx: (*)
         write ( 1, iostat = stat, iomsg = msg , rec = 5 )    g
         if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteb' ) ) error stop 1_4
   end select

   select type ( h => b2 )
      type is ( base(*) ) ! tcx: (*)
         write ( 1, iostat = stat, iomsg = msg , rec = 4 ) h
         if ( ( stat /= 0 ) .or. ( msg /= 'dtiowriteb' ) ) error stop 2_4
   end select

   select type ( i => c1 )
      class is ( child(*,4) ) ! tcx: (*,4)
         write ( 1, iostat = stat, iomsg = msg , rec = 3 )  i
   ...
   b1 => c1
   deallocate ( b2 )
   allocate ( b2(3), source = (/ child(3,4) ( 'jkl', 1002 ), child(3,4) ( 'mno', 101 ), child(3,4) ( 'pqr', 10 ) /) ) ! tcx: (3,4) ! tcx: (3,4) ! tcx: (3,4)

   select type ( h => b1 )
      class default
         write ( 1, iostat = stat, iomsg = msg, rec = 2 ) h
         if ( ( stat /= 0 ) .or. ( msg /= 'dtiowritec' ) ) error stop 4_4
   end select

   select type ( i => b2 )
      class default
         write ( 1, iostat = stat, iomsg = msg, rec = 1 ) i
         if ( ( stat /= 0 ) .or. ( msg /= 'dtiowritec' ) ) error stop 5_4
   end select

   deallocate ( b2, c1 )
   allocate (base(3):: b1(3), b2(3) ) ! tcx: base(3)
   allocate (child(3,4):: c1(2)) ! tcx: child(3,4) ! tcx: (3,4)
   select type ( g => b1 )
      class is ( base(*) ) ! tcx: (*)
         read ( 1, iostat = stat, iomsg = msg , rec = 5  ) g
   ...
   select type ( h => b2 )
      type is ( base(*) ) ! tcx: (*)
         read ( 1, iostat = stat, iomsg = msg , rec = 4 ) h
   ...
   select type ( i => c1 )
      class is ( child(*,4) ) ! tcx: (*,4)
         read ( 1, iostat = stat, iomsg = msg , rec = 3 ) i
   ...
   select type ( h => b1 )
      class default
         read ( 1, iostat = stat, iomsg = msg , rec = 2  ) h
   ...
   select type ( i => b2 )
      class default
         read ( 1, iostat = stat, iomsg = msg , rec = 1 ) i
...
! type: base - added parameters (lbase1) to invoke with (3) / declare with (*) - 14 changes
