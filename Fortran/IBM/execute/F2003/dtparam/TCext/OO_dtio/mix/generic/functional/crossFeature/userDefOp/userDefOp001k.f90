...
program userDefOp001k
   use m

   open ( 1, file='userDefOp001k.1', form='formatted', access='sequential' )

   allocate ( b1, source = child(4) ( 1000, 1001 ) ) ! tcx: (4)
   allocate ( b2, source = child(4) ( 2000, 2002 ) ) ! tcx: (4)
   allocate ( child(4) :: b3 ) ! tcx: (4)
...
class(base(4)) function myAdd(a,b)
   ...
   select type ( a )
      ...
      type is ( child(4) ) ! tcx: (4)
         select type ( b )
            type is ( child(4) ) ! tcx: (4)
               allocate ( myAdd, source = child(4) ( i=(a%i+b%i), j=(a%j+b%j) ) ) ! tcx: (4)
            class default
               error stop 2_4
         end select
   end select
...
! type: base - added parameters (kbase1) to invoke with (4) / declare with (4) - 15 changes
! type: child - added parameters () to invoke with (4) / declare with (4) - 9 changes
