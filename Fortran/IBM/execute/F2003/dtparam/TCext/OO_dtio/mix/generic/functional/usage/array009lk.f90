...
program array009lk
   ...
   open ( 1, file = 'array009lk.1', form='formatted', access='sequential' )

   allocate ( b1(3), source = (/ child(3,4) ( 'abc', 101 ), child(3,4) ( 'def', 102 ), child(3,4) ( 'ghi', 103 ) /) ) ! tcx: (3,4) ! tcx: (3,4) ! tcx: (3,4)
   allocate ( c1(2,2), source = reshape ( source = (/ gen3(3,4)( 'abc', 201, 'ABC' ), gen3(3,4)( 'def', 202, 'DEF' ), & ! tcx: (3,4) ! tcx: (3,4)
                                                      gen3(3,4)( 'ghi', 203, 'GHI' ), gen3(3,4)( 'jkl', 204, 'JKL' ) /), shape = (/2,2/))) ! tcx: (3,4) ! tcx: (3,4)

   write ( 1, *, iostat = stat, iomsg = msg ) b1, c1

   deallocate ( b1 )
   allocate ( b1(2), source = (/ gen3(3,4)( 'abc', 301, 'ABC' ), gen3(3,4)( 'def', 302, 'DEF' ) /) ) ! tcx: (3,4) ! tcx: (3,4)

   write ( 1, "(2(DT))", iostat = stat, iomsg = msg ) b1

   rewind 1

   deallocate ( b1, c1 )
   allocate ( child(3,4) :: b1(3) ) ! tcx: (3,4)
   allocate ( gen3(3,4) :: c1(2,2) ) ! tcx: (3,4)

   read ( 1, "(7(1X,DT))", iostat = stat, iomsg = msg )    b1, c1
   ...
   deallocate ( b1 )
   allocate ( gen3(3,4) :: b1(2) ) ! tcx: (3,4)

   read ( 1, "(2DT)", iostat = stat, iomsg = msg ) b1
...
! type: gen3 - added parameters () to invoke with (3,4) / declare with (*,4) - 12 changes
