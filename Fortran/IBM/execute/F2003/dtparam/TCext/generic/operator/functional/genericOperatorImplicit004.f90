...
! ftcx_dtp -qnol /tstdev/F2003/generic/operator/functional/genericOperatorImplicit004.f
! opt variations: -ql
...
!*  DRIVER STANZA              : xlf95
!*
!*  DESCRIPTION                : Binary Operator: implicit statement with poly array
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
!* ===================================================================
!*
...
program genericOperatorImplicit004
   use m

   implicit class(base(4)) (b)
   implicit class(child(4)) (c)

   allocatable :: b1, b2(:), b3(:), c1(:)

   allocate ( b1, source = base(4)(2) )
   allocate ( c1(3), source = (/ child(4)(1), child(4)(2), child(4)(3) /) )

   allocate ( b2(3), source = b1 * b1 )
   if ( ( b2(1)%x /= 4 ) .or. ( b2(2)%x /= 4 ) .or. ( b2(3)%x /= 4 ) )  error stop 1_4

   deallocate ( b2 )
   allocate (b2(3), source = (/ base(4)(1), base(4)(2), base(4)(3) /) )

   allocate ( b3(3), source = b1 * c1 )
   ...
   allocate ( b3(3) , source = b1 * b2((/ 1,3,2 /)) )
