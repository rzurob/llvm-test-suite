...
! ftcx_dtp -qck -qk -ql -qreuse=all -qdeferredlp /tstdev/OO_poly/func_return/ffuncRet005a1_1.f
! SCCS ID Information
! *********************************************************************
! %START
! %MAIN: YES
! %PRECMD: rm -f *.mod
! %COMPOPTS: -qfree=f90
! %GROUP: ffuncRet005a1_1.f
! %VERIFY:
! %STDIN:
! %STDOUT: ffuncRet005a1_1.out
! %EXECARGS:
! %POSTCMD: spiff -r1e-6 ffuncRet005a1_1.out $TR_SRC/ffuncRet005a1_1.vf && rm -f ffuncRet005a1_1.out
! %END
! *********************************************************************
!*  ===================================================================
!*
...
program ffuncRet005a1_1
use m1
    type (triangle(4,20)) :: t1
    class (shape(4,:)), allocatable :: s1

    t1%vertices = ((/point(4)(1,1),point(4)(0,2), point(4)(3,1)/))

    call printArea(t1%genShape())

    !! makes a little change on t1's value
    t1%vertices(2) = point(4) (10,2)
