...
! ftcx_dtp -qck -qk -ql /tstdev/OO_poly/allocate/falloc018.f
! SCCS ID Information
! *********************************************************************
! %START
! %MAIN: YES
...
use m1
    ...
    allocate (b1, source = child(4,1,15) (1, 'b1_m'))

    allocate (b2(-1:1), source = (/child(4,1,15)(-1,'b1_m_1'), child(4,1,15)(0,'b1_m_2'), &
                                   child(4,1,15)(1,'b1_m_3')/))
...
use m1
    if (.not. allocated (b1)) allocate (b1, source = child(4,1,15)(1, 'b1'))

    if (.not. allocated (b2)) allocate (b2(0:1), source=(/child(4,1,15)(0, 'b2_1'), &
                                        child(4,1,15)(1, 'b2_2')/))
end subroutine
