...
! ftcx_dtp -qck -qk -ql -qreuse=all -qdeferredlp /tstdev/OO_poly/associate/StmtByStmt/Exec/AttrTargetArrPtrSec.f
! *********************************************************************
! %START
! %MAIN: YES
! %PRECMD: 
! %COMPOPTS: -qfree=f90 
! %GROUP:  AttrTargetArrPtrSec.f  
! %VERIFY:  
! %STDIN:
! %STDOUT: 
! %EXECARGS:
...
!*  ===================================================================
!*
!*  TEST CASE NAME             : AttrTargetArrPtrSec 
!*  TEST CASE TITLE            : 
!*
!*  PROGRAMMER                 : Feng Ye
!*  DATE                       : Feb 22, 2005
...
!*  DESCRIPTION
!*    
!*   The selector is a section of an array pointer 
!*   
!*    () 
!*
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  PROGRAM AttrTargetArrAlloc 
  USE M
  CLASS (Child(4,:)),  POINTER :: W(:)
  CLASS (Zero(4,:)),  POINTER :: PtrZ(:)
  CLASS (Base(4,:)),  POINTER :: PtrB(:)
  CLASS (Child(4,:)), POINTER :: PtrC(:)

  ALLOCATE(W(3:5), SOURCE=Child(4,20)(BaseID=-1, ChildID=-2) )
  ...
  CLASS IS (Child(4,*))
    ...
    IF( ANY(LBOUND(PtrC) .NE. (/1/) ) ) STOP 32

    IF( ANY(UBOUND(PtrZ) .NE. (/3/) ) ) STOP 40
    IF( ANY(UBOUND(PtrB) .NE. (/3/) ) ) STOP 41
    IF( ANY(UBOUND(PtrC) .NE. (/3/) ) ) STOP 42
