...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  ASSOCIATE ( As => RESHAPE((/(Child(4)(BaseId=i, ChildId=-i), i=1,4 )/), (/2,2/) ))
    ...
    IF ( ANY (As%Base%GetID()      .NE. RESHAPE((/ 1, 2, 3, 4/), (/2,2/)) ) ) ERROR STOP 33
    IF ( ANY (As%GetID() .NE. RESHAPE((/-1,-2,-3,-4/), (/2,2/)) ) ) ERROR STOP 34
    ...
    ASSOCIATE (As =>  As%Base%GetID())
      IF ( ANY(As .NE. RESHAPE((/ 1, 2, 3, 4/), (/2,2/)) )) ERROR STOP 70
    END ASSOCIATE

    ASSOCIATE ( As => MERGE(1, 2, 1>0))
      IF ( As .NE. 1 ) ERROR STOP 55
    END ASSOCIATE

    ASSOCIATE ( As => (MERGE(As, As, Mask )) )
      IF ( ANY (LBOUND(As) .NE. (/1,1/) ) )    ERROR STOP 50
      IF ( ANY (SHAPE(As)  .NE. (/2,2/) ) )    ERROR STOP 52
      IF ( ANY (As%BaseID  .NE. RESHAPE((/ 1, 2, 3, 4/), (/2,2/)) ) ) ERROR STOP 53
      IF ( ANY (As%ChildID .NE. RESHAPE((/-1,-2,-3,-4/), (/2,2/)) ) ) ERROR STOP 54
    END ASSOCIATE
