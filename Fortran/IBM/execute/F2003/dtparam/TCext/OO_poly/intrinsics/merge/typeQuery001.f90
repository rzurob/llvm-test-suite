...
! ftcx_dtp -qck -qk -ql -qreuse=all -qdeferredlp /tstdev/OO_poly/intrinsics/merge/typeQuery001.f
! *********************************************************************
! %START
! %MAIN: YES
! %PRECMD: rm -f *.mod
...
use m
    ...
    allocate(b3(2,3), SOURCE=reshape((/(Child(4)(-i,-i),i=1,6)/),(/2,3/)))
    m1 = reshape((/.FALSE.,.TRUE.,.TRUE.,.FALSE.,.TRUE.,.FALSE./),(/2,3/))

    if(.NOT. same_type_as(merge(b1,b2,.FALSE.), Child(4)(1,1))) error stop 1_4
    if(.NOT. extends_type_of(merge(b2,b1,.FALSE.), Base(4)(1))) error stop 2_4

    if(.NOT. extends_type_of(merge(b3,b2,m1), Base(4)(1))) error stop 3_4
