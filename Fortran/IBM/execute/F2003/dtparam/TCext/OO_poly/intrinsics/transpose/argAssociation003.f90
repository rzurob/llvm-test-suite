...
! ftcx_dtp -ql -qdeferredlp -qreuse=base /tstdev/OO_poly/intrinsics/transpose/argAssociation003.f
...
use m
    type(Base(20,4)) :: b1(3,4)
    type(Child(20,4)) :: c1(2,3)
    class(Base(:,4)), pointer :: b2(:,:)
    class(Child(:,4)), allocatable :: c2(:,:)
    class(*), allocatable :: u1(:,:)

    b1 = reshape((/(Base(20,4)(i),i=1,12)/), (/3,4/))
    c1 = reshape((/(Child(20,4)(i,i-1),i=2,7)/), (/2,3/))

    allocate(b2(4,2), SOURCE=reshape((/(Child(20,4)(i,i+1),i=2,9)/), (/4,2/)))
    allocate(c2(3,3), SOURCE=reshape((/(Child(20,4)(i,i),i=-9,-1)/), (/3,3/)))
    allocate(u1(2,4), SOURCE=reshape((/(Base(20,4)(i),i=3,10)/), (/2,4/)))
