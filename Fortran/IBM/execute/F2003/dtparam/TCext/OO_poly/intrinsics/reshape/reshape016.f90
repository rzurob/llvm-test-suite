...
! ftcx_dtp -qk -ql -qdeferredlp -qreuse=base /tstdev/OO_poly/intrinsics/reshape/reshape016.f
...
! %COMPOPTS: -qfree=f90
! %GROUP: reshape016.f
! %VERIFY: reshape016.out:reshape016.vf
! %STDIN:
! %STDOUT: reshape016.out
! %EXECARGS:
! %POSTCMD:
! %END
! *********************************************************************
...
use m
    ...
    allocate(b1(20), SOURCE = (/ (Base(4,20)(i), i=1,20) /))
