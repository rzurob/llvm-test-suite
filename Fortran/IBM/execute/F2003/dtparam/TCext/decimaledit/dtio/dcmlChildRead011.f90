...
! ftcx_dtp -ql -qdeferredlp -qreuse=none /tstdev/F2003/decimaledit/dtio/dcmlChildRead011.f
...
module m
    ...
    use iso_fortran_env
        class(base(*,8,8)), intent(inout) :: dtv
...
use m
    class(base(:,8,8)), pointer :: b1(:)

    logical(4), external :: precision_r8, precision_x6

    allocate (base(20,8,8) :: b1(3))
