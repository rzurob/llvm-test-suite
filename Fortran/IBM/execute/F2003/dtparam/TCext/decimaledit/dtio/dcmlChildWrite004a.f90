...
! ftcx_dtp -ql -qdeferredlp /tstdev/F2003/decimaledit/dtio/dcmlChildWrite004a.f
...
module m
    type base(n1,k1,k2,k3)    ! (20,4,8,4)
    ...
    type, extends(base) :: child    ! (20,4,8,4)
        class(*), allocatable :: x
...
use m
    ...
    allocate (b1, source=base(20,4,8,4)(1, null(), .true.))

    allocate (b1%data, source= -1.2d0)

    allocate (b2(size(d1)), source=(/(child(20,4,8,4)(i*10, d1(i), mod(i,3) == 0, i*1.0d1),&
            i = 1, 10)/))
