...
! ftcx_dtp -ql -qnodefaultpv -qdeferredlp -qreuse=none /tstdev/F2003/decimaledit/dtio/modeInquire002.f
! opt variations: -qck -qnol -qdefaultpv -qnodeferredlp -qreuse=self
...
module m
    ...
    type base(n2,k2)    ! (20,4)
        integer, kind                      :: k2
        integer, len                       :: n2
        character(n2), pointer             :: name => null()
        real(k2), allocatable              :: data
        class(dataType(:,k2)), allocatable :: d2
    end type
    ...
    subroutine readBaseFmtd (dtv, unit, iotype, v_list, iostat, iomsg)
        ...
        if (.not. allocated(dtv%d2)) allocate(dataType(dtv%n2,4) :: dtv%d2)
...
use m
    ...
    allocate (base(20,4) :: b1(10), b2(10))
