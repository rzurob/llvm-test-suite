...
!*                               introduced via use only statement and renamed;
!*                               extended type's parent and its parent in procedure
!*                               calls)
!*
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
...
module m
    ...
    subroutine initializeChild (c, intVal, realVal, charVal)
        type(child(4,4,20)), intent(inout) :: c
...
module m1
use m, only: newBase => child, getValue, setValue, initializeChild
    ...
    type(newBase(4,4,20)) :: c1_m
    type(newChild(4,4,20,2)) :: n1_m
...
program fext022
    ...
    type (newBase(4,4,20)) :: c1
    type (newChild(4,4,20,2)) :: n1
