...
! ftcx_dtp -ql /tstdev/F2003/asynchIO/attribute/baseObj/baseObjSpecExpr02.f
! opt variations: -qnol
...
MODULE mLength
    ...
    TYPE tLength(N1,K1)    ! (20,4)
        INTEGER, KIND :: K1
        INTEGER, LEN  :: N1
        INTEGER(K1)   :: arrayLength
    END TYPE tLength
END MODULE mLength
...
PROGRAM baseObjSpecExpr02
    ...
    TYPE(tLength(20,4)), PARAMETER :: lengths = tLength(20,4)( 10 )
...
INTEGER FUNCTION WriteList(lgth, state)
    ...
    INTERFACE
        INTEGER FUNCTION AsynchronousWrite(n, list, stmt)
            ...
            TYPE(tLength(*,4)), ASYNCHRONOUS, INTENT(IN) :: n
