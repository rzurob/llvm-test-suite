...
! ftcx_dtp -qck -qdeferredlp -qreuse=base /tstdev/F2003/asynchIO/attribute/misc/asynchAttrSelectType02.f
...
MODULE mBase
    TYPE tBase(K1,N1,K2)    ! (1,8,4)
        INTEGER, KIND             :: K1,K2
        INTEGER, LEN              :: N1
        CHARACTER(kind=K1,len=N1) :: base
        LOGICAL(K2)               :: saved
    END TYPE tBase
END MODULE mBase
...
MODULE mDerived1
    ...
    TYPE, EXTENDS(tBase) :: tDerived1    ! (1,8,4)
        INTEGER(K2) :: derived
    END TYPE tDerived1
...
MODULE mDerived2
    ...
    TYPE, EXTENDS(tBase) :: tDerived2    ! (1,8,4)
        REAL(K2) :: derived
    END TYPE tDerived2
...
PROGRAM asynchAttrSelectType02
    ...
    TYPE(tBase(1,8,4)), TARGET :: base = tBase(1,8,4)('Base',.FALSE.)
    TYPE(tDerived1(1,8,4)), TARGET :: derived1 = tDerived1(1,8,4)('Derived1',.FALSE.,8765)
    TYPE(tDerived2(1,8,4)), TARGET :: derived2 = tDerived2(1,8,4)('Derived2',.FALSE.,5.678)

    CLASS(tBase(1,:,4)), POINTER :: basePtr
...
!            USE mDerived2
            ...
            CLASS(tBase(1,:,4)), POINTER, INTENT(INOUT) :: theBase
