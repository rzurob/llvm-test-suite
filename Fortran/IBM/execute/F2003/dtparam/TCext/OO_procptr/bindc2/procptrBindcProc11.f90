...
! ftcx_dtp -qk -qnol /tstdev/OO_procptr/bindc2/procptrBindcProc11.f
...
program procptrBindcProc11

   use ISO_C_BINDING,ONLY : C_F_PROCPOINTER, C_FUNPTR, C_INT_LEAST16_T, C_FUNLOC, C_ASSOCIATED, C_LOC, C_PTR
   ...
   type(dt(4)) :: dtype
   integer(C_INT_LEAST16_T), target :: i
   type(C_PTR) :: j , res
   integer(C_INT_LEAST16_T), pointer :: p, pp

   procedure(cfunc), pointer :: funptr=>null()

   i = 34_C_INT_LEAST16_T
   j = C_LOC(i)
...
end program procptrBindcProc11
