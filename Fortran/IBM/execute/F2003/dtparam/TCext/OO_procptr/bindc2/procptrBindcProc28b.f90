...
! ftcx_dtp -qk -ql /tstdev/OO_procptr/bindc2/procptrBindcProc28b.f
...
!*                                c signed char. Also associate procedure 
...
program procptrBindcProc28b
   ...
   if (p /= ichar('A') ) error stop 17_4 
   call C_F_POINTER(j,p)
   if ( ASSOCIATED(p,i) ) error stop 18_4
   if ( p /= ichar('C') ) error stop 19_4
   ...
   if (p /= ichar('A') ) error stop 27_4
   call C_F_POINTER(j,p)
   if ( ASSOCIATED(p,i) ) error stop 28_4
   if ( p /= ichar('C') ) error stop 29_4

   if (pp /= ichar('A') ) error stop 30_4
   call C_F_POINTER(res,pp)
   if ( ASSOCIATED(pp,i) ) error stop 31_4
   if ( pp /= ichar('C') ) error stop 32_4

end program procptrBindcProc28b
