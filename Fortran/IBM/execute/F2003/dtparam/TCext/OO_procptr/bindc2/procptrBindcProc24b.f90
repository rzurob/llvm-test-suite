...
program procptrBindcProc24b
   ...
   if (p /= cmplx(5.0d0,5.0d0) ) error stop 27_4
   call C_F_POINTER(j,p)
   if ( ASSOCIATED(p,i) ) error stop 28_4
   if ( p /= cmplx(10.0d0,10.0d0) ) error stop 29_4

   if (pp /= cmplx(5.0d0,5.0d0) ) error stop 30_4
   call C_F_POINTER(res,pp)
   if ( ASSOCIATED(pp,i) ) error stop 31_4
   if ( pp /= cmplx(10.0d0,10.0d0) ) error stop 32_4

end program procptrBindcProc24b
