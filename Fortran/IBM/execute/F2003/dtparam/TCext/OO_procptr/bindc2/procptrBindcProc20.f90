...
! ftcx_dtp -qk -ql /tstdev/OO_procptr/bindc2/procptrBindcProc20.f
...
program procptrBindcProc20

   use ISO_C_BINDING,ONLY : C_F_PROCPOINTER, C_FUNPTR, C_FLOAT, C_FUNLOC, C_ASSOCIATED, C_LOC, C_PTR
   ...
   if (p /= 5.0e0 ) error stop 26_4
   call C_F_POINTER(j,p)
   if ( p /= 5.0e0 ) error stop 27_4

   if (pp /= 5.0e0 ) error stop 28_4
   call C_F_POINTER(res,pp)
   if ( ASSOCIATED(pp,i) ) error stop 29_4
   if ( pp /= 10.0e0 ) error stop 30_4

end program procptrBindcProc20
