...
! ftcx_dtp -qk -qnol /tstdev/OO_procptr/bindc2/procptrBindcProc23b.f
...
!*                                c float _Complex. Also associate procedure
...
program procptrBindcProc23b

   use ISO_C_BINDING,ONLY : C_F_PROCPOINTER, C_FUNPTR, C_FLOAT_COMPLEX, C_FUNLOC, C_ASSOCIATED, C_LOC, C_PTR
   ...
   type(dt(4)) :: dtype
   complex(C_FLOAT_COMPLEX), target :: i
   type(C_PTR) :: j, res
   complex(C_FLOAT_COMPLEX), pointer :: p, pp
   ...
   if (p /= cmplx(5.0e0,5.0e0) ) error stop 17_4
   call C_F_POINTER(j,p)
   if ( ASSOCIATED(p,i) ) error stop 18_4
   if ( p /= cmplx(10.0e0,10.0e0) ) error stop 19_4
   ...
   if ( p /= cmplx(10.0e0,10.0e0) ) error stop 29_4
...
end program procptrBindcProc23b
