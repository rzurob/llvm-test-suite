...
! ftcx_dtp -qk -ql /tstdev/OO_procptr/bindc2/procptrBindcProc13.f
...
program procptrBindcProc13

   use ISO_C_BINDING,ONLY : C_F_PROCPOINTER, C_FUNPTR, C_INT_LEAST64_T, C_FUNLOC, C_ASSOCIATED, C_LOC, C_PTR
   ...
   type(dt(4,20)) :: dtype
   integer(C_INT_LEAST64_T), target :: i
   type(C_PTR) :: j , res
   integer(C_INT_LEAST64_T), pointer :: p, pp

   procedure(cfunc), pointer :: funptr=>null()

   i = 34_C_INT_LEAST64_T
   j = C_LOC(i)
...
end program procptrBindcProc13
