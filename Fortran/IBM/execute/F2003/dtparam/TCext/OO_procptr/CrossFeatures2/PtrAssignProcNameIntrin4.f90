...
! ftcx_dtp -qk -ql /tstdev/OO_procptr/CrossFeatures2/PtrAssignProcNameIntrin4.f
...
!   PROCEDURE(ExtLEN),    POINTER, NOPASS :: PtrLEN
  ...
  V%PtrDTANH => DTANH
  IF (ABS(V%PtrDTANH(1.0D0) - 0.76159416D0) .GT. .00001 ) ERROR STOP 11

  V%PtrEXP => EXP
  IF (ABS(V%PtrEXP(1.0) - 2.7182818 ) .GT. .00001 ) ERROR STOP 12

  V%PtrIABS => IABS
  IF (V%PtrIABS(-3) .NE.  3 ) ERROR STOP 13

  V%PtrIDIM => IDIM
  IF (V%PtrIDIM(-3, 2) .NE. 0 ) ERROR STOP 14

  V%PtrIDNINT => IDNINT
  IF (V%PtrIDNINT(2.783D0) .NE. 3) ERROR STOP 15

  V%PtrINDEX => INDEX
  IF (V%PtrINDEX('FORTRAN', 'R') .NE. 3 ) ERROR STOP 16

  V%PtrISIGN => ISIGN
  IF (V%PtrISIGN(-3, 2) .NE. 3 ) ERROR STOP 17

  V%PtrLEN => LEN
  IF (V%PtrLEN('-3') .NE. 2 ) ERROR STOP 18

  V%PtrMOD => MOD
  IF (V%PtrMOD(-8, -5) .NE. -3 ) ERROR STOP 19

  V%PtrNINT => NINT
  IF (V%PtrNINT(2.783) .NE. 3 ) ERROR STOP 20
