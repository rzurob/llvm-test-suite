...
! ftcx_dtp -qck -qk -ql -qdeferredlp /tstdev/F2003/deferlen/functional/allocate/deferLenAllocate014.f
...
module m
   ...
   type, extends(base) :: child(k2)    ! (4,20,2)
      integer, kind :: k2
      integer(k2)   :: i
...
program deferLenAllocate014
   ...
   write ( 1, iostat = stat, iomsg = msg ) base(4,20)('abcdefghi'), child(4,20,2)('IBM', 2003, 'FORTRAN')
   if ( ( stat /= 0 ) .or. ( msg /= '' ) ) error stop 1_4

   rewind 1

   allocate ( b1%c1, source = '         ' )
   allocate ( b2, source = child(4,20,2) ( '   ', -999, '       ' ) )
