...
!*  ===================================================================
!*
!*  TEST CASE NAME             : dummyArgDeferPolyTarget02.f   
!*  TEST CASE TITLE            :
!*
...
!*
!*  DESCRIPTION
!*  1. actual argument is a pointer which points to target,the bound is remapped.
...
program dummyArgDeferPolyTarget02
  use m
  implicit none
  ...
  select type(pbase)
     type is(child(*,*))
        if(lbound(pbase,1) /= -5)                 error stop 24_4
        if(ubound(pbase,1) /= -3)                 error stop 25_4

        if(pbase%l1 /= 15)                        error stop 26_4
        if(pbase%l2 /= 20)                        error stop 27_4
        if(any(pbase%firstname /= "no name"))     error stop 28_4
        if(any(pbase%lastname /= "no name"))      error stop 29_4
     class default
        error stop 101_4
  end select

  if(pchild%l1 /= 15)                             error stop 30_4
  if(pchild%l2 /= 20)                             error stop 31_4
  if(lbound(pchild,1) /= 3)                       error stop 32_4
  if(ubound(pchild,1) /= 5)                       error stop 33_4
  if(any(pchild%firstname /= "no name"))          error stop 34_4
  if(any(pchild%lastname /= "no name"))           error stop 35_4
