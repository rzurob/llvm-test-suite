...
!*
!*  DESCRIPTION
!*  An optional dummy argument that is not present is subject to the following restriction:
!* It shall not be supplied as an actual argument corresponding to a non-optional dummy argument other than as the argument of the present intrinsic ...
...
program dummyArgDeferNonPolyOptional04
  use m
  implicit none
  ...
  dtp2=outer1() ! argument is not present
  ...
  contains
    ...
    function outer1(arg)
       ...
       if(present(arg)) then
           ...
           outer1=[dtp(2,6)([1,2,3],["uname1","uname2","uname3"])]
       end if 
    end function

    function inner1(arg)
       type(dtp(2,:)),allocatable :: arg(:) ! non-optional dummy argument
    ...
    function inner2(arg)
       type(dtp(2,:)),pointer :: arg ! non-optional dummy argument
