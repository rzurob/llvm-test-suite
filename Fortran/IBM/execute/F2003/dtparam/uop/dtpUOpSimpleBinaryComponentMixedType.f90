...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpUOpSimpleBinaryComponentMixedType
!*  TEST CASE TITLE            : 
!*
...
!*
!*  SECONDARY FUNCTIONS TESTED : correct operator is applied with mixed type results and involvement of intrinsic types
!*
...
!*  DESCRIPTION
!*
!*  Define several types with derived type components and type-bound procedures
!*  and generic bindings for binary operators, and verify that the correct function
...
module dtpUOpSimpleBinaryComponentMixedTypemod
  ...
  type dk (k)
     ...
     integer(k)    :: ivar
   contains
     generic :: operator(+) => binaryPlusK2, binaryPlusK4, binaryPlusK2i, binaryPlusK4i
     procedure, pass :: binaryPlusK2   ! dk(2)+dk(2) => dk(2)
     procedure, pass :: binaryPlusK4   ! dk(4)+dk(4) => dk(4)
     procedure, pass :: binaryPlusK2i  ! dk(2)+i(2)  => dk(2)
     procedure, pass :: binaryPlusK4i  ! dk(4)+i(4)  => dk(4)
  ...
  type dl (l)
     ...
     character(1)  :: cvar(l)
   contains
     generic :: operator(+) => binaryPlusL, binaryPlusLi
     procedure, pass :: binaryPlusL   ! dl+dl => dl
     procedure, pass :: binaryPlusLi  ! dl+int=> d2l(4)
  ...
  type d2k (k,k2) ! replaced ivar2 with dkvar and dkvar2
     integer, kind :: k, k2
     type(dk(k2))  :: dkvar
     integer(k)    :: ivar
     type(dk(k2))  :: dkvar2
  ...
  type d2l (k,l) ! inserted dlvar and dlvar2, made ivar an array
     integer, kind :: k
     integer, len  :: l
     character(1)  :: cvar(l)
     type(dl(l))   :: dlvar
...
contains
  ...
  integer function binarySlash2K224(this,that)
    class(d2k(2,2)), intent(in) :: this
    class(dk(4)), intent(in) :: that
    binarySlash2K224 = (int(this%ivar) * int(this%dkvar%ivar))
...
end module dtpUOpSimpleBinaryComponentMixedTypemod


program dtpUOpSimpleBinaryComponentMixedType

  use dtpUOpSimpleBinaryComponentMixedTypemod
  implicit none
...
end program dtpUOpSimpleBinaryComponentMixedType
