...
module m
    ...
    elemental type(point(4)) function negate4 (p1)
    ...
    type(point(8)) elemental function negate8 (p1)
...
use m
    type(point(8)) :: b1(60,50)
    class(point(8)), allocatable :: b2(:,:)

    type (point(4)), pointer :: b3(:)

    logical(4), external :: precision_r8, precision_r4

    forall (i=1:60, j=1:50)
        b1(i,j) = -point(8)(-i*10-j, y=log(1.0d0*(i+j*10)))
    end forall

    allocate (b2(0:59,0:49), source=-b1)

    allocate (b3(200), source=-(/(point(4)(x=i, y=1.0/i), i=1, 200)/))
