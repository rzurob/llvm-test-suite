...
!*
!*  DESCRIPTION                :
!*  To perform Unformatted Sequential File I/O with Type Bound Procedures
!*  (using the NOPASS attribute).  The Extended Derived Type refrences the
!*  Base Derived Type's Type Bound Procedures to perform the actual I/O:
!*
...
PROGRAM sequentialReadWrite02
    ...
    OPEN(11, ACCESS='sequential', ACTION='readwrite',&
    ...
    WRITE(11, IOSTAT=iostat, IOMSG=iomsg) eA
    IF (iostat /= 0) THEN
        PRINT *, "WRITE(", iostat, "): ", iomsg
        STOP 12
    END IF
    ...
    WRITE(11, IOSTAT=iostat, IOMSG=iomsg) [ (-1_8, i = 1_4, 5_4) ]
    IF (iostat /= 0) THEN
        PRINT *, "WRITE(", iostat, "): ", iomsg
        STOP 40
    END IF

    WRITE(11, IOSTAT=iostat, IOMSG=iomsg) [ (-1_4, i = 1_4, 5_4) ]
    IF (iostat /= 0) THEN
        PRINT *, "WRITE(", iostat, "): ", iomsg
        STOP 41
    END IF
    ...
    READ(11, IOSTAT=iostat, IOMSG=iomsg) ceA
    IF (iostat /= 0) THEN
        PRINT *, "WRITE(", iostat, "): ", iomsg
        STOP 61
    END IF
...
END PROGRAM sequentialReadWrite02
