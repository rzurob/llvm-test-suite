...
!* =====================================================================
!*
!*  TEST CASE NAME             : sequentialRead02
!*
...
!*
!*  DESCRIPTION                :
!*  To read data for an Array of an Extended Derived from a Sequential
!*  file in an order which is determined at run-time.
!*
...
PROGRAM sequentialRead02
    ...
    !
    k = M * (L * 2)
    OPEN(7, FILE='sequentialRead02.dat', ACTION='write', RECL=k,&
         ACCESS='sequential', FORM='unformatted', IOSTAT=iostat, IOMSG=iomsg)
    IF (iostat /= 0) THEN
    ...
    DO i = 1, (N / (L * 2))
        j = ((readOrder( i ) - 1) * L * 2) + 1

        WRITE(7, IOSTAT=iostat, IOMSG=iomsg) v( j:(j + (L * 2) - 1) )
        IF (iostat /= 0) THEN
    ...
    !
    !  READ() e from Data File:  32 byte Sequential File Record Length
    !
    OPEN(7, FILE='sequentialRead02.dat', ACTION='read', RECL=k,&
         ACCESS='sequential', FORM='unformatted', IOSTAT=iostat, IOMSG=iomsg)
    IF (iostat /= 0) THEN
        PRINT *, 'OPEN(', iostat, ') ', iomsg( 1:LEN_TRIM( iomsg ) )
        STOP 30
    ...
    DO i = 1, SIZE( readOrder )
        ...
        READ(7, IOSTAT=iostat, IOMSG=iomsg) e( j )
        IF (iostat /= 0) THEN
