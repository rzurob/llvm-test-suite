...
!*
!*  DESCRIPTION                :
!*  To read data for an Array of Derived from a Sequential file in an
...
PROGRAM sequentialRead01
    ...
    !
    k = M * L
    OPEN(7, FILE='sequentialRead01.dat', ACTION='write', RECL=k,&
    ...
    DO i = 1, (N / L)
        ...
        WRITE(7, IOSTAT=iostat, IOMSG=iomsg) v( j:(j + 1) )
        IF (iostat /= 0) THEN
    ...
    !
    !  READ() t from Data File:  16 byte Sequential File Record Length
    !
    OPEN(7, FILE='sequentialRead01.dat', ACTION='read', RECL=k,&
         ACCESS='sequential', FORM='unformatted', IOSTAT=iostat, IOMSG=iomsg)
    IF (iostat /= 0) THEN
        PRINT *, 'OPEN(', iostat, ') ', iomsg( 1:LEN_TRIM( iomsg ) )
        STOP 30
    ...
    DO i = 1, SIZE( readOrder )
        ...
        READ(7, IOSTAT=iostat, IOMSG=iomsg) t( j )
        IF (iostat /= 0) THEN
