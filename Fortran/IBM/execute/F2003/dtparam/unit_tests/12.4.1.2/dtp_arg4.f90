...
if (any(obj1(1)%element .ne. (/120, 120/))) error stop 7
if (any(obj1(2)%element .ne. (/24 , 24 /))) error stop 8
if (any(obj1(3)%element .ne. (/360, 360/))) error stop 9
if (any(obj1(4)%element .ne. (/48 , 48 /))) error stop 10
...
if (pa(1)%d .ne. 2) error stop 1

! - verify the bounds expr with length type parameter
if (ubound(pa(2)%element, 1) .ne. 2) error stop 2

! - verify the component values
if (any(pa(1)%element .ne. (/12, 12/))) error stop 4
if (any(pa(2)%element .ne. (/36, 36/))) error stop 5
if (any(pa%avar .ne. (/1, 3/))) error stop 6
