...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpPtrAssignArrayDeferred
!*
!*  DATE                       : 2009-01-12
!*
!*  PRIMARY FUNCTIONS TESTED   : Pointer Assignment without Lower Bounds Specification or Remap
!*
!*  SECONDARY FUNCTIONS TESTED : via pointers with deferred length parameters, assign scalars to arrays in main program
!*
...
!*  DESCRIPTION
!*
!*  In the main program, via pointers with deferred length and deferred length parameters,
!*  assign scalars to arrays of a parameterised derived type for which there is no user-
!*  defined assignment, and verify that the type parameters and data values are as expected.
!*
!* ============================================================================
!234567890123456789012345678901234567890123456789012345678901234567890123456789

module dtpPtrAssignArrayDeferredmod
...
end module dtpPtrAssignArrayDeferredmod

program dtpPtrAssignArrayDeferred

  use dtpPtrAssignArrayDeferredmod
  implicit none
...
!  type(base(:)), pointer      :: b_Dp(:)
  ...
  b_Dp = base(3)('abc')
  d_D4p = derived(3,4)('def',4.1_4,.true.,[1111111111,2122222222,1333333333])
  d2_D48Dp = d2(3,4,8,2)('ghi',5.9_4,.false.,[-1111111111,-2122222222,-1333333333], &
                        reshape([(1111_2*i,i=1,6)],[3,2]), &
                        derived(2,8)('xz',11235.81321D34,.true.,[76543211234567_8,-123456787654321_8]))

  b_Dp => b_5
  b_Dp  = base(5)('abcde')
  d_D8p = derived(5,8)('defgh',1.23456789D11,.true., &
                      [1111111111111_8,2222222222222_8,3333333333333_8,4444444444444_8,5555555555555_8])
  d2_D84Dp = d2(5,8,4,1)('defij',9.87654321D-12,.true., &
                        [-1111111111111_8,-2222222222222_8,-3333333333333_8,-4444444444444_8,-5555555555555_8], &
...
contains
  ...
  subroutine checkValues
    ...
    if (size(b_3) /= 5) stop 22
    if (size(d_34) /= 3) stop 23
    if (size(d2_3482) /= 1) stop 24

    if (size(b_5) /= 4) stop 32
    if (size(d_58) /= 2) stop 33
    ...
    if (b_5%l /= 5 .or. len(b_5%ch) /= 5 .or. any(b_5%ch /= 'abcde')) stop 12
...
end program dtpPtrAssignArrayDeferred
