...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpPtrAssignDummyArgAutoFixed
!*  TEST CASE TITLE            : 
!*
...
!*  The pointers passed in to the procedures have fixed lengths; the procedures
!*  rely on length dummy arguments and treat the pointers as having automatic length parameters.
!*
...
block data dtpPtrAssignDummyArgAutoFixedData
...
end block data dtpPtrAssignDummyArgAutoFixedData


program dtpPtrAssignDummyArgAutoFixed
  ...
  interface
    subroutine assignBase(left,right,n)
      import :: Base
      type(Base(n)), pointer, intent(in)      :: left
      type(Base(left%l)), pointer, intent(in) :: right
      integer, intent(in) :: n
    end subroutine assignBase

    subroutine assignDerived4(left,right,n)
      import :: Derived
      type(Derived(n,4)), pointer, intent(in)      :: left
      type(Derived(left%l,4)), pointer, intent(in) :: right
      integer, intent(in) :: n
    end subroutine assignDerived4

    subroutine assignDerived8(left,right,n)
      import :: Derived
      type(Derived(n,8)), pointer, intent(in)      :: left
      type(Derived(left%l,8)), pointer, intent(in) :: right
      integer, intent(in) :: n
    end subroutine assignDerived8

    subroutine assignD2_48(left,right,m,n)
      import :: D2
      type(D2(m,4,8,n)), pointer, intent(in)            :: left
      type(D2(left%l,4,8,left%l2)), pointer, intent(in) :: right
      integer, intent(in) :: m, n
    end subroutine assignD2_48

    subroutine assignD2_84(left,right,m,n)
      import :: D2
      type(D2(m,8,4,n)), pointer, intent(in)            :: left
      type(D2(left%l,8,4,left%l2)), pointer, intent(in) :: right
      integer, intent(in) :: m, n
  ...
  call assignBase(b_3lp, b_3rp, 3)
  call assignBase(b_5lp, b_5rp, 5)

  call assignDerived4(d_34lp, d_34rp, 3)
  call assignDerived8(d_58lp, d_58rp, 5)

  call assignD2_48(d2_3482lp, d2_3482rp, 3, 2)
  call assignD2_84(d2_5841lp, d2_5841rp, 5, 1)

  call checkValues

end program dtpPtrAssignDummyArgAutoFixed
