...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpPtrAssignModuleCommonFromVarsDeferred
!*  TEST CASE TITLE            : 
!*
...
!*
!*  SECONDARY FUNCTIONS TESTED : assign variables to variables in common in module procedure (with deferred length parameter)
!*
...
!*  DESCRIPTION
!*
!*  In module procedures, use structure constructors to assign values to
...
module dtpPtrAssignModuleCommonFromVarsDeferredmod
  ...
  type(base(:)), pointer      :: b_3_modp
  type(derived(:,4)), pointer :: d_34_modp
  type(d2(:,4,8,:)), pointer  :: d2_3482_modp
...
contains
  ...
  subroutine localTest
    ...
    type(base(:)), pointer      :: b_3_l1p
    type(derived(:,4)), pointer :: d_34_l1p
    type(d2(:,4,8,:)), pointer  :: d2_3482_l1p

    type(base(:)), pointer      :: b_5_l1p
    type(derived(:,8)), pointer :: d_58_l1p
    type(d2(:,8,4,:)), pointer  :: d2_5841_l1p
  ...
  subroutine dummyArgTest(b_1, d_1, d2_1, b_2, d_2, d2_2, b_1p, d_1p, d2_1p, b_2p, d_2p, d2_2p)
    ...
    type(base(*)), intent(out), pointer      :: b_1p
    type(derived(*,4)), intent(out), pointer :: d_1p
    type(d2(*,4,8,*)), intent(out), pointer  :: d2_1p

    type(base(*)), intent(out), pointer      :: b_2p
    type(derived(*,8)), intent(out), pointer :: d_2p
    type(d2(*,8,4,*)), intent(out), pointer  :: d2_2p
    ...
    b_1p  = base(3)('abc')
    d_1p  = derived(3,4)('def',4.1_4,.true.,[1111111111,2122222222,1333333333])
    d2_1p = d2(3,4,8,2)('ghi',5.9_4,.false.,[-1111111111,-2122222222,-1333333333], &
                        reshape([(1111_2*i,i=1,6)],[3,2]), &
                        derived(2,8)('xz',11235.81321D34,.true.,[76543211234567_8,-123456787654321_8]))

    b_2p  = base(5)('abcde')
    d_2p  = derived(5,8)('defgh',1.23456789D11,.true., &
                         [1111111111111_8,2222222222222_8,3333333333333_8,4444444444444_8,5555555555555_8])
    d2_2p = d2(5,8,4,1)('defij',9.87654321D-12,.true., &
                        [-1111111111111_8,-2222222222222_8,-3333333333333_8,-4444444444444_8,-5555555555555_8], &
                        reshape([(1111_2*i,i=1,5)],[5,1]),derived(1,4)('y',9.87654E-12,.true.,[-12345678_4]))
...
end module dtpPtrAssignModuleCommonFromVarsDeferredmod


program dtpPtrAssignModuleCommonFromVarsDeferred

  use dtpPtrAssignModuleCommonFromVarsDeferredmod
  implicit none
  integer i

  type(base(3)), target      :: b_3_main
  type(derived(3,4)), target :: d_34_main
  type(d2(3,4,8,2)), target  :: d2_3482_main

  type(base(5)), target      :: b_5_main
  type(derived(5,8)), target :: d_58_main
  type(d2(5,8,4,1)), target  :: d2_5841_main

  type(base(3)), pointer      :: b_3_mainp
  type(derived(3,4)), pointer :: d_34_mainp
  type(d2(3,4,8,2)), pointer  :: d2_3482_mainp

  type(base(5)), pointer      :: b_5_mainp
  type(derived(5,8)), pointer :: d_58_mainp
  type(d2(5,8,4,1)), pointer  :: d2_5841_mainp

  call localTest
  call moduleTest
  call dummyArgTest(b_3_main, d_34_main, d2_3482_main, b_5_main, d_58_main, d2_5841_main, b_3_mainp, d_34_mainp, d2_3482_mainp, b_5_mainp, d_58_mainp, d2_5841_mainp)

end program dtpPtrAssignModuleCommonFromVarsDeferred
