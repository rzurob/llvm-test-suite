...
!*
!*  SECONDARY FUNCTIONS TESTED : assign to COMMON variables in main program and check values via pointers with fixed length parameters
!*
...
!*  parameters and data values are as expected, via pointers with fixed length parameters.
!*
...
block data dtpPtrAssignCommonFromMainFixedChkData
...
end block data dtpPtrAssignCommonFromMainFixedChkData


program dtpPtrAssignCommonFromMainFixedChk
...
contains
  ...
  subroutine checkValues
    ...
    type(d2(3,4,8,2)), pointer  :: d2_3481p
...
end program dtpPtrAssignCommonFromMainFixedChk
