...
!*  ============================================================================
!*
!*  TEST CASE NAME             : dtpIAssignInternalHostAssociatedFromVars
!*
!*  DATE                       : 2008-11-12
!*
!*  PRIMARY FUNCTIONS TESTED   : Intrinsic Assignment without Allocation
!*
!*  SECONDARY FUNCTIONS TESTED : assign variables to host associated variables in internal subprogram
!*
...
!*  and then assign those variables to host-associated variables.
!*  Verify that the type parameters and data values are as expected.
!*
!* ============================================================================
!234567890123456789012345678901234567890123456789012345678901234567890123456789

module dtpIAssignInternalHostAssociatedFromVarsmod
...
end module dtpIAssignInternalHostAssociatedFromVarsmod


program dtpIAssignInternalHostAssociatedFromVars

  use :: dtpIAssignInternalHostAssociatedFromVarsmod

  implicit none

  type(base(3))      :: b_3, b_3a
  type(derived(3,4)) :: d_34, d_34a
  type(d2(3,4,8,2))  :: d2_3482, d2_3482a

  type(base(5))      :: b_5, b_5a
  type(derived(5,8)) :: d_58, d_58a
  type(d2(5,8,4,1))  :: d2_5841, d2_5841a
...
contains
  ...
  subroutine hostAssociatedTest
    ...
    b_3a     = base(3)('abc')
    d_34a    = derived(3,4)('def',4.1_4,.true.,[1111111111,2122222222,1333333333])
    d2_3482a = d2(3,4,8,2)('ghi',5.9_4,.false.,[-1111111111,-2122222222,-1333333333], &
                           reshape([(1111_2*i,i=1,6)],[3,2]), &
                           derived(2,8)('xz',11235.81321D34,.true.,[76543211234567_8,-123456787654321_8]))

    b_5a     = base(5)('abcde')
    d_58a    = derived(5,8)('defgh',1.23456789D11,.true., &
                            [1111111111111_8,2222222222222_8,3333333333333_8,4444444444444_8,5555555555555_8])
    d2_5841a = d2(5,8,4,1)('defij',9.87654321D-12,.true., &
                           [-1111111111111_8,-2222222222222_8,-3333333333333_8,-4444444444444_8,-5555555555555_8], &
                           reshape([(1111_2*i,i=1,5)],[5,1]),derived(1,4)('y',9.87654E-12,.true.,[-12345678_4]))
...
end program dtpIAssignInternalHostAssociatedFromVars
