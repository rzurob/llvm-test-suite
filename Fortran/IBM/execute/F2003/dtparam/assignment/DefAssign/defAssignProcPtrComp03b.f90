...
!*  3. Defect 362336 block other issues, hard to debug test case without fixing defect 362336. leave it to future analysis.
...
module m
   ...
   type,extends(child) :: gen3(k3,l3)
      ...
      contains
          procedure,pass :: assign=>assigngen3
          generic :: assignment(=)=>assign
   end type
   ...
   contains
     ...
     elemental subroutine assignchild(this,dt)
         class(child(2,*,4,*)),intent(inout) :: this
         class(*),intent(in)    :: dt
     ...
     elemental subroutine assigngen3(this,dt)
         class(gen3(2,*,4,*,8,*)),intent(inout) :: this
         class(*),intent(in)    :: dt
...
program defAssignProcPtrComp03a
   ...
   integer :: i
   class(base(2,:)),allocatable :: obj1(:),obj2(:),obj4(:)
   ...
   print *,"****TEST  1****"
   obj2(2)=procptr1(obj1(1))

   select type (obj2)
   type is (gen3(2,*,4,*,8,*))
     call assigngen3(obj2(2),fun(obj1(1)))
   end select
   ...
   select type(x=>obj2)
        type is(base(2,*))
            ...
            if(any(x(2)%i1 /= [4,5,6]))                 error stop 102
