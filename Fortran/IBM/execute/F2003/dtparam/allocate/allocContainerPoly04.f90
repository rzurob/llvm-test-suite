...
!* =====================================================================
!*
!*  TEST CASE NAME             : allocContainerPoly04
!*
!*  DATE                       : January  7, 2009
...
MODULE cMod
    ...
    CONTAINS
        ...
        FUNCTION NewC(l1, l2, s, rc)
            INTEGER :: l1
            INTEGER :: l2
            INTEGER :: s
            INTEGER(4) :: rc

            CLASS(c(2,l1,l2)), POINTER :: NewC( : )

            INTEGER :: stat

            CHARACTER(255) :: errmsg

            ALLOCATE(NewC( s ), STAT=stat, ERRMSG=errmsg)
            IF (stat /= 0) THEN
                PRINT *, "ALLOCATE(NewC(", s, ",STAT=", stat, ") ", errmsg
                CALL zzrc( rc )
            END IF
...
END PROGRAM allocContainerPoly04
