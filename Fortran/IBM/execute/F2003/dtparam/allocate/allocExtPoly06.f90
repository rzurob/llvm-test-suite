...
MODULE allocMod
    ...
    CONTAINS
        ...
        INTEGER(4) FUNCTION AllocFunc( extArg )
            CLASS(ext(4,*,*)), POINTER :: extArg( : )

            CHARACTER(255) :: errmsg

            ALLOCATE(extArg( length ), STAT=AllocFunc, ERRMSG=errmsg)
            IF (AllocFunc /= 0_4) THEN
                PRINT *, "ALLOCATE(extArg(",length,",STAT=",AllocFunc,") ",errmsg
                AllocFunc = rc
            END IF
...
PROGRAM allocExtPoly06
    ...
    DO length = 1, 10
        ...
        stat = AllocFunc( cExt )
        IF (stat == 0_4)  stat = Check( )

        IF (stat /= 0_4)  CALL zzrc( stat )
    END DO

END PROGRAM allocExtPoly06
