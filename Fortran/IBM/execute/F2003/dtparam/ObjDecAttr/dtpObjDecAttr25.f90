...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  SELECT CASE (M)
  CASE (1)
    IF (  ALLOCATED ( Tar0) ) ERROR STOP 11
    IF (  ASSOCIATED( Tar1) ) ERROR STOP 12
    IF (  ALLOCATED ( Tar2) ) ERROR STOP 13

    IF (  ASSOCIATED(Ptr0) ) ERROR STOP 14
    IF (  ASSOCIATED(Ptr1) ) ERROR STOP 15
  ...
  CASE (2)
    IF ( .NOT. ALLOCATED ( Tar0) ) ERROR STOP 21
    IF ( .NOT. ASSOCIATED( Tar1) ) ERROR STOP 22
    IF ( .NOT. ALLOCATED ( Tar2) ) ERROR STOP 23

    IF ( .NOT. ASSOCIATED(Ptr0) ) ERROR STOP 24
    IF ( .NOT. ASSOCIATED(Ptr1) ) ERROR STOP 25
    IF ( .NOT. ASSOCIATED(Ptr2) ) ERROR STOP 26

    IF ( Ptr0%L0         .NE. 1 ) ERROR STOP 31
    IF ( LBOUND(Ptr0,1)  .NE. N ) ERROR STOP 32
    IF ( SIZE(Ptr0)      .NE. N ) ERROR STOP 33

    IF ( Ptr1%L0        .NE. 3       )  ERROR STOP 41
    IF ( Ptr1%L1        .NE. 5       )  ERROR STOP 42
    IF ( LBOUND(ptr1, 1).NE. N       )  ERROR STOP 43
    IF ( SIZE(Ptr1)     .NE. N       )  ERROR STOP 44
    IF ( ANY( Ptr1%C1   .NE. "XYZ" ) )  ERROR STOP 45

    IF ( Ptr2%L0        .NE. 3       )  ERROR STOP 41
    IF ( Ptr2%L1        .NE. 5       )  ERROR STOP 42
    IF ( Ptr2%L2        .NE. 7       )  ERROR STOP 43
    IF ( LBOUND(ptr2,1) .NE. N       )  ERROR STOP 44
    IF ( SIZE(Ptr2)     .NE. N       )  ERROR STOP 45

    DO I=N, 2*N-1
      IF ( Ptr2(I)%L0                .NE.   3        )  ERROR STOP 81
      IF ( Ptr2(I)%L1                .NE.   5        )  ERROR STOP 82
      IF ( Ptr2(I)%L2                 .NE.   7       )  ERROR STOP 83
      ...
      IF ( ASSOCIATED( Ptr2(I)%Ptr2) .EQV.   .TRUE.  )  ERROR STOP 92
      IF ( Ptr2(I)%Ptr2%K2           .NE.    8       )  ERROR STOP 93
      IF ( Ptr2(I)%Ptr2%L2           .NE.    7       )  ERROR STOP 94
    END DO
