...
!*  ===================================================================
!*
!*  TEST CASE NAME             : dtParameterInquiryArrayCharComp03.f   
!*  TEST CASE TITLE            :
!*
...
!* 4. CHARACTER ARRAY POINTER COMPONENT
!* 5. DEFECT 353295 353685
...
module m
   type base(k1,k2,l1,l2)
      ...
      character(k1),pointer :: c1(:)
      character(k2),pointer :: c2(:)
      character(len=l1),pointer :: c3(:,:)
      character(len=l2),pointer :: c4(:)
      character(l1+l2),pointer  :: c5(:)
      character(k2),pointer,dimension(:) :: c6 !defect 353685
      character(k1+k2),pointer,dimension(:) :: c7
      character(k1%kind),pointer :: c8(:)
      character(k1%kind+k2%kind),pointer :: c9(:,:)
      character(kind(k1+k2)),pointer :: c10(:)
      character(l1%kind+l2%kind),pointer :: c11(:)   ! defect 353295
      character(k1+k2),pointer :: c12(:) ! defect 353685
      character(l2%kind),pointer :: c13(:)
      character(l1%kind),pointer :: c14(:)

   end type 
         
end module

  program dtParameterInquiryArrayCharComp03
  use m
  implicit none
