...
!*  the integer default.  "c" was originally declared as:
...
!*  Unfortunately, the KIND of these parameters is not handled correctly, so we have
!*  to coerce the type parameters to have the right KIND using:
!*    character(kind(int(k1,1)+int(k2,k1))) :: c(kind(int(k1,1)+int(k2,k1)))
!*  Also, it seems that IF statements may handle the KIND information differently than
