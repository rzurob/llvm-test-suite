...
!*  the integer default.  "i7" and "i9" were originally declared as:
...
!*  kind(k1+k2) should both be 2, and not the default kind.  So, we have
!*  to coerce the type parameters to have the right KIND using:
!*     integer(kind(int(k2,k1))),allocatable :: i7(:)
!*     integer(kind(int(k1,1)+int(k2,k1))),allocatable :: i9(:)
!*  One other line has to be changed with a workaround for defect 353689:
!*     allocate(integer(t%k1+t%k2) :: t%i6(-1:1,-1:0))
!*  just needs to dropt the superfluous type specifier.
