...
program typeParamInquiryDTIntrinsic09
  use m
  implicit none
  ...
  if(dt1%i /= 1 )                                       error stop 27_4
  if(dt1%i%kind /= kind(dt1%i) .or. dt1%i%kind /= 2)    error stop 28_4
  if(dt1%c1 /= "first")                                 error stop 29_4
     ...
     subroutine sub(dt)
        ...
        select type(dt)
           type is (third)
              dt%r=3.0
              dt%i=3
              allocate(dt%c1,source="third")
              allocate(dt%c2,source=t3)
           type is (second)
              dt%i=2
              allocate(dt%c1,source="second")
              allocate(dt%c2,source=t2)
           type is (first)
              dt%i=1
