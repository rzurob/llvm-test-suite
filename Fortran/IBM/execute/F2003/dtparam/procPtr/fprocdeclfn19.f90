...
USE N
 ...
 CONTAINS
  ...
  USE M
     TYPE(Child1(4)),ALLOCATABLE, INTENT(INOUT) :: x
	 ALLOCATE(x)
	 IF ( allocated(x) .eqv. .TRUE. ) THEN
	  alloc_Child1_TYPES=0
	 ELSE
  ...
  USE M
	 ...
	 ALLOCATE(x)
	 IF ( allocated(x) .eqv. .TRUE. ) THEN
	  alloc_Child2_TYPES=0
	 ELSE
	  alloc_Child2_TYPES=1
	 END IF
  END FUNCTION alloc_Child2_TYPES

  INTEGER FUNCTION alloc_Child3_TYPES(x)
  USE M
     TYPE(Child3(4)),ALLOCATABLE, INTENT(INOUT) :: x
	 ALLOCATE(x)
	 IF ( allocated(x) .eqv. .TRUE. ) THEN
	  alloc_Child3_TYPES=0
	 ELSE
	  alloc_Child3_TYPES=1
	 END IF
  END FUNCTION alloc_Child3_TYPES

  INTEGER FUNCTION alloc_Child4_TYPES(x)
  USE M
     TYPE(Child4(4)),ALLOCATABLE, INTENT(INOUT) :: x
	 ALLOCATE(x)
	 IF ( allocated(x) .eqv. .TRUE. ) THEN
	  alloc_Child4_TYPES=0
	 ELSE
...
USE FnInt
 ...
 PROCEDURE(Ialloc_Child1_TYPES), POINTER :: procptr1=>null()
 ...
 TYPE(Child4(4)),ALLOCATABLE,TARGET:: T4a,T4b
 ...
 IF ( AllocRes .eq. 1 ) THEN
  STOP 1
 END IF
  AllocRes=procptr1(T1b)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 2
 END IF
  AllocRes=procptr2(T2a)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 3
 END IF
  AllocRes=procptr2(T2b)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 4
 END IF
  AllocRes=procptr3(T3a)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 5
 END IF
  AllocRes=procptr3(T3b)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 6
 END IF
  AllocRes=procptr4(T4a)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 7
 END IF
  AllocRes=procptr4(T4b)
 IF ( AllocRes .eq. 1 ) THEN
  STOP 8
 END IF
 !INTEGER
...
print*,"---POINTER---"
 ...
 print*,T1a%Num
  ...
  print*,"Real:"
 print*,"Using Procedure pointer:"
 call procptr(T2c,T2d)
 print*,T5%Rnum%Num
 print*,"Using subroutine call:"
 call What_TYPES	(T2c,T2d)
 print*,T5%Rnum%Num
 print*,"Using INTERFACE SWITCH:"
 call SWITCH(T2c,T2d)
 print*,T2a%Num
  ...
  print*,"Complex:"
 print*,"Using Procedure pointer:"
 call procptr(T3c,T3d)
 print*,T5%Cnum%Num
 print*,"Using subroutine call:"
 call What_TYPES	(T3c,T3d)
 print*,T5%Cnum%Num
 print*,"Using INTERFACE SWITCH:"
 call SWITCH(T3c,T3c)
 print*,T3a%Num
  ...
  print*,"Logical:"
 print*,"Using Procedure pointer:"
 call procptr(T4c,T4d)
 print*,T5%Lnum%Num
 print*,"Using subroutine call:"
 call What_TYPES	(T4c,T4d)
 print*,T5%Lnum%Num
 print*,"Using INTERFACE SWITCH:"
 call SWITCH(T4c,T4c)
 print*,T4a%Num
