...
PROGRAM FunctionResult03
      ...
      SELECT TYPE ( s => func(u_poly) )
        CLASS IS (NextGen(4,*,4,*,*,*))
           ...
           IF ( s%C0       .NE. '0' ) STOP 23
           IF ( SIZE(s%F0) .NE.   1 ) STOP 24
           IF ( ANY(s%F0 .NEQV. .True.) ) STOP 25
           IF ( s%k2       .NE.   4 ) STOP 26
           IF ( s%l2       .NE.   2 ) STOP 27
           IF ( SIZE(s%A0) .NE.   2 ) STOP 28
           IF ( ANY(s%A0   .NE. -1) ) STOP 29
           IF ( s%l3       .NE.   5 ) STOP 30
           IF ( s%l4       .NE.  10 ) STOP 31
           IF ( .NOT. ASSOCIATED(s%ptr) ) ALLOCATE( s%ptr, SOURCE = func( Base(4,3) ('IBM', .False.) ) )
             SELECT TYPE ( p => s%ptr )
                CLASS IS (Base(4,*))
                   IF ( p%k1       .NE.     4 ) STOP 40
                   IF ( p%l1       .NE.     3 ) STOP 41
                   IF ( LEN(p%C0)  .NE.     3 ) STOP 42
                   IF ( p%C0       .NE. 'IBM' ) STOP 23
                   IF ( SIZE(p%F0) .NE.     3 ) STOP 44
                   IF ( ANY(p%F0 .NEQV. .False.) ) STOP 45
