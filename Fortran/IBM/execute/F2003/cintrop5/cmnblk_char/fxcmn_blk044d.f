...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/cmn_blk001.sh fxcmn_blk044d cxcmn_blk042
! %COMPOPTS: -qfree=f90 
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
! %STDOUT: 
! %EXECARGS:
! %POSTCMD: rm -f *.o *.mod fxcmn_blk044d fxcmn_blk044d.out
! %END
!**********************************************************************
!*  ===================================================================
...
module fmod1 
   ...
   CONTAINS
	...
	character(kind=C_CHAR)  :: char1(16)
...
! ----------------------------------------------------------------------------
	...
	call Intern_FSub()
        ...
        if ( char1(1)   .ne.    '\f'    )                       error stop 82
        if ( char1(2)   .ne.    'N'     )                       error stop 83
        if ( char1(3)   .ne.    'o'     )                       error stop 84
        if ( char1(4)   .ne.    ','     )                       error stop 85
        if ( char1(5)   .ne.    ' '     )                       error stop 86
        if ( char1(6)   .ne.    'I'     )                       error stop 87
        if ( char1(7)   .ne.    '\''    )                       error stop 88
        if ( char1(8)   .ne.    "m"     )                       error stop 89
        if ( char1(9)   .ne.    ' '     )                       error stop 90
        if ( char1(10)  .ne.    'a'     )                       error stop 91
        if ( char1(11)  .ne.    ' '     )                       error stop 92
        if ( char1(12)  .ne.    'd'     )                       error stop 93
        if ( char1(13)  .ne.    'o'     )                       error stop 94
        if ( char1(14)  .ne.    'g'     )                       error stop 95
