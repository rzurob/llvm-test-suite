...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/cmn_blk001.sh fxcmn_blk052d cxcmn_blk052
! %COMPOPTS: -qfree=f90 
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
! %STDOUT: 
! %EXECARGS:
! %POSTCMD: rm -f *.o *.mod fxcmn_blk052d fxcmn_blk052d.out
! %END
!**********************************************************************
!*  ===================================================================
...
! ----------------------------------------------------------------------------
	...
	call Intern_FSub()
        ...
        if ( log1(1)   .neqv.  .true.  )        error stop 50
        if ( log1(2)   .neqv.  .false. )        error stop 51
        if ( log1(3)   .neqv.  .false. )        error stop 52
        if ( log1(4)   .neqv.  .true.  )        error stop 53
        if ( log1(5)   .neqv.  .true.  )        error stop 54
