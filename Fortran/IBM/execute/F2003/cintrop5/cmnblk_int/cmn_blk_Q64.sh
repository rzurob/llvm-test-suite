...
#----------------------------------------------------------------------------------------------
#-- This script will be used to compile programs that need -q64 option.
#----------------------------------------------------------------------------------------------
# ...let's force -q64 then
OPTIONS="$OPTIONS -q64"
...
#----------------------------------------------------------------------------------------------
#-- Exit program if run without -q64 
#----------------------------------------------------------------------------------------------

echo ${OPTIONS} | grep -q "\-q64"
RC=$?
if [[ ${RC} -ne 0 ]]; then
   echo " ***  This program must be run with -q64 option "
   echo " ***            Exiting program             *** "
   exit 222
fi
