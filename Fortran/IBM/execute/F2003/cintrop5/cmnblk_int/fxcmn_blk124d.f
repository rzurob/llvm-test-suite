...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/cmn_blk001.sh fxcmn_blk124d cxcmn_blk124
! %COMPOPTS: -qfree=f90
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
! %POSTCMD: rm -f *.o *.mod fxcmn_blk124d fxcmn_blk124d.out
! %END
!**********************************************************************
!*  ===================================================================
!*
...
module fmod1
   ...
   !*** Temporary Matrix (for comparison purposes)
    INTEGER(8)         tmp_c_int_fast64_t(2,2,2)
...
! ----------------------------------------------------------------------------
          ...
          bind(c,Name='_') ::    /blk_int_C_INT_FAST64_T/
...
! ----------------------------------------------------------------------------
         ...
         do i = 1, 8
           ...
           if ( res_C_INT_FAST64_T(i)    .ne.    cmp_C_INT_FAST64_T(i))          call zzrc(10+i)
...
! ----------------------------------------------------------------------------
         ...
         do k = 1, 2
            do j = 1, 2
               do i = 1, 2
                 tmp_c_int_fast64_t(i,j,k)  =           int_c_int_fast64_t(k,j,i)
             end do
           end do
         end do
...
! ----------------------------------------------------------------------------
         ...
         cmp_C_INT_FAST64_T  =   RESHAPE( tmp_C_INT_FAST64_T, (/8/))

         do i = 1, 8

           if ( res_C_INT_FAST64_T(i)    .ne.    cmp_C_INT_FAST64_T(i))          call zzrc(20+i)
...
! ----------------------------------------------------------------------------
         ...
         do i = 1, 8
           ...
           if ( res_C_INT_FAST64_T(i)    .ne.    cmp_C_INT_FAST64_T(i))          call zzrc(50+i)
