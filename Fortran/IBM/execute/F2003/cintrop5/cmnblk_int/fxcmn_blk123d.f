...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/cmn_blk001.sh fxcmn_blk123d cxcmn_blk123
! %COMPOPTS: -qfree=f90 
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
! %STDOUT: 
! %EXECARGS:
! %POSTCMD: rm -f *.o *.mod fxcmn_blk123d fxcmn_blk123d.out
! %END
!**********************************************************************
!*  ===================================================================
...
module fmod1 
   ...
   !*** Temporary Matrix (for comparison purposes) 
    INTEGER(4)         tmp_c_int_fast32_t(2,2,2) 
...
! ----------------------------------------------------------------------------
          ...
          bind(c,Name='_') ::    /blk_int_C_INT_FAST32_T/ 
...
! ----------------------------------------------------------------------------
         ...
         do i = 1, 8   
           ...
           if ( res_C_INT_FAST32_T(i)    .ne.    cmp_C_INT_FAST32_T(i))          call zzrc(10+i) 
...
! ----------------------------------------------------------------------------
         ...
         do k = 1, 2 
            do j = 1, 2 
               do i = 1, 2 
                 tmp_c_int_fast32_t(i,j,k)  =           int_c_int_fast32_t(k,j,i) 
             end do 
           end do 
         end do 
...
! ----------------------------------------------------------------------------
         ...
         cmp_C_INT_FAST32_T  =   RESHAPE( tmp_C_INT_FAST32_T, (/8/)) 

         do i = 1, 8   

           if ( res_C_INT_FAST32_T(i)    .ne.    cmp_C_INT_FAST32_T(i))          call zzrc(20+i) 
...
! ----------------------------------------------------------------------------
         ...
         do i = 1, 8   
           ...
           if ( res_C_INT_FAST32_T(i)    .ne.    cmp_C_INT_FAST32_T(i))          call zzrc(50+i) 
