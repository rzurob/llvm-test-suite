...
! %START
! %MAIN: YES
! %PRECMD: ${TR_SRC}/cmn_blk_qlngdbl.sh fxcmn_blk593 cxcmn_blk551
! %COMPOPTS: -qfree=f90
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
! %POSTCMD: rm -f fxcmn_blk593.o cxcmn_blk551.o fxcmn_blk593
! %END
!**********************************************************************
!*  ===================================================================
!*
...
!*
!*				 Test:  BIND(C) statement inside internal subroutine in module procedure
!*
...
! ----------------------------------------------------------------------------
      ...
      do i = 1, 2
        ...
        if ( int_c_int_least32_t(k,j,i)    .ne. 80 )      error stop 170
        if ( r_c_double_s8a(k,j,i)         .ne. 79 )      error stop 171
        if ( int_s2b(k,j,i)                .ne. 78 )      error stop 172
        if ( int_c_int_fast32_t(k,j,i)     .ne. 77 )      error stop 173
        if ( int_s4a(k,j,i)                .ne. 76 )      error stop 174
        if ( int_c_int_fast64_t(k,j,i)     .ne. 75 )      error stop 175
        if ( real_s16c(k,j,i)              .ne. 74 )      error stop 176
        if ( int_s4b(k,j,i)                .ne. 73 )      error stop 177
        if ( int_s8b(k,j,i)                .ne. 72 )      error stop 178
        if ( int_c_signed_char(k,j,i)      .ne. 71 )      error stop 179
        if ( int_c_short(k,j,i)            .ne. 70 )      error stop 180
        if ( real_s8b(k,j,i)               .ne. 69 )      error stop 181
        if ( int_c_int(k,j,i)              .ne. 68 )      error stop 182
        if ( int_c_long_long(k,j,i)        .ne. 67 )      error stop 183
        if ( int_c_long(k,j,i)             .ne. 66 )      error stop 184
        if ( char_c_char(k,j,i)            .ne. 'Z' )     error stop 185
        if ( int_c_size_t(k,j,i)           .ne. 65 )      error stop 186
        if ( int_c_intptr_t(k,j,i)         .ne. 64 )      error stop 187
        if ( real_s4c(k,j,i)               .ne. 63 )      error stop 188
        if ( int_c_intmax_t(k,j,i)         .ne. 62 )      error stop 189
        if ( int_c_int8_t(k,j,i)           .ne. 61 )      error stop 190
        if ( int_c_int16_t(k,j,i)          .ne. 60 )      error stop 191
        if ( int_c_int32_t(k,j,i)          .ne. 59 )      error stop 192
        if ( r_c_float_s4a(k,j,i)          .ne. 58 )      error stop 193
        if ( int_c_int64_t(k,j,i)          .ne. 57 )      error stop 194
        if ( int_c_int_least8_t(k,j,i)     .ne. 56 )      error stop 195
        if ( int_c_int_least16_t(k,j,i)    .ne. 55 )      error stop 196
        if ( char1(k,j,i)                  .ne. 'Z' )     error stop 197
        if ( int_c_int_least64_t(k,j,i)    .ne. 54 )      error stop 198
        if ( log1(k,j,i)               .neqv. .false. )   error stop 199

        if ( int_c_int_fast8_t(k,j,i)      .ne. 53 )      error stop 130
        if ( int_s1b(k,j,i)                .ne. 52 )      error stop 131
        if ( int_c_int_fast16_t(k,j,i)     .ne. 51 )      error stop 132
        if ( real_s4b(k,j,i)               .ne. 50 )      error stop 133
