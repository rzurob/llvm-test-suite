...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  IF (ABS(ExtATAN2(1.5574077, 1.0) - 1.0) .GT. .00001 ) ERROR STOP 11

  IF (ABS(ExtCABS((3.0, 4.0)) - 5.0) .GT. .00001 ) ERROR STOP 12

  IF (ABS(ExtCCOS((1.0, .0)) - (0.5403022766,-0.000000E+00)) .GT. .00001 ) ERROR STOP 13

  IF (ABS(ExtCEXP((1.0, .0)) - (2.718281746,0.00E+00)) .GT. .00001 ) ERROR STOP 14

  IF (ABS(EXP(ExtCLOG((1.0, 1.0))) - (1.0, 1.0)) .GT. .00001 ) ERROR STOP 15

  IF (ExtCONJG((2.0, -2.0)) .NE. (2.0, 2.0) )   ERROR STOP 16

  IF (ABS(ExtCOS(1.0) - 0.54030231) .GT.  .00001 )   ERROR STOP 17

  IF (ABS(ExtCOSH(1.0) - 1.5430806) .GT.  .00001 )   ERROR STOP 18

  IF (ABS(ExtCSIN((1.0,1.0)) - (1.298457623,0.6349639297)) .GT. .00001 ) ERROR STOP 19

  IF (ABS(ExtCSQRT((1.0,1.0)) - (1.098684072,0.4550898671)) .GT. .00001 ) ERROR STOP 20
