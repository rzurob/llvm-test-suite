...
!     INTEGER, OPTIONAL, INTENT(IN) ::  Arg2
  ...
  PtrABS => ABS
  IF (ABS(PtrABS(-1.0) - 1.0) .GT. .00001 ) ERROR STOP 11

  PtrACOS => ACOS
  IF (PtrABS(PtrACOS(0.54030231) - 1.0) .GT. .00001 ) ERROR STOP 12

  PtrAIMAG => AIMAG
  IF (PtrAIMAG((2.0, -1.0)) .NE. -1.0 )   ERROR STOP 13

  PtrAINT => AINT
  IF (PtrAINT(-2.783) .NE. -2.0 )   ERROR STOP 14

  PtrALOG => ALOG
  IF (PtrALOG(1.0) .NE. 0.0 ) ERROR STOP 15

  PtrLOG10 => ALOG10
  IF (PtrLOG10(1.0) .NE. 0.0 ) ERROR STOP 16

  PtrAMOD => AMOD
  IF (PtrAMOD(3.0, 2.0) .NE. 1.0 ) ERROR STOP 17

  PtrANINT => ANINT
  IF (PtrANINT(2.783) .NE. 3.0 ) ERROR STOP 18

  PtrASIN => ASIN
  IF (PtrABS(PtrASIN(0.84147098)-1.0) .GT. .00001 ) ERROR STOP 19

  PtrATAN => ATAN
  IF (PtrABS(PtrATAN(1.5574077)-1.0) .GT. .00001 ) ERROR STOP 20
