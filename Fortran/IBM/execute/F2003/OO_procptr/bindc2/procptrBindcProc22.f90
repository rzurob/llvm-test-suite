...
program procptrBindcProc22
   ...
   if (.not.precision_R6(p,5.0q0)) error stop 26_4

   call C_F_POINTER(j,p)
   if (.not.precision_R6(p,5.0q0)) error stop 27_4
   if (.not.precision_R6(pp,5.0q0)) error stop 28_4

   call C_F_POINTER(res,pp)
   if ( ASSOCIATED(pp,i) ) error stop 29_4
   if (.not.precision_R6(pp,10.0q0)) error stop 30_4

end program procptrBindcProc22
