...
! %MAIN: YES
! %PRECMD: rm -f  fort.* 
! %COMPOPTS: -qposition=appendunknown
! %GROUP: fxstio211.f 
! %VERIFY:
! %STDIN:
! %STDOUT:
...
!* TEST1 : integer
   ...
   if (fsize(2) .ne. 2*fsize(1))     error stop 111
...
!* TEST2 : complex
   ...
   if (fsize(2) .ne. 2*fsize(1))     error stop 21
   close(1, status='delete')
...
!* TEST4 : byte
   ...
   if (fsize(2) .ne. 2*fsize(1))     error stop 41
   close(1, status='delete')
