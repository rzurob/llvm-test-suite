...
!*
!*  DESCRIPTION                : Test record structure in list-directed
!*				 stream I/O.
!*
!* ===================================================================
!*  REVISION HISTORY
...
!**********************************************************
     ...
     OPEN(1, FILE='fxstio154.dat', FORM='FORMATTED', ACCESS='STREAM', &
    &     STATUS='REPLACE', IOSTAT=ios, ERR=90, DELIM='QUOTE')

     WRITE(1, FMT=*, IOSTAT=ios, ERR=91) st_in1
     WRITE(1, FMT=*, IOSTAT=ios, ERR=91, POS=5000) (st_in2(i),i=1,N)
     WRITE(1, FMT=*, IOSTAT=ios, ERR=91, POS=15000) st_in3

     REWIND(1, IOSTAT=ios, ERR=93)

     READ(1, FMT=*, IOSTAT=ios, ERR=92) st_out1
     READ(1, FMT=*, IOSTAT=ios, ERR=92, POS=5000) st_out2
     READ(1, FMT=*, IOSTAT=ios, ERR=92, POS=15000) ((st_out3(i,j),i=1,N),j=1,N)
...
!**********************************************************
     ...
     if ( .not. precision_X8(st_in1%x8, st_out1%x8) ) error stop 12
     if ( st_in1%l4 .neqv. st_out1%l4 ) error stop 13
     if ( st_in1%ch15 .ne. st_out1%ch15 ) error stop 14

     i = 1
     j = 1
     do while (i .le. N)
        if ( st_in2(i)%i4 .ne. st_out2(i)%i4 ) error stop 15
        if ( .not. Array_Check (st_in2(i)%r4, st_out2(i)%r4) ) error stop 16
   ...
   &           error stop 21
           if ( .not. precision_X8(st_in3(i,j)%x8, st_out3(i,j)%x8) ) &
   &           error stop 22
           if ( st_in3(i,j)%l4 .neqv. st_out3(i,j)%l4 ) error stop 23
           if ( st_in3(i,j)%ch15 .ne. st_out3(i,j)%ch15 ) error stop 24
