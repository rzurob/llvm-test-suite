...
!*                               stream access using sequential I/O.
!*
!* ===================================================================
!*  REVISION HISTORY
...
!**********************************************************
     ...
     OPEN(1, FILE='fxstio133.dat', FORM='FORMATTED', ACCESS='STREAM', &
    &     STATUS='REPLACE', IOSTAT=ios, ERR=90)
     ...
     OPEN(2, FILE='fxstio133.dat', FORM='FORMATTED', ACCESS='SEQUENTIAL', &
    &     STATUS='OLD', IOSTAT=ios, ERR=90)

     call read_sequential(2)
...
90   print *, "Error while openning the file: IOSTAT = ", ios
	...
	subroutine write_stream(unit)
	   integer unit
           ...
           WRITE(unit, FMT='(I10)',IOSTAT=ios,ERR=91,POS=6,ADVANCE='no') i4_in
           WRITE(unit, FMT='(I20)', IOSTAT=ios, ERR=91) i8_in

           WRITE(unit, FMT='(F9.6)',IOSTAT=ios,ERR=91,POS=37,ADVANCE='no') r4_in
           WRITE(unit, FMT='(D25.17)', IOSTAT=ios, ERR=91) r8_in
           WRITE(unit, FMT='(Q40.32)', IOSTAT=ios, ERR=91, POS=71) r16_in

           WRITE(unit, FMT='(D25.17)', IOSTAT=ios, ERR=91, POS=112) d8_in

           WRITE(unit, FMT='(2E15.7)',IOSTAT=ios,ERR=91,POS=138,ADVANCE='no') x8_in
           WRITE(unit, FMT='(2D25.17)',IOSTAT=ios,ERR=91, ADVANCE='no') x16_in
           WRITE(unit, FMT='(2Q40.32)',IOSTAT=ios,ERR=91) x32_in

           WRITE(unit, FMT='(L4)', IOSTAT=ios, ERR=91, POS=299) l2_in
           WRITE(unit, FMT='(L4)', IOSTAT=ios, ERR=91, POS=303) l4_in
           WRITE(unit, FMT='(L4)', IOSTAT=ios, ERR=91, POS=307) l8_in

           WRITE(unit, FMT='(A1)', IOSTAT=ios, ERR=91, POS=312) ch1_in
           WRITE(unit, FMT='(A16)', IOSTAT=ios, ERR=91, POS=313) ch15_in

           WRITE(unit, FMT='(B8)', IOSTAT=ios, ERR=91, POS=330) b_in
...
91         print *, "Error while writing to the file: IOSTAT = ", ios
	...
	subroutine read_sequential(unit)
	   integer unit
          ...
          &      x8_out, x16_out, x32_out
           READ(unit, FMT='(3L4)', IOSTAT=ios, ERR=92) &
          &      l2_out, l4_out, l8_out
           READ(unit, FMT='(A1,A16)', IOSTAT=ios, ERR=92) &
          &      ch1_out, ch15_out
