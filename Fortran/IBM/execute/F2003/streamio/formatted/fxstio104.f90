...
! %START
! %MAIN: YES
! %PRECMD: cp $TR_SRC/check_array.inc .; cp $TR_SRC/check_interface.inc .; rm -f fxstio104.dat
! %COMPOPTS:
! %GROUP:  fxstio104.f
! %VERIFY:
! %STDIN:
...
!*
!*  DESCRIPTION                : Test derived type (scalar and array) with
...
!**********************************************************
     ...
     OPEN(1, FILE='fxstio104.dat', FORM='FORMATTED', ACCESS='STREAM', &
    &     STATUS='NEW', IOSTAT=ios, ERR=90)

     WRITE(1, FMT=190, IOSTAT=ios, ERR=91) dt_in1
     WRITE(1, FMT=191, IOSTAT=ios, ERR=91) dt_in2
     WRITE(1, FMT=192, IOSTAT=ios, ERR=91) dt_in3

     REWIND(1, IOSTAT=ios, ERR=93)

     READ(1, FMT=190, IOSTAT=ios, ERR=92) dt_out1
     READ(1, FMT=191, IOSTAT=ios, ERR=92) dt_out2
     READ(1, FMT=192, IOSTAT=ios, ERR=92) dt_out3
