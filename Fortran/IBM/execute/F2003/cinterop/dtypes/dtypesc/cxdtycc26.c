...
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].var_a[i][j] != (double)(i*DIM2+j+2)+I*(double)(i*DIM2+j+2) ) exit(37);
#else
      if ( (dta+k)->vdt2[j][i].var_a[i][j] != createcomplex((double)(i*DIM2+j+2),(double)(i*DIM2+j+2)) ) exit(37);
#endif
      if ( (dta+k)->vdt2[j][i].var_b[i][j] != i*DIM2+j+3 ) exit(39);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].var_c[i][j] != (double)(i*DIM2+j+4)+I*(double)(i*DIM2+j+4) ) exit(41);
#else
      if ( (dta+k)->vdt2[j][i].var_c[i][j] != createcomplex((double)(i*DIM2+j+4),(double)(i*DIM2+j+4)) ) exit(41);
#endif
      if ( (dta+k)->vdt2[j][i].var_d[i][j] != i*DIM2+j+5 ) exit(43);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].var_e[i][j] != (long double)(i*DIM2+j+6)+I*(long double)(i*DIM2+j+6) ) exit(45);
#else
      if ( (dta+k)->vdt2[j][i].var_e[i][j] != createcomplexl((long double)(i*DIM2+j+6),(long double)(i*DIM2+j+6)) ) exit(45);
#endif
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].var_f[i][j] != (float)(i*DIM2+j+7)+I*(float)(i*DIM2+j+7) ) exit(47);
#else
      if ( (dta+k)->vdt2[j][i].var_f[i][j] != createcomplexf((float)(i*DIM2+j+7),(float)(i*DIM2+j+7)) ) exit(47);
#endif
      if ( (dta+k)->vdt2[j][i].var_g[i][j] != i*DIM2+j+8 ) exit(49);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].var_h[i][j] != (float)(i*DIM2+j+9)+I*(float)(i*DIM2+j+9) ) exit(51);
#else
      if ( (dta+k)->vdt2[j][i].var_h[i][j] != createcomplexf((float)(i*DIM2+j+9),(float)(i*DIM2+j+9)) ) exit(51);
#endif

      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_a[i][j] != i*DIM2+j+2 ) exit(53);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != (double)(i*DIM2+j+3)+I*(double)(i*DIM2+j+3) ) exit(55);
#else
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != createcomplex((double)(i*DIM2+j+3),(double)(i*DIM2+j+3)) ) exit(55);
#endif
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_c[i][j] != i*DIM2+j+4 ) exit(57);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != (float)(i*DIM2+j+5)+I*(float)(i*DIM2+j+5) ) exit(59);
#else
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != createcomplexf((float)(i*DIM2+j+5),(float)(i*DIM2+j+5)) ) exit(59);
#endif
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != (long double)(i*DIM2+j+6)+I*(long double)(i*DIM2+j+6) ) exit(61);
#else
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != createcomplexl((long double)(i*DIM2+j+6),(long double)(i*DIM2+j+6)) ) exit(61);
#endif
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != (double)(i*DIM2+j+7)+I*(double)(i*DIM2+j+7) ) exit(63);
#else
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != createcomplex((double)(i*DIM2+j+7),(double)(i*DIM2+j+7)) ) exit(63);
#endif
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_g[i][j] != i*DIM2+j+8 ) exit(65);
#ifdef CMPLX
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != (float)(i*DIM2+j+9)+I*(float)(i*DIM2+j+9) ) exit(67);
#else
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != createcomplexf((float)(i*DIM2+j+9),(float)(i*DIM2+j+9)) ) exit(67);
#endif
     }
    }
   }
...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
#ifdef CMPLX
      if ( (dtb+k)->var_a[i][j] != (float)(2*(i*DIM2+j+2)-1)+I*(float)(2*(i*DIM2+j+2)-1) ) exit(69);
#else
      if ( (dtb+k)->var_a[i][j] != createcomplexf((float)(2*(i*DIM2+j+2)-1),(float)(2*(i*DIM2+j+2)-1)) ) exit(69);
#endif
      if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(71);
#ifdef CMPLX
      if ( (dtb+k)->var_c[i][j] != (long double)(2*(i*DIM2+j+4)+1)+I*(long double)(2*(i*DIM2+j+4)+1) ) exit(73);
#else
      if ( (dtb+k)->var_c[i][j] != createcomplexl((long double)(2*(i*DIM2+j+4)+1),(long double)(2*(i*DIM2+j+4)+1)) ) exit(73);
#endif
#ifdef CMPLX
      if ( (dtb+k)->var_d[i][j] != (double)(2*(i*DIM2+j+5)+2)+I*(double)(2*(i*DIM2+j+5)+2) ) exit(75);
#else
      if ( (dtb+k)->var_d[i][j] != createcomplex((double)(2*(i*DIM2+j+5)+2),(double)(2*(i*DIM2+j+5)+2)) ) exit(75);
#endif
#ifdef CMPLX
      if ( (dtb+k)->var_e[i][j] != (float)(2*(i*DIM2+j+6)+3)+I*(float)(2*(i*DIM2+j+6)+3) ) exit(77);
#else
      if ( (dtb+k)->var_e[i][j] != createcomplexf((float)(2*(i*DIM2+j+6)+3),(float)(2*(i*DIM2+j+6)+3)) ) exit(77);
#endif
      if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(79);
#ifdef CMPLX
      if ( (dtb+k)->var_g[i][j] != (double)(2*(i*DIM2+j+8)+5)+I*(double)(2*(i*DIM2+j+8)+5) ) exit(81);
#else
      if ( (dtb+k)->var_g[i][j] != createcomplex((double)(2*(i*DIM2+j+8)+5),(double)(2*(i*DIM2+j+8)+5)) ) exit(81);
#endif
      if ( (dtb+k)->var_h[i][j] != 2*(i*DIM2+j+9)+6 ) exit(83);

#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].var_a[i][j] != (double)(2*(i*DIM2+j+2)-1)+I*(double)(2*(i*DIM2+j+2)-1) ) exit(85);
#else
      if ( (dtb+k)->vdt2[j][i].var_a[i][j] != createcomplex((double)(2*(i*DIM2+j+2)-1),(double)(2*(i*DIM2+j+2)-1)) ) exit(85);
#endif
      if ( (dtb+k)->vdt2[j][i].var_b[i][j] != 2*(i*DIM2+j+3) ) exit(87);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].var_c[i][j] != (double)(2*(i*DIM2+j+4)+1)+I*(double)(2*(i*DIM2+j+4)+1) ) exit(89);
#else
      if ( (dtb+k)->vdt2[j][i].var_c[i][j] != createcomplex((double)(2*(i*DIM2+j+4)+1),(double)(2*(i*DIM2+j+4)+1)) ) exit(89);
#endif
      if ( (dtb+k)->vdt2[j][i].var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(91);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+6)+3)+I*(long double)(2*(i*DIM2+j+6)+3) ) exit(93);
#else
      if ( (dtb+k)->vdt2[j][i].var_e[i][j] != createcomplexl((long double)(2*(i*DIM2+j+6)+3),(long double)(2*(i*DIM2+j+6)+3)) ) exit(93);
#endif
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].var_f[i][j] != (float)(2*(i*DIM2+j+7)+4)+I*(float)(2*(i*DIM2+j+7)+4) ) exit(95);
#else
      if ( (dtb+k)->vdt2[j][i].var_f[i][j] != createcomplexf((float)(2*(i*DIM2+j+7)+4),(float)(2*(i*DIM2+j+7)+4)) ) exit(95);
#endif
      if ( (dtb+k)->vdt2[j][i].var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(97);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+9)+6)+I*(float)(2*(i*DIM2+j+9)+6) ) exit(99);
#else
      if ( (dtb+k)->vdt2[j][i].var_h[i][j] != createcomplexf((float)(2*(i*DIM2+j+9)+6),(float)(2*(i*DIM2+j+9)+6)) ) exit(99);
#endif

      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(101);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != (double)(2*(i*DIM2+j+3))+I*(double)(2*(i*DIM2+j+3)) ) exit(103);
#else
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != createcomplex((double)(2*(i*DIM2+j+3)),(double)(2*(i*DIM2+j+3))) ) exit(103);
#endif
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(105);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != (float)(2*(i*DIM2+j+5)+2)+I*(float)(2*(i*DIM2+j+5)+2) ) exit(107);
#else
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != createcomplexf((float)(2*(i*DIM2+j+5)+2),(float)(2*(i*DIM2+j+5)+2)) ) exit(107);
#endif
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+6)+3)+I*(long double)(2*(i*DIM2+j+6)+3) ) exit(109);
#else
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != createcomplexl((long double)(2*(i*DIM2+j+6)+3),(long double)(2*(i*DIM2+j+6)+3)) ) exit(109);
#endif
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != (double)(2*(i*DIM2+j+7)+4)+I*(double)(2*(i*DIM2+j+7)+4) ) exit(111);
#else
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != createcomplex((double)(2*(i*DIM2+j+7)+4),(double)(2*(i*DIM2+j+7)+4)) ) exit(111);
#endif
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(113);
#ifdef CMPLX
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+9)+6)+I*(float)(2*(i*DIM2+j+9)+6) ) exit(115);
#else
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != createcomplexf((float)(2*(i*DIM2+j+9)+6),(float)(2*(i*DIM2+j+9)+6)) ) exit(115);
#endif
     }
    }
   }
