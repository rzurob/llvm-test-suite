...
#ifdef CMPLX
      if ( (dta+k)->vdt1[j][i].var_b[i][j] != (double)(i*DIM2+j+3)+I*(double)(i*DIM2+j+3) ) exit(39);
#else
      if ( (dta+k)->vdt1[j][i].var_b[i][j] != createcomplex((double)(i*DIM2+j+3),(double)(i*DIM2+j+3)) ) exit(39);
#endif
      if ( (dta+k)->vdt1[j][i].var_c[i][j] != i*DIM2+j+4 ) exit(41);
#ifdef CMPLX
      if ( (dta+k)->vdt1[j][i].var_d[i][j] != (float)(i*DIM2+j+5)+I*(float)(i*DIM2+j+5) ) exit(43);
#else
      if ( (dta+k)->vdt1[j][i].var_d[i][j] != createcomplexf((float)(i*DIM2+j+5),(float)(i*DIM2+j+5)) ) exit(43);
#endif
#ifdef CMPLX
      if ( (dta+k)->vdt1[j][i].var_e[i][j] != (long double)(i*DIM2+j+6)+I*(long double)(i*DIM2+j+6) ) exit(45);
#else
      if ( (dta+k)->vdt1[j][i].var_e[i][j] != createcomplexl((long double)(i*DIM2+j+6),(long double)(i*DIM2+j+6)) ) exit(45);
#endif
#ifdef CMPLX
      if ( (dta+k)->vdt1[j][i].var_f[i][j] != (double)(i*DIM2+j+7)+I*(double)(i*DIM2+j+7) ) exit(47);
#else
      if ( (dta+k)->vdt1[j][i].var_f[i][j] != createcomplex((double)(i*DIM2+j+7),(double)(i*DIM2+j+7)) ) exit(47);
#endif
      if ( (dta+k)->vdt1[j][i].var_g[i][j] != i*DIM2+j+8 ) exit(49);
#ifdef CMPLX
      if ( (dta+k)->vdt1[j][i].var_h[i][j] != (float)(i*DIM2+j+9)+I*(float)(i*DIM2+j+9) ) exit(51);
#else
      if ( (dta+k)->vdt1[j][i].var_h[i][j] != createcomplexf((float)(i*DIM2+j+9),(float)(i*DIM2+j+9)) ) exit(51);
#endif
     }
    }
   }
...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
#ifdef CMPLX
      if ( (dtb+k)->var_a[i][j] != (float)(2*(i*DIM2+j+2)-1)+I*(float)(2*(i*DIM2+j+2)-1) ) exit(53);
#else
      if ( (dtb+k)->var_a[i][j] != createcomplexf((float)(2*(i*DIM2+j+2)-1),(float)(2*(i*DIM2+j+2)-1)) ) exit(53);
#endif
      if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(55);
#ifdef CMPLX
      if ( (dtb+k)->var_c[i][j] != (long double)(2*(i*DIM2+j+4)+1)+I*(long double)(2*(i*DIM2+j+4)+1) ) exit(57);
#else
      if ( (dtb+k)->var_c[i][j] != createcomplexl((long double)(2*(i*DIM2+j+4)+1),(long double)(2*(i*DIM2+j+4)+1)) ) exit(57);
#endif
#ifdef CMPLX
      if ( (dtb+k)->var_d[i][j] != (double)(2*(i*DIM2+j+5)+2)+I*(double)(2*(i*DIM2+j+5)+2) ) exit(59);
#else
      if ( (dtb+k)->var_d[i][j] != createcomplex((double)(2*(i*DIM2+j+5)+2),(double)(2*(i*DIM2+j+5)+2)) ) exit(59);
#endif
#ifdef CMPLX
      if ( (dtb+k)->var_e[i][j] != (float)(2*(i*DIM2+j+6)+3)+I*(float)(2*(i*DIM2+j+6)+3) ) exit(61);
#else
      if ( (dtb+k)->var_e[i][j] != createcomplexf((float)(2*(i*DIM2+j+6)+3),(float)(2*(i*DIM2+j+6)+3)) ) exit(61);
#endif
      if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(63);
#ifdef CMPLX
      if ( (dtb+k)->var_g[i][j] != (double)(2*(i*DIM2+j+8)+5)+I*(double)(2*(i*DIM2+j+8)+5) ) exit(65);
#else
      if ( (dtb+k)->var_g[i][j] != createcomplex((double)(2*(i*DIM2+j+8)+5),(double)(2*(i*DIM2+j+8)+5)) ) exit(65);
#endif
      if ( (dtb+k)->var_h[i][j] != 2*(i*DIM2+j+9)+6 ) exit(67);

      if ( (dtb+k)->vdt1[j][i].var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(69);
#ifdef CMPLX
      if ( (dtb+k)->vdt1[j][i].var_b[i][j] != (double)(2*(i*DIM2+j+3))+I*(double)(2*(i*DIM2+j+3)) ) exit(71);
#else
      if ( (dtb+k)->vdt1[j][i].var_b[i][j] != createcomplex((double)(2*(i*DIM2+j+3)),(double)(2*(i*DIM2+j+3))) ) exit(71);
#endif
      if ( (dtb+k)->vdt1[j][i].var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(73);
#ifdef CMPLX
      if ( (dtb+k)->vdt1[j][i].var_d[i][j] != (float)(2*(i*DIM2+j+5)+2)+I*(float)(2*(i*DIM2+j+5)+2) ) exit(75);
#else
      if ( (dtb+k)->vdt1[j][i].var_d[i][j] != createcomplexf((float)(2*(i*DIM2+j+5)+2),(float)(2*(i*DIM2+j+5)+2)) ) exit(75);
#endif
#ifdef CMPLX
      if ( (dtb+k)->vdt1[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+6)+3)+I*(long double)(2*(i*DIM2+j+6)+3) ) exit(77);
#else
      if ( (dtb+k)->vdt1[j][i].var_e[i][j] != createcomplexl((long double)(2*(i*DIM2+j+6)+3),(long double)(2*(i*DIM2+j+6)+3)) ) exit(77);
#endif
#ifdef CMPLX
      if ( (dtb+k)->vdt1[j][i].var_f[i][j] != (double)(2*(i*DIM2+j+7)+4)+I*(double)(2*(i*DIM2+j+7)+4) ) exit(79);
#else
      if ( (dtb+k)->vdt1[j][i].var_f[i][j] != createcomplex((double)(2*(i*DIM2+j+7)+4),(double)(2*(i*DIM2+j+7)+4)) ) exit(79);
#endif
      if ( (dtb+k)->vdt1[j][i].var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(81);
#ifdef CMPLX
      if ( (dtb+k)->vdt1[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+9)+6)+I*(float)(2*(i*DIM2+j+9)+6) ) exit(83);
#else
      if ( (dtb+k)->vdt1[j][i].var_h[i][j] != createcomplexf((float)(2*(i*DIM2+j+9)+6),(float)(2*(i*DIM2+j+9)+6)) ) exit(83);
#endif
     }
    }
   }
