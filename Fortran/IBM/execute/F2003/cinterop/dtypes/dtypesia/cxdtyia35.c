...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
       if ( (dtb+k)->var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(85);
       if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(87);
       if ( (dtb+k)->var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(89);
       if ( (dtb+k)->var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(91);
       if ( (dtb+k)->var_e[i][j] != 2*(i*DIM2+j+6)+3 ) exit(93);
       if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(95);
       if ( (dtb+k)->var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(97);
       if ( (dtb+k)->var_h[i][j] != 2*(i*DIM2+j+9)+6 ) exit(99);

       if ( (dtr+k)->var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(101);
       if ( (dtr+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(103);
       if ( (dtr+k)->var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(105);
       if ( (dtr+k)->var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(107);
       if ( (dtr+k)->var_e[i][j] != 2*(i*DIM2+j+6)+3 ) exit(109);
       if ( (dtr+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(111);
       if ( (dtr+k)->var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(113);
