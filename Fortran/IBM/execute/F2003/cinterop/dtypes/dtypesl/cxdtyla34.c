   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
      ...
      if ( (dta+k)->vdt1[j][i].var_a[i][j] != 1 ) exit(37);
      if ( (dta+k)->vdt1[j][i].var_b[i][j] != i*DIM2+j+3 ) exit(39);
      if ( (dta+k)->vdt1[j][i].var_c[i][j] != 'B' ) exit(41);
      if ( (dta+k)->vdt1[j][i].var_d[i][j] != i*DIM2+j+5 ) exit(43);
      if ( (dta+k)->vdt1[j][i].var_e[i][j] != i*DIM2+j+6 ) exit(45);
      if ( (dta+k)->vdt1[j][i].var_f[i][j] != i*DIM2+j+7 ) exit(47);
      if ( (dta+k)->vdt1[j][i].var_g[i][j] != 1 ) exit(49);
      if ( (dta+k)->vdt1[j][i].var_h[i][j] != i*DIM2+j+9 ) exit(51);
     }
    }
   }
   free(dta);
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
      ...
      if ( (dtb+k)->vdt1[j][i].var_a[i][j] != 1 ) exit(69);
      if ( (dtb+k)->vdt1[j][i].var_b[i][j] != 2*(i*DIM2+j+3) ) exit(71);
      if ( (dtb+k)->vdt1[j][i].var_c[i][j] != 'B' ) exit(73);
      if ( (dtb+k)->vdt1[j][i].var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(75);
      if ( (dtb+k)->vdt1[j][i].var_e[i][j] != 2*(i*DIM2+j+6)+3 ) exit(77);
      if ( (dtb+k)->vdt1[j][i].var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(79);
      if ( (dtb+k)->vdt1[j][i].var_g[i][j] != 1 ) exit(81);
      if ( (dtb+k)->vdt1[j][i].var_h[i][j] != 2*(i*DIM2+j+9)+6 ) exit(83);
     }
    }
   }
