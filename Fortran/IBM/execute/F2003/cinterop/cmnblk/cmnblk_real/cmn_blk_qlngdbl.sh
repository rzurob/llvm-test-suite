...
function compile_C {
   ...
   echo ${KILL} xlc ${TR_SRC}/${2}.c -c -qnolm -qlanglvl=stdc99 ${OPTIONSc} ${align_C} ${DINT} ${PDF} $3
   ${KILL} xlc ${TR_SRC}/${2}.c  -c -qnolm -qlanglvl=stdc99 ${OPTIONSc} ${align_C} ${DINT} ${PDF} $3
   RC=$?
   if [[ ${RC} = 0 ]]; then
...
#----------------------------------------------------------------------------------------------
   ...
   RUN_PROG $1 $2 -qdebug=aixldbl128
   ...
   #-- 64 bit option for gcc is -m64 not -q64
   echo ${OPTIONS} | grep -e '-q64' > /dev/null
   RC=$?
   if [[ ${RC} -eq 0 ]]; then
      OPTIONSgcc=`echo ${OPTIONSgcc} | sed 's/\-q64/\-m64/g'`
   fi


   echo Compiling C file using GCC ...
   echo ${KILL} gcc ${TR_SRC}/$2.c -c -mlong-double-128 ${OPTIONSgcc} $DINT -qfloat=complexgcc
   ${KILL} gcc ${TR_SRC}/$2.c -c -mlong-double-128 ${OPTIONSgcc} $DINT -qfloat=complexgcc
   RC=$?
   if [[ ${RC} = 0 ]]; then
     echo Compilation successful for file ${2}.c
   else
