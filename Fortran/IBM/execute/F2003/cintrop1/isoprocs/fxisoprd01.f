...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisop00.presh fxisoprd01 cxisoprd01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,16) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,16) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,16) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,16) ) error stop 90
      end do
   end do

end program fxisoprd01
