...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopid00 cxisopid00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_LONG_LONG
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_LONG_LONG
...
program fxisopid00
   use ISO_C_BINDING, ONLY : C_PTR, C_LONG_LONG, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_LONG_LONG), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_LONG_LONG), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_LONG_LONG), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
