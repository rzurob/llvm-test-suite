...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopra01 cxisopra01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 1
   ...
   if ( pa%a /= 10.0e0 ) error stop 30

!! Test 2

   do i = 1, DIMB
      dtb%b(i) = real(i,C_FLOAT)
   end do
   pb => dtb
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_FLOAT) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_FLOAT) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_FLOAT) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_FLOAT) ) error stop 90
      end do
   end do

end program fxisopra01
