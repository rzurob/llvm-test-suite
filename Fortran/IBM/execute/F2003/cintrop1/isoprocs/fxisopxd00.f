...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoq00.presh fxisopxd00 cxisopxd00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisopxd00
   ...
   complex(16), target :: a, b(DIMB), c(DIMC,DIMC)
   complex(16), allocatable, target :: al, bl(:), cl(:,:)
   complex(16), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 1
   ...
   if ( pa /= cmplx(10.0q0,10.0q0) ) error stop 30
   ...
   if ( pa /= cmplx(10.0q0,10.0q0) ) error stop 42
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,16) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,16) ) error stop 66
      end do
   end do
...
!! Test 4
   ...
   if ( pa /= cmplx(10.0q0,10.0q0) ) error stop 78
   ...
   if ( pa /= cmplx(10.0q0,10.0q0) ) error stop 90
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,16) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,16) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopxd00
