...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopis00 cxisopis00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_SIZE_T
...
program fxisopis00
   use ISO_C_BINDING, ONLY : C_PTR, C_SIZE_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_SIZE_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_SIZE_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_SIZE_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
