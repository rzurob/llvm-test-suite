...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopir00 cxisopir00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INTPTR_T
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_INTPTR_T
...
program fxisopir00
   use ISO_C_BINDING, ONLY : C_PTR, C_INTPTR_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INTPTR_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INTPTR_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INTPTR_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
