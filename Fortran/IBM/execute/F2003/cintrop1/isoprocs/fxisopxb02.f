...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrison00.presh fxisopxb02 cxisopxb02
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 2
   ...
   do i = 1, DIMT
      do j = 1, DIMB
         dtb(i)%b(j) = cmplx(j,j,C_DOUBLE_COMPLEX)
      end do
   end do
   pb => dtb
   ...
   do i = 1, DIMT
      do j = 1, DIMB
         if ( pb(i)%b(j) /= cmplx(j+1,j+1,C_DOUBLE_COMPLEX) ) error stop 42
      end do
   end do
...
!! Test 3
   ...
   do i = 1, DIMT
      do j = 1, DIMT
         do k = 1, DIMC
            do l = 1, DIMC
               dtc(i,j)%c(k,l) = cmplx(k+l,k+l,C_DOUBLE_COMPLEX)
            end do
         end do
      end do
   end do
   ...
   do i = 1, DIMT
      do j = 1, DIMT
         do k = 1, DIMC
            do l = 1, DIMC
               if ( pc(i,j)%c(k,l) /= cmplx(k+l+1,k+l+1,C_DOUBLE_COMPLEX) ) error stop 54
            end do
         end do
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMT
      do j = 1, DIMB
         dtbl(i)%b(j) = cmplx(j,j,C_DOUBLE_COMPLEX)
      end do
   end do
   pb => dtbl
...
!! Test 6
   ...
   do i = 1, DIMT
      do j = 1, DIMT
         do k = 1, DIMC
            do l = 1, DIMC
               dtcl(i,j)%c(k,l) = cmplx(k+l,k+l,C_DOUBLE_COMPLEX)
            end do
         end do
      end do
   end do
