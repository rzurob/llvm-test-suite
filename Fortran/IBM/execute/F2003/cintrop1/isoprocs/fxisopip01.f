...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopip01 cxisopip01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisopip01
   ...
   type, bind(c) :: dts0
      integer(C_INT_FAST64_T) :: a
   end type

   type, bind(c) :: dts1
      integer(C_INT_FAST64_T) :: b(DIMB)
   end type

   type, bind(c) :: dts2
      integer(C_INT_FAST64_T) :: c(DIMC,DIMC)
   end type
