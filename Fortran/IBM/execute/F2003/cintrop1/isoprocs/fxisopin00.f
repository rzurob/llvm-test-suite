...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoi00.presh fxisopin00 cxisopin00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisopin00
   ...
   integer(C_INT_FAST16_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT_FAST16_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT_FAST16_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
