...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopig00 cxisopig00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_INT32_T
...
program fxisopig00
   use ISO_C_BINDING, ONLY : C_PTR, C_INT32_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INT32_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT32_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT32_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
