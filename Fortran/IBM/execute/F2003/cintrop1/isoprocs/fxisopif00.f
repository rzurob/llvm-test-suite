...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopif00 cxisopif00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_INT16_T
...
program fxisopif00
   use ISO_C_BINDING, ONLY : C_PTR, C_INT16_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INT16_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT16_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT16_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
