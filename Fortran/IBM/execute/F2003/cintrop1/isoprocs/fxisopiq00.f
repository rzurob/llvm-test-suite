...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopiq00 cxisopiq00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INTMAX_T
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_INTMAX_T
...
program fxisopiq00
   use ISO_C_BINDING, ONLY : C_PTR, C_INTMAX_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INTMAX_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INTMAX_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INTMAX_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
