...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisoprb00 cxisoprb00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisoprb00
   ...
   real(C_DOUBLE), target :: a, b(DIMB), c(DIMC,DIMC)
   real(C_DOUBLE), allocatable, target :: al, bl(:), cl(:,:)
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,C_DOUBLE) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,C_DOUBLE) ) error stop 66
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,C_DOUBLE) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,C_DOUBLE) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisoprb00
