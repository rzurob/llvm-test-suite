...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopib00 cxisopib00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_INT
...
program fxisopib00
   ...
   integer(C_INT), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
