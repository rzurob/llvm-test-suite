...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopra00 cxisopra00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_FLOAT
...
program fxisopra00
   use ISO_C_BINDING, ONLY : C_PTR, C_FLOAT, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   real(C_FLOAT), target :: a, b(DIMB), c(DIMC,DIMC)
   real(C_FLOAT), allocatable, target :: al, bl(:), cl(:,:)
   real(C_FLOAT), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,C_FLOAT) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,C_FLOAT) ) error stop 66
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,C_FLOAT) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,C_FLOAT) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopra00
