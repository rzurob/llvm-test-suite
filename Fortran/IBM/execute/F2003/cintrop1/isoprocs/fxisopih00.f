...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopih00 cxisopih00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
program fxisopih00
   ...
   integer(C_INT64_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT64_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT64_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
