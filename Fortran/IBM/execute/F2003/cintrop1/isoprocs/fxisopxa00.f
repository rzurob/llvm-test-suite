...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrison00.presh fxisopxa00 cxisopxa00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_FLOAT_COMPLEX
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_FLOAT_COMPLEX
...
program fxisopxa00
   use ISO_C_BINDING, ONLY : C_PTR, C_FLOAT_COMPLEX, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   complex(C_FLOAT_COMPLEX), target :: a, b(DIMB), c(DIMC,DIMC)
   complex(C_FLOAT_COMPLEX), allocatable, target :: al, bl(:), cl(:,:)
   complex(C_FLOAT_COMPLEX), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 1
   ...
   if ( pa /= cmplx(10.0e0,10.0e0) ) error stop 30
   ...
   if ( pa /= cmplx(10.0e0,10.0e0) ) error stop 42

!! Test 2

   do i = 1, DIMB
      b(i) = cmplx(i,i,C_FLOAT_COMPLEX)
   end do
   pb => b
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,C_FLOAT_COMPLEX) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         c(i,j) = cmplx(i+j,i+j,C_FLOAT_COMPLEX)
      end do
   end do
   pc => c
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,C_FLOAT_COMPLEX) ) error stop 66
      end do
   end do
...
!! Test 4
   ...
   if ( pa /= cmplx(10.0e0,10.0e0) ) error stop 78
   ...
   if ( pa /= cmplx(10.0e0,10.0e0) ) error stop 90
...
!! Test 5
   ...
   do i = 1, DIMB
      bl(i) = cmplx(i,i,C_FLOAT_COMPLEX)
   end do
   pb => bl
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,C_FLOAT_COMPLEX) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         cl(i,j) = cmplx(i+j,i+j,C_FLOAT_COMPLEX)
      end do
   end do
   pc => cl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,C_FLOAT_COMPLEX) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopxa00
