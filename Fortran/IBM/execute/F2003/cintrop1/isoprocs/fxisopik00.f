...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopik00 cxisopik00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INT_LEAST32_T
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_INT_LEAST32_T
...
program fxisopik00
   use ISO_C_BINDING, ONLY : C_PTR, C_INT_LEAST32_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INT_LEAST32_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT_LEAST32_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT_LEAST32_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
