...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrison00.presh fxisopxc01 cxisopxc01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 2
   ...
   do i = 1, DIMB
      dtb%b(i) = cmplx(i,i,C_LONG_DOUBLE_COMPLEX)
   end do
   pb => dtb
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,C_LONG_DOUBLE_COMPLEX) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtc%c(i,j) = cmplx(i+j,i+j,C_LONG_DOUBLE_COMPLEX)
      end do
   end do
   pc => dtc
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,C_LONG_DOUBLE_COMPLEX) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      dtbl%b(i) = cmplx(i,i,C_LONG_DOUBLE_COMPLEX)
   end do
   pb => dtbl
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,C_LONG_DOUBLE_COMPLEX) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtcl%c(i,j) = cmplx(i+j,i+j,C_LONG_DOUBLE_COMPLEX)
      end do
   end do
   pc => dtcl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,C_LONG_DOUBLE_COMPLEX) ) error stop 90
      end do
   end do

end program fxisopxc01
