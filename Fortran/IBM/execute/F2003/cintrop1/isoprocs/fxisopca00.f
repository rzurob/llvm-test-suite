...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopca00 cxisopca00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisopca00
   ...
   character(C_CHAR), target :: a, b(DIMB), c(DIMC,DIMC)
   character(C_CHAR), allocatable, target :: al, bl(:), cl(:,:)
   character(C_CHAR), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) /= achar(iachar('A')+i+3) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= achar(iachar('J')+i+j-2) ) error stop 66
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= achar(iachar('A')+i+3) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= achar(iachar('J')+i+j-2) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopca00
