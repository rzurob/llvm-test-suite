...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopca01 cxisopca01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= achar(iachar('A')+i+3) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtc%c(i,j) = achar(iachar('A')+i+j-2)
      end do
   end do
   pc => dtc
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= achar(iachar('J')+i+j-2) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= achar(iachar('A')+i+3) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtcl%c(i,j) = achar(iachar('A')+i+j-2)
      end do
   end do
   pc => dtcl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= achar(iachar('J')+i+j-2) ) error stop 90
      end do
   end do
