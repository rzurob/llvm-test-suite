...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisoprc01 cxisoprc01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_LONG_DOUBLE) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtc%c(i,j) = real(i+j,C_LONG_DOUBLE)
      end do
   end do
   pc => dtc
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_LONG_DOUBLE) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_LONG_DOUBLE) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtcl%c(i,j) = real(i+j,C_LONG_DOUBLE)
      end do
   end do
   pc => dtcl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_LONG_DOUBLE) ) error stop 90
      end do
   end do

end program fxisoprc01
