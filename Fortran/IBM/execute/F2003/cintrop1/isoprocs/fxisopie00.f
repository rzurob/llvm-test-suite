...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopie00 cxisopie00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_INT8_T
...
program fxisopie00
   use ISO_C_BINDING, ONLY : C_PTR, C_INT8_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INT8_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT8_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT8_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
