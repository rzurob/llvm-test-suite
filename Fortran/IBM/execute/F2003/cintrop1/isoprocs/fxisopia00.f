...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopia00 cxisopia00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_SHORT
...
program fxisopia00
   use ISO_C_BINDING, ONLY : C_PTR, C_SHORT, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_SHORT), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_SHORT), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_SHORT), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
