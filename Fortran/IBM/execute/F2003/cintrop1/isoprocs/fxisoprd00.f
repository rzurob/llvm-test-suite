...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisop00.presh fxisoprd00 cxisoprd00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisoprd00
   ...
   real(16), target :: a, b(DIMB), c(DIMC,DIMC)
   real(16), allocatable, target :: al, bl(:), cl(:,:)
   real(16), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,16) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,16) ) error stop 66
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= real(i+1,16) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= real(i+j+1,16) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisoprd00
