...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisoprb01 cxisoprb01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_DOUBLE
...
program fxisoprb01
   use ISO_C_BINDING, ONLY : C_PTR, C_DOUBLE, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   type, bind(c) :: dts1
      real(C_DOUBLE) :: b(DIMB)
   end type

   type, bind(c) :: dts2
      real(C_DOUBLE) :: c(DIMC,DIMC)
   end type
...
!! Test 1
   ...
   if ( pa%a /= 10.0d0 ) error stop 30

!! Test 2

   do i = 1, DIMB
      dtb%b(i) = real(i,C_DOUBLE)
   end do
   pb => dtb
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_DOUBLE) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_DOUBLE) ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= real(i+1,C_DOUBLE) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= real(i+j+1,C_DOUBLE) ) error stop 90
      end do
   end do

end program fxisoprb01
