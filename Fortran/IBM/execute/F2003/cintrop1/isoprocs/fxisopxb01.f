...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrison00.presh fxisopxb01 cxisopxb01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
program fxisopxb01
   ...
   type, bind(c) :: dts1
      complex(C_DOUBLE_COMPLEX) :: b(DIMB)
   end type

   type, bind(c) :: dts2
      complex(C_DOUBLE_COMPLEX) :: c(DIMC,DIMC)
   end type
...
!! Test 1
   ...
   if ( pa%a /= cmplx(10.0d0,10.0d0) ) error stop 30

!! Test 2

   do i = 1, DIMB
      dtb%b(i) = cmplx(i,i,C_DOUBLE_COMPLEX)
   end do
   pb => dtb
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,C_DOUBLE_COMPLEX) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtc%c(i,j) = cmplx(i+j,i+j,C_DOUBLE_COMPLEX)
      end do
   end do
   pc => dtc
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,C_DOUBLE_COMPLEX) ) error stop 54
      end do
   end do
...
!! Test 4
   ...
   if ( pa%a /= cmplx(10.0d0,10.0d0) ) error stop 66
...
!! Test 5
   ...
   do i = 1, DIMB
      dtbl%b(i) = cmplx(i,i,C_DOUBLE_COMPLEX)
   end do
   pb => dtbl
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,C_DOUBLE_COMPLEX) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         dtcl%c(i,j) = cmplx(i+j,i+j,C_DOUBLE_COMPLEX)
      end do
   end do
   pc => dtcl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,C_DOUBLE_COMPLEX) ) error stop 90
      end do
   end do

end program fxisopxb01
