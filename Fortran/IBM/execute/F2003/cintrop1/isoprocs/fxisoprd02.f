...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisop00.presh fxisoprd02 cxisoprd02
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 1
   ...
   do i = 1, DIMT
      if ( pa(i)%a /= 10.0q0 ) error stop 30
   end do
...
!! Test 2
   ...
   do i = 1, DIMT
      do j = 1, DIMB
         if ( pb(i)%b(j) /= real(j+1,16) ) error stop 42
      end do
   end do
...
!! Test 3
   ...
   do i = 1, DIMT
      do j = 1, DIMT
         do k = 1, DIMC
            do l = 1, DIMC
               if ( pc(i,j)%c(k,l) /= real(k+l+1,16) ) error stop 54
            end do
         end do
      end do
   end do
