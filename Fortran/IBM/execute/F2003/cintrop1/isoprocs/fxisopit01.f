...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopit01 cxisopit01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
program fxisopit01
   ...
   type, bind(c) :: dts1
      integer(C_SIGNED_CHAR) :: b(DIMB)
   end type

   type, bind(c) :: dts2
      integer(C_SIGNED_CHAR) :: c(DIMC,DIMC)
   end type
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= iachar('A')+i+3 ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= iachar('J')+i+j-2 ) error stop 54
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= iachar('A')+i+3 ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= iachar('J')+i+j-2 ) error stop 90
      end do
   end do

end program fxisopit01
