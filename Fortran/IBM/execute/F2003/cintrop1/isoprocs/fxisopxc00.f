...
program fxisopxc00
   ...
   complex(C_LONG_DOUBLE_COMPLEX), target :: a, b(DIMB), c(DIMC,DIMC)
...
!! Test 2
   ...
   do i = 1, DIMB
      b(i) = cmplx(i,i,C_LONG_DOUBLE_COMPLEX)
   end do
   pb => b
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,C_LONG_DOUBLE_COMPLEX) ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         c(i,j) = cmplx(i+j,i+j,C_LONG_DOUBLE_COMPLEX)
      end do
   end do
   pc => c
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,C_LONG_DOUBLE_COMPLEX) ) error stop 66
      end do
   end do
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) /= cmplx(i+1,i+1,C_LONG_DOUBLE_COMPLEX) ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         cl(i,j) = cmplx(i+j,i+j,C_LONG_DOUBLE_COMPLEX)
      end do
   end do
   pc => cl
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) /= cmplx(i+j+1,i+j+1,C_LONG_DOUBLE_COMPLEX) ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopxc00
