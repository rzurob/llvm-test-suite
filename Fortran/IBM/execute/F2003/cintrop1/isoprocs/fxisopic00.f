...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopic00 cxisopic00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*  DESCRIPTION                : 
!*
!*	- testing interoperable kind type parameter C_LONG
...
program fxisopic00
   use ISO_C_BINDING, ONLY : C_PTR, C_LONG, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_LONG), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_LONG), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_LONG), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
