...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa00.presh fxisopil00 cxisopil00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*
!*  KEYWORD(S)                 : C_INT_LEAST64_T
!*  TARGET(S)                  :
!*  NUMBER OF TESTS CONDITIONS :
!*
!*  DESCRIPTION                :
!*
!*	- testing interoperable kind type parameter C_INT_LEAST64_T
...
program fxisopil00
   use ISO_C_BINDING, ONLY : C_PTR, C_INT_LEAST64_T, C_LOC, C_ASSOCIATED, C_F_POINTER
   ...
   integer(C_INT_LEAST64_T), target :: a, b(DIMB), c(DIMC,DIMC)
   integer(C_INT_LEAST64_T), allocatable, target :: al, bl(:), cl(:,:)
   integer(C_INT_LEAST64_T), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
