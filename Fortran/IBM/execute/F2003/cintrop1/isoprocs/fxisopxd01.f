...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoq00.presh fxisopxd01 cxisopxd01
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 1
   ...
   if ( pa%a /= cmplx(10.0q0,10.0q0) ) error stop 30

!! Test 2

   do i = 1, DIMB
      dtb%b(i) = cmplx(i,i,16)
   end do
   pb => dtb
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,16) ) error stop 42
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,16) ) error stop 54
      end do
   end do
...
!! Test 4
   ...
   if ( pa%a /= cmplx(10.0q0,10.0q0) ) error stop 66
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb%b(i) /= cmplx(i+1,i+1,16) ) error stop 78
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc%c(i,j) /= cmplx(i+j+1,i+j+1,16) ) error stop 90
      end do
   end do

end program fxisopxd01
