...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoo00.presh fxisopla00 cxisopla00
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
program fxisopla00
   ...
   logical(C_BOOL), target :: a, b(DIMB), c(DIMC,DIMC)
   logical(C_BOOL), allocatable, target :: al, bl(:), cl(:,:)
   logical(C_BOOL), pointer :: pa, pb(:), pc(:,:)
   type(C_PTR) :: cp
...
!! Test 1
   ...
   if ( pa .neqv. .false. ) error stop 30
   ...
   if ( pa .neqv. .false. ) error stop 42
...
!! Test 2
   ...
   do i = 1, DIMB
      if ( pb(i) .neqv. .false. ) error stop 54
   end do
...
!! Test 3
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) .neqv. .false. ) error stop 66
      end do
   end do
...
!! Test 4
   ...
   if ( pa .neqv. .false. ) error stop 78
   ...
   if ( pa .neqv. .false. ) error stop 90
...
!! Test 5
   ...
   do i = 1, DIMB
      if ( pb(i) .neqv. .false. ) error stop 102
   end do
...
!! Test 6
   ...
   do i = 1, DIMC
      do j = 1, DIMC
         if ( pc(i,j) .neqv. .false. ) error stop 114
      end do
   end do

   deallocate(cl)

end program fxisopla00
