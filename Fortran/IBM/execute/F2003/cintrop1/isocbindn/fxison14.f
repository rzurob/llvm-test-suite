...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrison00.presh fxison14 cxison14
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 8
   ...
   if ( dta%a /= (5.0e0,5.0e0) ) error stop 76
   if ( dta%b /= (10.0d0,10.0d0) ) error stop 78
...
!! Test 8a
   ...
   if ( dta%a /= (5.0e0,5.0e0) ) error stop 80
   if ( dta%b /= (10.0d0,10.0d0) ) error stop 82
...
!! Test 9a
   ...
   if ( dtb%a /= (5.0e0,5.0e0) ) error stop 92
   if ( dtb%b /= (10.0d0,10.0d0) ) error stop 94
   if ( dtb%d0%a /= (5.0e0,5.0e0) ) error stop 96
   if ( dtb%d0%b /= (10.0d0,10.0d0) ) error stop 98
...
!! Test 10
   ...
   if ( dtb%a /= (5.0e0,5.0e0) ) error stop 100
   if ( dtb%b /= (10.0d0,10.0d0) ) error stop 102
   if ( dtb%d0%a /= (5.0e0,5.0e0) ) error stop 104
   if ( dtb%d0%b /= (10.0d0,10.0d0) ) error stop 106
...
!! Test 10a
   ...
   if ( dtb%a /= (5.0e0,5.0e0) ) error stop 108
   if ( dtb%b /= (10.0d0,10.0d0) ) error stop 110
   if ( dtb%d0%a /= (5.0e0,5.0e0) ) error stop 112
   if ( dtb%d0%b /= (10.0d0,10.0d0) ) error stop 114
...
!! Test 11
   ...
   if ( dtc%a /= (5.0e0,5.0e0) ) error stop 116
   if ( dtc%b /= (10.0d0,10.0d0) ) error stop 118
   if ( dtc%d1%a /= (5.0e0,5.0e0) ) error stop 120
   if ( dtc%d1%b /= (10.0d0,10.0d0) ) error stop 122
   if ( dtc%d1%d0%a /= (5.0e0,5.0e0) ) error stop 124
   if ( dtc%d1%d0%b /= (10.0d0,10.0d0) ) error stop 126
...
!! Test 11a
   ...
   if ( dtc%a /= (5.0e0,5.0e0) ) error stop 128
   if ( dtc%b /= (10.0d0,10.0d0) ) error stop 130
   if ( dtc%d1%a /= (5.0e0,5.0e0) ) error stop 132
   if ( dtc%d1%b /= (10.0d0,10.0d0) ) error stop 134
   if ( dtc%d1%d0%a /= (5.0e0,5.0e0) ) error stop 136
   if ( dtc%d1%d0%b /= (10.0d0,10.0d0) ) error stop 138
...
!! Test 12
   ...
   if ( dtc%a /= (5.0e0,5.0e0) ) error stop 140
   if ( dtc%b /= (10.0d0,10.0d0) ) error stop 142
   if ( dtc%d1%a /= (5.0e0,5.0e0) ) error stop 144
   if ( dtc%d1%b /= (10.0d0,10.0d0) ) error stop 146
   if ( dtc%d1%d0%a /= (5.0e0,5.0e0) ) error stop 148
   if ( dtc%d1%d0%b /= (10.0d0,10.0d0) ) error stop 150
...
!! Test 12a
   ...
   if ( dtc%a /= (5.0e0,5.0e0) ) error stop 152
   if ( dtc%b /= (10.0d0,10.0d0) ) error stop 154
   if ( dtc%d1%a /= (5.0e0,5.0e0) ) error stop 156
   if ( dtc%d1%b /= (10.0d0,10.0d0) ) error stop 158
   if ( dtc%d1%d0%a /= (5.0e0,5.0e0) ) error stop 160
   if ( dtc%d1%d0%b /= (10.0d0,10.0d0) ) error stop 162
...
!! Test 13
   ...
   if ( dta%a /= (10.0e0,10.0e0) ) error stop 164
   if ( dta%b /= (20.0d0,20.0d0) ) error stop 166
...
!! Test 14
   ...
   if ( dtb%a /= (10.0e0,10.0e0) ) error stop 168
   if ( dtb%b /= (20.0d0,20.0d0) ) error stop 170
   if ( dtb%d0%a /= (10.0e0,10.0e0) ) error stop 172
   if ( dtb%d0%b /= (20.0d0,20.0d0) ) error stop 174
...
!! Test 15
   ...
   if ( dtc%a /= (10.0e0,10.0e0) ) error stop 176
   if ( dtc%b /= (20.0d0,20.0d0) ) error stop 178
   if ( dtc%d1%a /= (10.0e0,10.0e0) ) error stop 180
   if ( dtc%d1%b /= (20.0d0,20.0d0) ) error stop 182
   if ( dtc%d1%d0%a /= (10.0e0,10.0e0) ) error stop 184
   if ( dtc%d1%d0%b /= (20.0d0,20.0d0) ) error stop 186
