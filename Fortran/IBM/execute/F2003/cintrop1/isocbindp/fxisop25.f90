...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa01.presh fxisop25
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 3
   ...
   do i = 1, 5
      if ( a(i) /= real(i+1,16) ) error stop 24
   end do
...
!! Test 5
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j,16) ) error stop 28
      end do
   end do
...
!! Test 7
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j,16) ) error stop 32
      end do
   end do
...
!! Test 8
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j,16) ) error stop 34
      end do
   end do

end program fxisop25
...
real(16) function fnt1(a)
   ...
   do i = 1, 5
      if ( a(i) /= real(i,16) ) error stop 36
      a(i) = real(i+1,16)
   end do
...
real(16) function fnt2(a)
   ...
   do i = 1, 5
      if ( a(i) /= real(i,16) ) error stop 38
   end do
...
real(16) function fnt3(a)
   ...
   do i = 1, 5
      if ( a(i) /= real(i,16) ) error stop 40
      a(i) = real(i+1,16)
   end do
...
real(16) function fnt5(aa)
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j-1,16) ) error stop 42
         aa(j,i) = real(i+j,16)
      end do
   end do
...
real(16) function fnt6(aa)
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j-1,16) ) error stop 44
      end do
   end do
...
real(16) function fnt7(aa)
   ...
   do i = 1, 5
      do j = 1, 10
         if ( aa(j,i) /= real(i+j-1,16) ) error stop 46
         aa(j,i) = real(i+j,16)
      end do
   end do
