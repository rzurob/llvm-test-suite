...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrisoa01.presh fxisop27a
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 9
   ...
   do i = 1, 5
      if ( dtb%a(i) /= real(i,16) ) error stop 48
      if ( dtb%d0%a(i) /= real(i,16) ) error stop 50
   end do
...
!! Test 11
   ...
   do i = 1, 5
      if ( dtc%a(i) /= real(i,16) ) error stop 56
      if ( dtc%d1%a(i) /= real(i,16) ) error stop 58
      if ( dtc%d1%d0%a(i) /= real(i,16) ) error stop 60
   end do
...
!! Test 12
   ...
   do i = 1, 5
      if ( dtc%a(i) /= real(i,16) ) error stop 62
      if ( dtc%d1%a(i) /= real(i,16) ) error stop 64
      if ( dtc%d1%d0%a(i) /= real(i,16) ) error stop 66
   end do
...
!! Test 13
   ...
   do i = 1, 5
      if ( dta%a(i) /= real(i*2,16) ) error stop 68
   end do
...
!! Test 14
   ...
   do i = 1, 5
      if ( dtb%a(i) /= real(i*2,16) ) error stop 70
      if ( dtb%d0%a(i) /= real(i*2,16) ) error stop 72
   end do
...
!! Test 15
   ...
   do i = 1, 5
      if ( dtc%a(i) /= real(i*2,16) ) error stop 74
      if ( dtc%d1%a(i) /= real(i*2,16) ) error stop 76
      if ( dtc%d1%d0%a(i) /= real(i*2,16) ) error stop 78
   end do


end program fxisop27a
...
real(16) function fnt1(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 80
      dt%a(i) = real(i+1,16)
   end do
...
real(16) function fnt2(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 82
      dt%a(i) = real(i+1,16)
   end do
...
real(16) function fnt3(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 84
      dt%a(i) = real(dt%a(i) + i,16)
      if ( dt%d0%a(i) /= real(i,16) ) error stop 86
      dt%d0%a(i) = real(dt%d0%a(i) + i,16)
   end do
...
real(16) function fnt4(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 88
      dt%a(i) = real(dt%a(i) + i,16)
      if ( dt%d0%a(i) /= real(i,16) ) error stop 90
      dt%d0%a(i) = real(dt%d0%a(i) + i,16)
   end do
...
real(16) function fnt5(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 92
      dt%a(i) = real(dt%a(i) + i,16)
      if ( dt%d1%a(i) /= real(i,16) ) error stop 94
      dt%d1%a(i) = real(dt%d1%a(i) + i,16)
      if ( dt%d1%d0%a(i) /= real(i,16) ) error stop 96
      dt%d1%d0%a(i) = real(dt%d1%d0%a(i) + i,16)
   end do
...
real(16) function fnt6(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 98
      dt%a(i) = real(dt%a(i) + i,16)
      if ( dt%d1%a(i) /= real(i,16) ) error stop 100
      dt%d1%a(i) = real(dt%d1%a(i) + i,16)
      if ( dt%d1%d0%a(i) /= real(i,16) ) error stop 102
      dt%d1%d0%a(i) = real(dt%d1%d0%a(i) + i,16)
   end do
...
real(16) function fnt7(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 104
   end do
...
real(16) function fnt8(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 106
   end do
...
real(16) function fnt9(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 108
      if ( dt%d0%a(i) /= real(i,16) ) error stop 110
   end do
...
real(16) function fnt10(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 112
      if ( dt%d0%a(i) /= real(i,16) ) error stop 114
   end do
...
real(16) function fnt11(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 116
      if ( dt%d1%a(i) /= real(i,16) ) error stop 118
      if ( dt%d1%d0%a(i) /= real(i,16) ) error stop 120
   end do
...
real(16) function fnt12(dt) bind(c)
   ...
   do i = 1, 5
      if ( dt%a(i) /= real(i,16) ) error stop 122
      if ( dt%d1%a(i) /= real(i,16) ) error stop 124
      if ( dt%d1%d0%a(i) /= real(i,16) ) error stop 126
   end do
