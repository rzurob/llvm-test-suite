...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  program main
      ...
      if ( any(ubound(l_p) .ne. (/2,3,1/))) stop 8
      ...
      if ( .not. associated(l_p)) stop 12
      if ( any(lbound(l_p) .ne. (/1,1,1/))) stop 15
      if ( any(ubound(l_p) .ne. (/0,0,0/))) stop 18

      select type (l_p)
	   type is (logical*8)
		! if mask is zero-size array, count returns zero
		if  ( count(l_p) /= 0 ) stop 20
	   class default
		stop 22
      end select
