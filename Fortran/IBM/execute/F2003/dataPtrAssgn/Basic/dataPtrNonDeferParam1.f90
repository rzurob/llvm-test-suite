...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  PtrReal8(0:, 0:) => Real8
  IF (ASSOCIATED(PtrReal8)) ERROR STOP  11

  ALLOCATE(Real8(1025, 1))
  PtrReal8(0:1024, 0:-1) => Real8(1, 1:0)
  IF (.NOT.ASSOCIATED(PtrReal8)) ERROR STOP  12
  !DEALLOCATE(PtrReal8)

  ALLOCATE(PtrC4(1,1))
  PtrC4(0:, 0:) => PtrC4Tar
  IF (ASSOCIATED(PtrC4)) ERROR STOP  13
  !DEALLOCATE(PtrC4)

  ALLOCATE(PtrC4(1,1))
  PtrC4(0:1, 1:0) => PtrC4Tar(1, 1:0)
  IF (ASSOCIATED(PtrC4)) ERROR STOP  14
  !DEALLOCATE(PtrC4)

  ALLOCATE(PtrC3(1,1), SOURCE="123")
  PtrC3(0:, 0:) => PtrC3Tar
  IF (ASSOCIATED(PtrC3)) ERROR STOP  23
  !DEALLOCATE(PtrC3)

  ALLOCATE(CHARACTER(3)::PtrC3Tar(1,1))
  PtrC3(0:1, 1:0) => PtrC3Tar(1, 1:0)
  IF (.NOT.ASSOCIATED(PtrC3)) ERROR STOP  24
  !DEALLOCATE(PtrC3)

  ALLOCATE(PtrC2(1,1))
  PtrC2(0:, 0:) => PtrC2Tar
  IF (ASSOCIATED(PtrC2)) ERROR STOP  15
  !DEALLOCATE(PtrC2)

  ALLOCATE(PtrC2Tar(1,1))
  PtrC2(0:1, 1:0) => PtrC2Tar(1, 1:0)
  IF (.NOT. ASSOCIATED(PtrC2)) ERROR STOP  16
  !DEALLOCATE(PtrC2)

  ALLOCATE(PtrC1(1,1), SOURCE="1")
  PtrC1(0:, 0:) => PtrC1Tar
  IF (ASSOCIATED(PtrC1)) ERROR STOP  17
  !DEALLOCATE(PtrC1)

  ALLOCATE(CHARACTER(1)::PtrC1Tar(1,1))
  PtrC1(0:1, 1:0) => PtrC1Tar(1, 1:0)
  IF (.NOT. ASSOCIATED(PtrC1)) ERROR STOP  18
  !DEALLOCATE(PtrC1)
