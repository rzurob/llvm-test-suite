...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  IF (ANY( LBOUND(P)         .NE. (/0, 0 /)))    ERROR STOP 22
  IF (ANY( UBOUND(P)         .NE. (/0, 0 /)))    ERROR STOP 23
  IF (ANY( P(0,0)%PtrC16Tar  .NE. (16.0_16, -16.0_16)))     ERROR STOP 24
  IF (ANY( P(0,0)%PtrL8Tar   .NEQV. .TRUE._8))   ERROR STOP 25


  PtrI8(10:10, 1:9 ) => T(1,1)%PtrI8Tar(:, 10)
  IF (.NOT. ASSOCIATED(PtrI8))                    ERROR STOP 41
  IF (ANY( LBOUND(PtrI8) .NE. (/10, 1 /)))        ERROR STOP 42
  IF (ANY( UBOUND(PtrI8) .NE. (/10, 9 /)))        ERROR STOP 43
  IF (ANY( PtrI8         .NE. -8_8))              ERROR STOP 44

  PtrR16(10:19, 9:9 ) => T(2,2)%PtrR16Tar(:, 10)
  IF (.NOT. ASSOCIATED(PtrR16))                   ERROR STOP 71
  IF (ANY( LBOUND(PtrR16) .NE. (/10, 9 /)))       ERROR STOP 72
  ...
  IF (ANY( LBOUND(PtrC16) .NE. (/19, 1 /)))       ERROR STOP 16
  IF (ANY( UBOUND(PtrC16) .NE. (/19, 0 /)))       ERROR STOP 17
