...
!234567890123456789012345678901234567890123456789012345678901234567890
  ...
  DO J =I, N1
    ...
    Ptr(I:, J:) => Tar2
    IF ( ANY( MAXLOC(Ptr)  .NE. (/N2, N1/)) )  ERROR STOP 11
    IF ( ANY( MINLOC(Ptr)  .NE. (/1, 1/)) )    ERROR STOP 12
    CALL Check2()

    Ptr(I:J, I:J) => Tar1
    IF ( ANY( MAXLOC(Ptr)  .NE. (/J-I+1, J-I+1/)) )  ERROR STOP 21
  ...
  SUBROUTINE Check2()
    IF (SIZE(Ptr)  .NE. N2*N1 )                    ERROR STOP 40
    IF (.NOT. ASSOCIATED(Ptr, Tar2))               ERROR STOP 41
    IF (ANY( LBOUND(Ptr) .NE. (/I, J /)))          ERROR STOP 42
    IF (ANY( UBOUND(Ptr) .NE. (/I+N2-1, J+N1-1/))) ERROR STOP 43
  END SUBROUTINE
