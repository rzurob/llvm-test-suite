...
! %START
! %MAIN: YES
! %PRECMD: $TR_SRC/fxieee.presh neg012
! %COMPOPTS: -qstrict -qfloat=nomaf -qautodbl=dblpad4 -qfree=f90 -qxlf90=signedzero
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
...
!*                               NaNs, and Denormals.
!*                               Only real*4 will be promoted.
!*
!*  SECONDARY FUNCTIONS TESTED : -qautodbl=dblpad4 option
!*
!*  REQUIRED COMPILER OPTIONS  : -qautodbl=dblpad4
!*
!*  KEYWORD(S)                 :
!*  TARGET(S)                  :
...
! Test  NaNS for real*4. The function should return false.
        ...
        R = range(var_4)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 207

        result = ieee_is_negative(var_4)
        P = precision(var_4)
        R = range(var_4)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 208
...
! Test positive infinity for real*8.
        ...
        var_8 = PINF_8 !X'7FF0000000000000'
        P = precision(var_8)
        R = range(var_8)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 216
...
! Test negative infinity for real*8.
        ...
        var_8 = NINF_8 !X'FFF0000000000000'
        P = precision(var_8)
        R = range(var_8)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 217
...
! Test Denormals for real*8
        ...
        R = range(var_8)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 248

        result = ieee_is_negative(var_8)
        P = precision(var_8)
        R = range(var_8)
        Z = ieee_selected_real_kind(P,R)
        if (Z /= 8) error stop 249
