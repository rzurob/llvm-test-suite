...
! %START
! %MAIN: YES
! %PRECMD:  $TR_SRC/fxieee.presh rint001
...
!234567890123456789012345678901234567890123456789012345678901234567890
           ...
           integer :: i
           real*4 :: xr(2), res(2)
           real*8 :: xr_8(2), res_8(2)
           real*16 :: xr_16(2), res_16(2)
...
!  test nereast
            ...
            if (res(2) /= -34367.0 ) error stop 3

            !  test real*8
            xr_8(1) = 2345.89_8
            xr_8(2) = -34367.3_8
            res_8 = ieee_rint(xr_8)
            if (res_8(1) /= 2346.0_8 ) error stop 4
            if (res_8(2) /= -34367.0_8 ) error stop 5
            ...
            if (res_16(1) /= 2346.0_16 ) error stop 6
            if (res_16(2) /= -34367.0_16 ) error stop 7

            ! Now check that no flags were turned on.
            call ieee_get_flag(ieee_all,flag_values)
            ! Known limitation: IEEE_INEXACT may signal for real*16
            do i = 1,4
                if (flag_values(i) .neqv. .false.)error stop 50
            enddo
...
! test  to zero
            ...
            if (res(2) /= -34367.0 ) error stop 10

            !  test real*8
            xr_8(1) = 2345.89_8
            xr_8(2) = -34367.63_8
            res_8 = ieee_rint(xr_8)
            if (res_8(1) /= 2345.0_8 ) error stop 11
            if (res_8(2) /= -34367.0_8 ) error stop 12
            ...
            xr_16(2) = -34367.93_16
            res_16 = ieee_rint(xr_16)
            if (res_16(1) /= 2345.0_16 ) error stop 13
            if (res_16(2) /= -34367.0_16 ) error stop 14
...
! test +INF
            ...
            if (rtype /= rt_up) error stop 15
             ...
             if (res(2) /= -34367.0 ) error stop 17

             !  test real*8
             xr_8(1) = 2345.29_8
             xr_8(2) = -34367.63_8
             res_8 = ieee_rint(xr_8)
             if (res_8(1) /= 2346.0_8 ) error stop 18
             if (res_8(2) /= -34367.0_8 ) error stop 19
             ...
             xr_16(2) = -34367.93_16
             res_16 = ieee_rint(xr_16)
             if (res_16(1) /= 2346.0_16 ) error stop 20
             if (res_16(2) /= -34367.0_16 ) error stop 21
...
! test to -INF
            ...
            if (res(2) /= -34368.0 ) error stop 24

            !  test real*8
            xr_8(1) = 2345.829_8
            xr_8(2) = -34367.163_8
            res_8 = ieee_rint(xr_8)
            if (res_8(1) /= 2345.0_8 ) error stop 25
            if (res_8(2) /= -34368.0_8 ) error stop 26
            ...
            res_16 = ieee_rint(xr_16)
            if (res_16(1) /= 2345.0_16 ) error stop 27
            if (res_16(2) /= -34368.0_16 ) error stop 28
