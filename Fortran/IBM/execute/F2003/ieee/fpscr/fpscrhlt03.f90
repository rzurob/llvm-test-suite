...
! %VERIFY:
! %STDIN:
! %STDOUT: fpscrhlt03.out
! %EXECARGS:
! %POSTCMD: rm -f fpscrhlt03.out
! %END
!**********************************************************************
!*  ===================================================================
!*
...
!* In ext_sub101, halting mode is not modified.
	  ...
	  integer*4 flag_values(5)
          ...
          if (any(flag_values .ne. 0 ))          error stop 101
...
!* In ext_sub102, all halting flags are set to true.
	subroutine ext_sub102()
          ...
          if (any(flag_values .eq. 0 ))          error stop 102
...
!* not modified.
        subroutine ext_sub103()
          ...
          if (flag_values(1) .eq. 0)      	error stop 113
          if (flag_values(2) .ne. 0)            error stop 123
          if (flag_values(3) .eq. 0)            error stop 133
          if (flag_values(4) .ne. 0)            error stop 143
          if (flag_values(5) .eq. 0)            error stop 153
...
!* not modified; and the remaining flags are set to true.
        subroutine ext_sub104()
          ...
          if (flag_values(1) .ne. 0)      	error stop 114
          if (flag_values(2) .eq. 0)            error stop 124
          if (flag_values(3) .ne. 0)            error stop 134
          if (flag_values(4) .eq. 0)            error stop 144
          if (flag_values(5) .ne. 0)            error stop 154
