...
! %VERIFY:
! %STDIN:
! %STDOUT: fpscrflg03.out
! %EXECARGS:
! %POSTCMD: rm -f fpscrflg03.out
! %END
!**********************************************************************
!*  ===================================================================
!*
...
!*				 In this testcase, main program will call a
!*				 subroutine and the subroutine will call another
!*                               subroutine.
!*				 It tests the following scenarios:
!*	1) Processes that use IEEE calling processes that don't use IEEE.
!*	2) Processes that don't use IEEE calling processes that use IEEE.
...
!***********************************************************************
!*  Main program doesn't uses IEEE, and calls subroutine that uses IEEE
...
!*** sub4: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                 error stop 3

!*** Clear all flags on entry into subroutine
          call clr_fpscr_flags(fp_overflow)
          call clr_fpscr_flags(fp_div_by_zero)
          call clr_fpscr_flags(fp_invalid)
...
!*** sub5: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 4
...
!*** sub6: flag was set in subroutine
          call ext_sub6()
          flag_values(1) = get_fpscr_flags(fp_overflow)
          flag_values(2) = get_fpscr_flags(fp_div_by_zero)
          flag_values(3) = get_fpscr_flags(fp_invalid)
          ...
          if (any(flag_values .eq. 0))                  error stop 5
...
!*** sub7: flag was set in subroutine
          call ext_sub7()
          flag_values(1) = get_fpscr_flags(fp_overflow)
          flag_values(2) = get_fpscr_flags(fp_div_by_zero)
          flag_values(3) = get_fpscr_flags(fp_invalid)
          ...
          if (any(flag_values .eq. 0))                  error stop 6
...
!*** sub11: flag was not set in subroutine
          ...
          flag_values(4) = get_fpscr_flags(fp_underflow)
          flag_values(5) = get_fpscr_flags(fp_inexact)
          if (any(flag_values .ne. 0))                  error stop 7
...
!*** sub13: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 8
...
!*** sub15: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 9
...
!*** sub16: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 10
...
!*** sub17: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 11
...
!***********************************************************************
!*  Main program doesn't uses IEEE, and calls subroutine that doesn't use IEEE
...
!*** sub20: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 12
...
!*  which itself calls subroutrine that uses IEEE.
!* --------------------------------------------------------------
!*  Call to an external subroutine with exception flags SET on
...
!*** sub7: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 25
...
!*** sub11: flag was not set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 27
...
!*** sub13: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 28
...
!*** sub15: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 29
...
!*** sub16: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 30
...
!*** sub17: flag was set in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 31
...
!*** sub21: flag was not changed in subroutine
          ...
          if (any(flag_values .eq. 0))                  error stop 32
...
!***  Sub6 calls sub4 which sets the flag  ***
!*  sub6 has no interface inside main program
        subroutine ext_sub6()
          use ieee_arithmetic
	  logical*4 flag_values(5)
	  ...
	  call ieee_get_flag(ieee_all, flag_values)
!*  Check if all flags are cleared when entering procedure.
          if (any(flag_values .neqv. .false.))           error stop 209

	  call ext_sub4()
	  call ieee_get_flag(ieee_all, flag_values)
!*  Check if all flags are set when exiting procedure.
          if (any(flag_values .neqv. .true.))           error stop 210

	end subroutine !!ext_sub6()

!=======================================================================

!***  Sub7 calls sub4 which sets the flag  ***
!*  sub7 has no interface inside main program
!*  sub4 has no interface inside sub7
	  ...
	  call ieee_get_flag(ieee_all, flag_values)
!*  Check if all flags are cleared when entering procedure.
          if (any(flag_values .neqv. .false.))           error stop 211
