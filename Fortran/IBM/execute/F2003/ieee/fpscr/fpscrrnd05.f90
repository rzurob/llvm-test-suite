...
!*  Main program uses IEEE, and calls subroutine that uses IEEE.
          call ext_sub102()
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 2
...
!*  Main program uses IEEE, and calls subroutine that does not use IEEE.
          call ext_sub202()
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 4
...
!***  Subroutines that use IEEE modules.
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 10
	  ...
	  call ext_sub103()
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 12

!***  Subroutines that do not use IEEE modules.
	  call ext_sub202()
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 14

	  call ext_sub203()
          call ieee_get_rounding_mode(round_value)
          if (round_value /= ieee_up)              error stop 15
...
!*  Rounding mode should be: round towards plus infinity
	  round_value = get_round_mode()
  	  if (round_value /= fp_rnd_rp)		error stop 20
...
!*	   rounding mode return to the values they had at procedure entry.
          ...
          call ext_sub102()
	  round_value = get_round_mode()
  	  if (round_value /= fp_rnd_rp)		error stop 21

          call ext_sub103()
	  round_value = get_round_mode()
  	  if (round_value /= fp_rnd_rp)		error stop 22
