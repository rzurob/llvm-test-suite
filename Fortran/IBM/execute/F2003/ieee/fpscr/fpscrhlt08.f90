...
! %VERIFY:
! %STDIN:
! %STDOUT: fpscrhlt08.out
! %EXECARGS:
! %POSTCMD: rm -f fpscrhlt08.out
! %END
!**********************************************************************
!*  ===================================================================
!*
...
!*  Set halting mode flags to true.
          ...
          if (any(flag_values .eq. 0 ))          error stop 1
...
!*  returned will not change from the flag on entry into procedure.
	  call ext_sub101()
          ...
          flag_values(4) = get_fpscr_flags(trp_underflow)
          flag_values(5) = get_fpscr_flags(trp_inexact)
          if (any(flag_values .eq. 0 ))          error stop 2

!*  Main program doesn't uses IEEE, and calls subroutine that does not use IEEE,
!*  which calls other subroutines that do or don't use IEEE.

!*  Since a call to a procedure that does not use IEEE is made by main,
!*  the flags returned will be the last values in that procedure.
	  call ext_sub201()
          ...
          if (flag_values(1) .eq. 0)            error stop 3
          if (flag_values(2) .ne. 0)            error stop 4
          if (flag_values(3) .eq. 0)            error stop 5
          if (flag_values(4) .ne. 0)            error stop 6
          if (flag_values(5) .eq. 0)            error stop 7
