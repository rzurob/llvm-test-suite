...
! %START
! %MAIN: YES
! %PRECMD: $TR_SRC/fxi3e2.presh fnormal08
! %COMPOPTS: -qfloat=nans:nofold -qautodbl=dbl -qfree=f90 -qstrict
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
...
!*                               of REAL(8) using -qautodbl=dbl option.
!*
!234567890123456789012345678901234567890123456789012345678901234567890
        ...
        real(8), dimension(5) :: normal_result
        real(8), parameter :: normal_pos1 = tiny(1.0_8)
        real(8), parameter :: normal_pos2 = huge(1.0_8)
...
!...test with PZERO_16
        test_pzero = PZERO_16
        if (ieee_support_datatype(test_pzero) ) then
...
!...lowest range values
        test_pnanq_lowest = pnanq_lowest_16
        ...
        test_nnanq_lowest = nnanq_lowest_16
...
!...highest range values
        test_pnanq_highest = pnanq_highest_16
        ...
        test_nnanq_highest = nnanq_highest_16
        if (ieee_support_datatype(test_nnanq_highest)) then
           if ( ieee_is_normal(test_nnanq_highest) .eqv. .true. ) then
              error stop 18
           endif
        endif
...
!...lowest range values
        ...
        test_pnans_lowest = pnans_lowest_16
        ...
        test_nnans_lowest = nnans_lowest_16
...
!...highest range values
        ...
        test_pnans_highest = pnans_highest_16
        ...
        test_nnans_highest = nnans_highest_16
...
!...Check that no flags were turned on by IEEE_IS_NORMAL unless IEEE_INVALID
!... which should be true
        call ieee_get_flag(ieee_all, actual_flag_values)

        if (actual_flag_values(1) .neqv. .false. ) then
              error stop 23
        endif
        if (actual_flag_values(2) .neqv. .false. ) then
              error stop 24
        endif
        if (actual_flag_values(3) .neqv. .true. ) then
              error stop 25
        endif
        if (actual_flag_values(1) .neqv. .false. ) then
              error stop 26
