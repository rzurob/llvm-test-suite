...
void ssub_int(signed char *i, signed short *j, signed int *k, signed long long *l);

void ssub_real(float *i, double *j);

void ssub_char(char *i);

void ssub_log(unsigned char *i );

void ssub_comp(float _Complex *a, double _Complex *b);
...
int main()
{
   ...
   if ( testf(crealf(ac8), crealf(cc8) ) || 
    ...
    if ( testd(creal(ac16), creal(cc16) ) || 
        testd(cimag(ac16), cimag(cc16) )  ) exit(64);
