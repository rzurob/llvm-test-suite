...
! %START
! %MAIN: YES
! %PRECMD: export CmdLine="fxclat36 +++++++++ ------------ ____________"
! %COMPOPTS:  -qfree=f90
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
! %POSTCMD: $TR_SRC/cmdline.sh fxclat36
! %END
! *********************************************************************
!*  ===================================================================
...
!234567890123456789012345678901234567890123456789012345678901234567890
      ...
      character(2049)              :: CmdLine = 'fxclat36 +++++++++ ------------ ____________'
      integer                      :: CmdCount, i
      ...
      call GET_COMMAND(COMMAND%C(1:257), LENGTH%C, STATUS%C)

      if ( (TRIM(COMMAND%C(1:257)) .ne. TRIM(CmdLine))  .or. &
      ...
      DO i  = 0, CmdCount
        ...
        if ( (TRIM(VALUE%C(1:257)) .ne. TRIM(Argument))       .or. &
      ...
      NAME%C = 'CmdLine     '
      TRIM_NAME%C = .true.
      call GET_ENVIRONMENT_VARIABLE(NAME%C(1:257), VALUE%C(1:257), LENGTH%C, STATUS%C, TRIM_NAME%C)
      if ( (TRIM(VALUE%C(1:257)) .ne. TRIM(CmdLine))  .or. &
           (LENGTH%C .ne. LEN(TRIM(CmdLine)))  .or. &
           (STATUS%C .ne. 0))                       &
      then
        error stop 66
