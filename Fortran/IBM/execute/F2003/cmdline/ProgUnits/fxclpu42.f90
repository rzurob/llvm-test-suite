...
! %START
! %MAIN: YES
! %PRECMD: export CmdLine="fxclpu42 \# \! === ++++ ---"
! %COMPOPTS:  -qfree=f90
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
! %STDOUT:
! %EXECARGS:
! %POSTCMD: $TR_SRC/cmdline.sh fxclpu42
! %END
! *********************************************************************
!*  ===================================================================
!*
!*  TEST CASE NAME             : fxclpu42.f
!*
!*  DATE                       : Oct. 1, 2003
!*
...
!*
!*  DESCRIPTION                : Invoke command line procedures within external/internal recursive
!*                             : functions through interface and return the derived type of
!*                             : result by result variables
...
!234567890123456789012345678901234567890123456789012345678901234567890
      ...
      IF (Result%CmdCount .ne. 5)                     ERROR STOP 69
      IF (Result%CmdLine .ne. 'fxclpu42 \\# \\! === ++++ --- ')  ERROR STOP 70
      ...
      ELSE
        ...
        F_COMMAND_ARGUMENT_COUNT = F_COMMAND_ARGUMENT_COUNT_INT(5, CmdCount)

      END IF

      CONTAINS

      RECURSIVE FUNCTION  F_COMMAND_ARGUMENT_COUNT_INT(Num, CmdCount)
      INTEGER F_COMMAND_ARGUMENT_COUNT_INT
      INTEGER CmdCount
      INTEGER   Num

      IF (Num .gt. 1) THEN

        F_COMMAND_ARGUMENT_COUNT_INT = F_COMMAND_ARGUMENT_COUNT_INT(Num - 1, CmdCount)

      ELSE

        F_COMMAND_ARGUMENT_COUNT_INT = COMMAND_ARGUMENT_COUNT()
      ...
      ELSE
        ...
        R = F_GET_COMMAND_INT(Num - 1, Result)

      END IF

      CONTAINS

      RECURSIVE FUNCTION F_GET_COMMAND_INT(Num, Result) result ( R )
      IMPLICIT  NONE
      ...
      R = F_GET_COMMAND_ARGUMENT_INT(Num - 1, Result)
      ...
      RECURSIVE FUNCTION F_GET_COMMAND_ARGUMENT_INT(Num, Result) result ( R )
      IMPLICIT NONE
      ...
      R = F_GET_ENVIRONMENT_VARIABLE_INT(Num - 1, Result)

      END IF

      CONTAINS

      RECURSIVE FUNCTION F_GET_ENVIRONMENT_VARIABLE_INT(Num, Result) result ( R )
      IMPLICIT  NONE
