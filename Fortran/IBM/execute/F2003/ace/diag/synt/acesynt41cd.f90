...
!*
!*  SECONDARY FUNCTIONS TESTED : AC containing assumed-size array (print)
!*
...
!*
!*  KEYWORD(S)                 : dummy argument, array constructor, assumed-size, call
!*  TARGET(S)                  : 
...
program acesynt41cd
  ...
  type(derived):: drank1 (2), drank2 (2,2)
  integer      :: i
  ...
  drank1 = [derived(9), derived(5)]; drank2 = reshape([derived:: (derived(i),i=1,4)],[2,2])
...
contains
  ...
  subroutine callAssumedDerived(m, arr)
    ...
    type(derived) :: arr(*), array(m)
    call subTest   ([derived:: arr]);    call subTest   ([integer:: arr % field])
    call subTestE  ([derived:: arr]);    call subTestE  ([integer:: arr % field])
    result = size  ([derived:: arr]);    result = size  ([integer:: arr % field])
    val = funTest  ([derived:: arr]);    val = funTest  ([integer:: arr % field])
  end subroutine callAssumedDerived

  subroutine callAssumed2Derived(m, arr)
    integer :: m, val, result
    type(derived) :: arr(2,*), array(m)
    call subTest   ([derived:: arr]);    call subTest   ([integer:: arr % field])
    call subTestE  ([derived:: arr]);    call subTestE  ([integer:: arr % field])
    result = size  ([derived:: arr]);    result = size  ([integer:: arr % field])
    val = funTest  ([derived:: arr]);    val = funTest  ([integer:: arr % field])
  ...
  subroutine callAssumed3Derived(l, m, n, arr)
    integer :: l, m, n, val, result
    type(derived) :: arr(m,l:*), array(n)
    call subTest   ([derived:: arr]);    call subTest   ([integer:: arr % field])
    call subTestE  ([derived:: arr]);    call subTestE  ([integer:: arr % field])
    result = ubound([derived:: arr], 1); result = ubound([integer:: arr % field], 1);
    val = funTest  ([derived:: arr]);    val = funTest  ([integer:: arr % field])
