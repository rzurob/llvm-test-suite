...
!*  ============================================================================
!*
!*  TEST CASE NAME             : acetdt01c
!*
!*  DATE                       : 2006-09-08
!*
!*  PRIMARY FUNCTIONS TESTED   : Array Constructor Enhancements
!*
!*  SECONDARY FUNCTIONS TESTED : no-parameter derived TS allocatable component with defined assignment via elementals
!*
...
!*      t2array = (/ t1array /)
!*
!*  The tricky part is that defined assignment will be invoked once in some cases,
!*  and sometimes twice!  When an array or a structure is constructed, we may find
!*  defined assigment invoked; when the array or structure so constructed is then
!*  assigned, we will find the defined assignment invoked again!
!*
...
!*  procedures with non-passed-object dummy array arguments and a generic interface.
!*
...
!    print *, "Assign 2*", that%datum, "(", (2*that%datum), ") to", this % datum
  ...
  subroutine validateFromSources(tgt, src1, src2)
    class (Container), intent(in) :: tgt(2), src1, src2
  ...
  subroutine validateFromSource(tgt, src)
    class (Container), intent(in) :: tgt, src
...
program acetdt01c
  ...
  type (Container) :: cont, cont1, cont2, cont3
  type (Container), target :: conta(2), conta2(2)
  type (Container), allocatable :: contall(:)
  type (Container), pointer :: contp(:)
  integer :: i
  ...
  conta  = (/ Container:: cont, cont2 /)
  call validateFromSources(conta, cont, cont2)

  conta2 = (/ Container:: conta /)
  call validateFromSources(conta2, conta(1), conta(2))
  ...
  conta  = (/ Container:: (cont, cont2, i=1,1) /)
  call validateFromSources(conta, cont, cont2)

  conta2 = (/ Container:: (conta, i=1,1) /)
  call validateFromSources(conta2, conta(1), conta(2))
