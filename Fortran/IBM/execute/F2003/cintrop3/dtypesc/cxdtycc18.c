...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
     for ( i = 0; i < DIM1; i++ ) {
      for ( j = 0; j < DIM2; j++ ) {
#ifdef CMPLX
       if ( (dtb+k)->var_a[i][j] != (float)(2*(i*DIM2+j+1)+1)+I*(float)(2*(i*DIM2+j+1)+1) ) exit(37);
#else
       if ( (dtb+k)->var_a[i][j] != createcomplexf((float)(2*(i*DIM2+j+1)+1),(float)(2*(i*DIM2+j+1)+1)) ) exit(37);
#endif
       if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+2)+1 ) exit(39);
#ifdef CMPLX
       if ( (dtb+k)->var_c[i][j] != (long double)(2*(i*DIM2+j+3)+1)+I*(long double)(2*(i*DIM2+j+3)+1) ) exit(41);
#else
       if ( (dtb+k)->var_c[i][j] != createcomplexl((long double)(2*(i*DIM2+j+3)+1),(long double)(2*(i*DIM2+j+3)+1)) ) exit(41);
#endif
#ifdef CMPLX
       if ( (dtb+k)->var_d[i][j] != (double)(2*(i*DIM2+j+4)+1)+I*(double)(2*(i*DIM2+j+4)+1) ) exit(43);
#else
       if ( (dtb+k)->var_d[i][j] != createcomplex((double)(2*(i*DIM2+j+4)+1),(double)(2*(i*DIM2+j+4)+1)) ) exit(43);
#endif
#ifdef CMPLX
       if ( (dtb+k)->var_e[i][j] != (float)(2*(i*DIM2+j+5)+1)+I*(float)(2*(i*DIM2+j+5)+1) ) exit(45);
#else
       if ( (dtb+k)->var_e[i][j] != createcomplexf((float)(2*(i*DIM2+j+5)+1),(float)(2*(i*DIM2+j+5)+1)) ) exit(45);
#endif
       if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+6)+1 ) exit(47);
#ifdef CMPLX
       if ( (dtb+k)->var_g[i][j] != (double)(2*(i*DIM2+j+7)+1)+I*(double)(2*(i*DIM2+j+7)+1) ) exit(49);
#else
       if ( (dtb+k)->var_g[i][j] != createcomplex((double)(2*(i*DIM2+j+7)+1),(double)(2*(i*DIM2+j+7)+1)) ) exit(49);
#endif
