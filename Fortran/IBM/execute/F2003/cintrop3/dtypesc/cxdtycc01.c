...
#ifdef CMPLX
   if ( dtx->var_a != 4.0f+I*4.0f ) exit(69);
#else
   if ( dtx->var_a != createcomplexf(4.0f,4.0f) ) exit(69);
#endif
   if ( dtx->var_b != 8 ) exit(71);
#ifdef CMPLX
   if ( dtx->var_c != 12.0l+I*12.0l ) exit(73);
#else
   if ( dtx->var_c != createcomplexl(12.0l,12.0l) ) exit(73);
#endif
#ifdef CMPLX
   if ( dtx->var_d != 16.0+I*16.0 ) exit(75);
#else
   if ( dtx->var_d != createcomplex(16.0,16.0) ) exit(75);
#endif
#ifdef CMPLX
   if ( dtx->var_e != 20.0f+I*20.0f ) exit(77);
#else
   if ( dtx->var_e != createcomplexf(20.0f,20.0f) ) exit(77);
#endif
   if ( dtx->var_f != 24 ) exit(79);
#ifdef CMPLX
   if ( dtx->var_g != 28.0+I*28.0 ) exit(81);
#else
   if ( dtx->var_g != createcomplex(28.0,28.0) ) exit(81);
#endif
...
#ifdef CMPLX
   dtz->var_a = dty->var_a + 1.0f+I*1.0f;
#else
   dtz->var_a = dty->var_a + createcomplexf(1.0f,1.0f);
#endif
   dtz->var_b = dty->var_b + 1;
#ifdef CMPLX
   dtz->var_c = dty->var_c + 1.0l+I*1.0l;
#else
   dtz->var_c = dty->var_c + createcomplexl(1.0l,1.0l);
#endif
#ifdef CMPLX
   dtz->var_d = dty->var_d + 1.0+I*1.0;
#else
   dtz->var_d = dty->var_d + createcomplex(1.0,1.0);
#endif
#ifdef CMPLX
   dtz->var_e = dty->var_e + 1.0f+I*1.0f;
#else
   dtz->var_e = dty->var_e + createcomplexf(1.0f,1.0f);
#endif
   dtz->var_f = dty->var_f + 1;
#ifdef CMPLX
   dtz->var_g = dty->var_g + 1.0+I*1.0;
#else
   dtz->var_g = dty->var_g + createcomplex(1.0,1.0);
#endif
