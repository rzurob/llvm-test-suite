...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtyd00.presh fxdtycd34 cxdtycc34
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!*      - Testing allocatable arrays of 2-levels deep derived types with integer and real components
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( dt(l,k)%vdt1(j,i)%var_b(i,j) /= (dble(i+j+(j-1)*DIM2),dble(i+j+(j-1)*DIM2)) ) error stop 38
       if ( dt(l,k)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+1 ) error stop 40
       if ( dt(l,k)%vdt1(j,i)%var_d(i,j) /= (real(i+j+(j-1)*DIM2+2),real(i+j+(j-1)*DIM2+2)) ) error stop 42
       if ( dt(l,k)%vdt1(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 44
       if ( dt(l,k)%vdt1(j,i)%var_f(i,j) /= (dble(i+j+(j-1)*DIM2+4),dble(i+j+(j-1)*DIM2+4)) ) error stop 46
       if ( dt(l,k)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+5 ) error stop 48
       if ( dt(l,k)%vdt1(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 50
      end do
     end do
    end do
   end do
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       dt(l,k)%vdt1(j,i)%var_b(i,j) = (dble(i+j+(j-1)*DIM2+1),dble(i+j+(j-1)*DIM2+1))
       dt(l,k)%vdt1(j,i)%var_c(i,j) = i+j+(j-1)*DIM2+2
       dt(l,k)%vdt1(j,i)%var_d(i,j) = (real(i+j+(j-1)*DIM2+3),real(i+j+(j-1)*DIM2+3))
       dt(l,k)%vdt1(j,i)%var_e(i,j) = (dble(i+j+(j-1)*DIM2+4),dble(i+j+(j-1)*DIM2+4))
       dt(l,k)%vdt1(j,i)%var_f(i,j) = (dble(i+j+(j-1)*DIM2+5),dble(i+j+(j-1)*DIM2+5))
       dt(l,k)%vdt1(j,i)%var_g(i,j) = i+j+(j-1)*DIM2+6
       dt(l,k)%vdt1(j,i)%var_h(i,j) = (real(i+j+(j-1)*DIM2+7),real(i+j+(j-1)*DIM2+7))
      end do
     end do
    end do
   end do
...
subroutine sub2(dtx,dty) bind(c)
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dtx(l,k)%var_a(i,j) /= (real(2*(i+j+(j-1)*DIM2-1)),real(2*(i+j+(j-1)*DIM2-1))) ) error stop 52
       if ( dtx(l,k)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2) ) error stop 54
       if ( dtx(l,k)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+1)),dble(2*(i+j+(j-1)*DIM2+1))) ) error stop 56
       if ( dtx(l,k)%var_d(i,j) /= (dble(2*(i+j+(j-1)*DIM2+2)),dble(2*(i+j+(j-1)*DIM2+2))) ) error stop 58
       if ( dtx(l,k)%var_e(i,j) /= (real(2*(i+j+(j-1)*DIM2+3)),real(2*(i+j+(j-1)*DIM2+3))) ) error stop 60
       if ( dtx(l,k)%var_f(i,j) /= 2*(i+j+(j-1)*DIM2+4) ) error stop 62
       if ( dtx(l,k)%var_g(i,j) /= (dble(2*(i+j+(j-1)*DIM2+5)),dble(2*(i+j+(j-1)*DIM2+5))) ) error stop 64
       if ( dtx(l,k)%var_h(i,j) /= 2*(i+j+(j-1)*DIM2+6) ) error stop 66

       if ( dtx(l,k)%vdt1(j,i)%var_a(i,j) /= 2*(i+j+(j-1)*DIM2-1) ) error stop 68
       if ( dtx(l,k)%vdt1(j,i)%var_b(i,j) /= (dble(2*(i+j+(j-1)*DIM2)),dble(2*(i+j+(j-1)*DIM2))) ) error stop 70
       if ( dtx(l,k)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+1) ) error stop 72
       if ( dtx(l,k)%vdt1(j,i)%var_d(i,j) /= (real(2*(i+j+(j-1)*DIM2+2)),real(2*(i+j+(j-1)*DIM2+2))) ) error stop 74
       if ( dtx(l,k)%vdt1(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+3)),dble(2*(i+j+(j-1)*DIM2+3))) ) error stop 76
       if ( dtx(l,k)%vdt1(j,i)%var_f(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)),dble(2*(i+j+(j-1)*DIM2+4))) ) error stop 78
       if ( dtx(l,k)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+5) ) error stop 80
       if ( dtx(l,k)%vdt1(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+6)),real(2*(i+j+(j-1)*DIM2+6))) ) error stop 82
      end do
     end do
    end do
   end do
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       dty(l,k)%vdt1(j,i)%var_b(i,j) = (dble(dtx(l,k)%vdt1(j,i)%var_b(i,j)),dble(dtx(l,k)%vdt1(j,i)%var_b(i,j))) + (2.0d0,2.0d0)
       dty(l,k)%vdt1(j,i)%var_c(i,j) = dtx(l,k)%vdt1(j,i)%var_c(i,j) + 3
       dty(l,k)%vdt1(j,i)%var_d(i,j) = (real(dtx(l,k)%vdt1(j,i)%var_d(i,j)),real(dtx(l,k)%vdt1(j,i)%var_d(i,j))) + (4.0e0,4.0e0)
       dty(l,k)%vdt1(j,i)%var_e(i,j) = (dble(dtx(l,k)%vdt1(j,i)%var_e(i,j)),dble(dtx(l,k)%vdt1(j,i)%var_e(i,j))) + (5.0q0,5.0q0)
       dty(l,k)%vdt1(j,i)%var_f(i,j) = (dble(dtx(l,k)%vdt1(j,i)%var_f(i,j)),dble(dtx(l,k)%vdt1(j,i)%var_f(i,j))) + (6.0d0,6.0d0)
       dty(l,k)%vdt1(j,i)%var_g(i,j) = dtx(l,k)%vdt1(j,i)%var_g(i,j) + 7
       dty(l,k)%vdt1(j,i)%var_h(i,j) = (real(dtx(l,k)%vdt1(j,i)%var_h(i,j)),real(dtx(l,k)%vdt1(j,i)%var_h(i,j))) + (8.0e0,8.0e0)
      end do
     end do
    end do
   end do
