...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtyd00.presh fxdtycd20 cxdtycc20
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 40
       if ( dta(l,k)%vdt2(j,i)%var_d(i,j) /= i+j+(j-1)*DIM2+4 ) error stop 42
       if ( dta(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+5),dble(i+j+(j-1)*DIM2+5)) ) error stop 44
       if ( dta(l,k)%vdt2(j,i)%var_f(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 46
       if ( dta(l,k)%vdt2(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+7 ) error stop 48
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+8),real(i+j+(j-1)*DIM2+8)) ) error stop 50

       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) /= i+j+(j-1)*DIM2+2 ) error stop 52
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 54
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+4 ) error stop 56
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(i+j+(j-1)*DIM2+5),real(i+j+(j-1)*DIM2+5)) ) error stop 58
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+6),dble(i+j+(j-1)*DIM2+6)) ) error stop 60
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(i+j+(j-1)*DIM2+7),dble(i+j+(j-1)*DIM2+7)) ) error stop 62
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+8 ) error stop 64
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+9),real(i+j+(j-1)*DIM2+9)) ) error stop 66
      end do
     end do
    end do
   end do
...
!! Test 2
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dtb(l,k)%var_a(i,j) /= (real(2*(i+j+(j-1)*DIM2)-1),real(2*(i+j+(j-1)*DIM2)-1)) ) error stop 68
       if ( dtb(l,k)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2+1)-1 ) error stop 70
       if ( dtb(l,k)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+2)-1),dble(2*(i+j+(j-1)*DIM2+2)-1)) ) error stop 72
       if ( dtb(l,k)%var_d(i,j) /= (dble(2*(i+j+(j-1)*DIM2+3)-1),dble(2*(i+j+(j-1)*DIM2+3)-1)) ) error stop 74
       if ( dtb(l,k)%var_e(i,j) /= (real(2*(i+j+(j-1)*DIM2+4)-1),real(2*(i+j+(j-1)*DIM2+4)-1)) ) error stop 76
       if ( dtb(l,k)%var_f(i,j) /= 2*(i+j+(j-1)*DIM2+5)-1 ) error stop 78
       if ( dtb(l,k)%var_g(i,j) /= (dble(2*(i+j+(j-1)*DIM2+6)-1),dble(2*(i+j+(j-1)*DIM2+6)-1)) ) error stop 80
       if ( dtb(l,k)%var_h(i,j) /= 2*(i+j+(j-1)*DIM2+7)-1 ) error stop 82

       if ( dtb(l,k)%vdt2(j,i)%var_a(i,j) /= (dble(2*(i+j+(j-1)*DIM2)),dble(2*(i+j+(j-1)*DIM2))) ) error stop 84
       if ( dtb(l,k)%vdt2(j,i)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2+1) ) error stop 86
       if ( dtb(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+2)),dble(2*(i+j+(j-1)*DIM2+2))) ) error stop 88
       if ( dtb(l,k)%vdt2(j,i)%var_d(i,j) /= 2*(i+j+(j-1)*DIM2+3) ) error stop 90
       if ( dtb(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)),dble(2*(i+j+(j-1)*DIM2+4))) ) error stop 92
       if ( dtb(l,k)%vdt2(j,i)%var_f(i,j) /= (real(2*(i+j+(j-1)*DIM2+5)),real(2*(i+j+(j-1)*DIM2+5))) ) error stop 94
       if ( dtb(l,k)%vdt2(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6) ) error stop 96
       if ( dtb(l,k)%vdt2(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+7)),real(2*(i+j+(j-1)*DIM2+7))) ) error stop 98

       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) /= 2*(i+j+(j-1)*DIM2)+1 ) error stop 100
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(2*(i+j+(j-1)*DIM2+1)+1),dble(2*(i+j+(j-1)*DIM2+1)+1)) ) error stop 102
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+2)+1 ) error stop 104
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(2*(i+j+(j-1)*DIM2+3)+1),real(2*(i+j+(j-1)*DIM2+3)+1)) ) error stop 106
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)+1),dble(2*(i+j+(j-1)*DIM2+4)+1)) ) error stop 108
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(2*(i+j+(j-1)*DIM2+5)+1),dble(2*(i+j+(j-1)*DIM2+5)+1)) ) error stop 110
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6)+1 ) error stop 112
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+7)+1),real(2*(i+j+(j-1)*DIM2+7)+1)) ) error stop 114
      end do
     end do
    end do
   end do

end program fxdtycd20
