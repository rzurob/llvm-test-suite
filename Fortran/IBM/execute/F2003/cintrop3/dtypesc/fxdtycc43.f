...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya01.presh fxdtycc43
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!! Test 3
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dta(l,k)%var_a(i,j) /= (real(2*(i+j+(j-1)*DIM2-1)+1),real(2*(i+j+(j-1)*DIM2-1)+1)) ) error stop 68
       if ( dta(l,k)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2)+2 ) error stop 70
       if ( dta(l,k)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+1)+3),dble(2*(i+j+(j-1)*DIM2+1)+3)) ) error stop 72
       if ( dta(l,k)%var_d(i,j) /= (dble(2*(i+j+(j-1)*DIM2+2)+4),dble(2*(i+j+(j-1)*DIM2+2)+4)) ) error stop 74
       if ( dta(l,k)%var_e(i,j) /= (real(2*(i+j+(j-1)*DIM2+3)+5),real(2*(i+j+(j-1)*DIM2+3)+5)) ) error stop 76
       if ( dta(l,k)%var_f(i,j) /= 2*(i+j+(j-1)*DIM2+4)+6 ) error stop 78
       if ( dta(l,k)%var_g(i,j) /= (dble(2*(i+j+(j-1)*DIM2+5)+7),dble(2*(i+j+(j-1)*DIM2+5)+7)) ) error stop 80
       if ( dta(l,k)%var_h(i,j) /= 2*(i+j+(j-1)*DIM2+6)+8 ) error stop 82

       if ( dta(l,k)%vdt2(j,i)%var_a(i,j) /= (dble(2*(i+j+(j-1)*DIM2-1)+1),dble(2*(i+j+(j-1)*DIM2-1)+1)) ) error stop 84
       if ( dta(l,k)%vdt2(j,i)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2)+2 ) error stop 86
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+1)+3),dble(2*(i+j+(j-1)*DIM2+1)+3)) ) error stop 88
       if ( dta(l,k)%vdt2(j,i)%var_d(i,j) /= 2*(i+j+(j-1)*DIM2+2)+4 ) error stop 90
       if ( dta(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+3)+5),dble(2*(i+j+(j-1)*DIM2+3)+5)) ) error stop 92
       if ( dta(l,k)%vdt2(j,i)%var_f(i,j) /= (real(2*(i+j+(j-1)*DIM2+4)+6),real(2*(i+j+(j-1)*DIM2+4)+6)) ) error stop 94
       if ( dta(l,k)%vdt2(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+5)+7 ) error stop 96
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+6)+8),real(2*(i+j+(j-1)*DIM2+6)+8)) ) error stop 98

       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) /= 2*(i+j+(j-1)*DIM2-1)+1 ) error stop 100
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(2*(i+j+(j-1)*DIM2)+2),dble(2*(i+j+(j-1)*DIM2)+2)) ) error stop 102
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+1)+3 ) error stop 104
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(2*(i+j+(j-1)*DIM2+2)+4),real(2*(i+j+(j-1)*DIM2+2)+4)) ) error stop 106
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+3)+5),dble(2*(i+j+(j-1)*DIM2+3)+5)) ) error stop 108
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)+6),dble(2*(i+j+(j-1)*DIM2+4)+6)) ) error stop 110
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+5)+7 ) error stop 112
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+6)+8),real(2*(i+j+(j-1)*DIM2+6)+8)) ) error stop 114
      end do
     end do
    end do
   end do
...
subroutine sub1()
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dta(l,k)%var_a(i,j) /= (real(i+j+(j-1)*DIM2-1),real(i+j+(j-1)*DIM2-1)) ) error stop 116
       if ( dta(l,k)%var_b(i,j) /= i+j+(j-1)*DIM2 ) error stop 118
       if ( dta(l,k)%var_c(i,j) /= (dble(i+j+(j-1)*DIM2+1),dble(i+j+(j-1)*DIM2+1)) ) error stop 120
       if ( dta(l,k)%var_d(i,j) /= (dble(i+j+(j-1)*DIM2+2),dble(i+j+(j-1)*DIM2+2)) ) error stop 122
       if ( dta(l,k)%var_e(i,j) /= (real(i+j+(j-1)*DIM2+3),real(i+j+(j-1)*DIM2+3)) ) error stop 124
       if ( dta(l,k)%var_f(i,j) /= i+j+(j-1)*DIM2+4 ) error stop 126
       if ( dta(l,k)%var_g(i,j) /= (dble(i+j+(j-1)*DIM2+5),dble(i+j+(j-1)*DIM2+5)) ) error stop 128
       if ( dta(l,k)%var_h(i,j) /= i+j+(j-1)*DIM2+6 ) error stop 130

       if ( dta(l,k)%vdt2(j,i)%var_a(i,j) /= (dble(i+j+(j-1)*DIM2-1),dble(i+j+(j-1)*DIM2-1)) ) error stop 132
       if ( dta(l,k)%vdt2(j,i)%var_b(i,j) /= i+j+(j-1)*DIM2 ) error stop 134
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(i+j+(j-1)*DIM2+1),dble(i+j+(j-1)*DIM2+1)) ) error stop 136
       if ( dta(l,k)%vdt2(j,i)%var_d(i,j) /= i+j+(j-1)*DIM2+2 ) error stop 138
       if ( dta(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 140
       if ( dta(l,k)%vdt2(j,i)%var_f(i,j) /= (real(i+j+(j-1)*DIM2+4),real(i+j+(j-1)*DIM2+4)) ) error stop 142
       if ( dta(l,k)%vdt2(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+5 ) error stop 144
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 146

       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) /= i+j+(j-1)*DIM2-1 ) error stop 148
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(i+j+(j-1)*DIM2),dble(i+j+(j-1)*DIM2)) ) error stop 150
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+1 ) error stop 152
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(i+j+(j-1)*DIM2+2),real(i+j+(j-1)*DIM2+2)) ) error stop 154
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 156
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(i+j+(j-1)*DIM2+4),dble(i+j+(j-1)*DIM2+4)) ) error stop 158
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+5 ) error stop 160
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 162
      end do
     end do
    end do
   end do
