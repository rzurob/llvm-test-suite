...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya01.presh fxdtycc45a
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
!! Test 3
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dtb(l,k)%var_a(i,j) /= (real(2*(i+j+(j-1)*DIM2-1)+1),real(2*(i+j+(j-1)*DIM2-1)+1)) ) error stop 116
       ...
       if ( dtb(l,k)%var_d(i,j) /= (dble(2*(i+j+(j-1)*DIM2+3)+2),dble(2*(i+j+(j-1)*DIM2+3)+2)) ) error stop 122
       if ( dtb(l,k)%var_e(i,j) /= (real(2*(i+j+(j-1)*DIM2+4)+3),real(2*(i+j+(j-1)*DIM2+4)+3)) ) error stop 124
       if ( dtb(l,k)%var_f(i,j) /= 2*(i+j+(j-1)*DIM2+5)+4 ) error stop 126
       if ( dtb(l,k)%var_g(i,j) /= (dble(2*(i+j+(j-1)*DIM2+6)+5),dble(2*(i+j+(j-1)*DIM2+6)+5)) ) error stop 128
       ...
       if ( dtb(l,k)%vdt2(j,i)%var_a(i,j) /= (dble(2*(i+j+(j-1)*DIM2-1)+1),dble(2*(i+j+(j-1)*DIM2-1)+1)) ) error stop 148
       if ( dtb(l,k)%vdt2(j,i)%var_b(i,j) /= 2*(i+j+(j-1)*DIM2+1) ) error stop 150
       if ( dtb(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(2*(i+j+(j-1)*DIM2+2)+1),dble(2*(i+j+(j-1)*DIM2+2)+1)) ) error stop 152
       if ( dtb(l,k)%vdt2(j,i)%var_d(i,j) /= 2*(i+j+(j-1)*DIM2+3)+2 ) error stop 154
       if ( dtb(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)+3),dble(2*(i+j+(j-1)*DIM2+4)+3)) ) error stop 156
       if ( dtb(l,k)%vdt2(j,i)%var_f(i,j) /= (real(2*(i+j+(j-1)*DIM2+5)+4),real(2*(i+j+(j-1)*DIM2+5)+4)) ) error stop 158
       if ( dtb(l,k)%vdt2(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6)+5 ) error stop 160
       if ( dtb(l,k)%vdt2(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+7)+6),real(2*(i+j+(j-1)*DIM2+7)+6)) ) error stop 162
       ...
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(2*(i+j+(j-1)*DIM2+1)),dble(2*(i+j+(j-1)*DIM2+1))) ) error stop 182
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+2)+1 ) error stop 184
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(2*(i+j+(j-1)*DIM2+3)+2),real(2*(i+j+(j-1)*DIM2+3)+2)) ) error stop 186
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(2*(i+j+(j-1)*DIM2+4)+3),dble(2*(i+j+(j-1)*DIM2+4)+3)) ) error stop 188
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(2*(i+j+(j-1)*DIM2+5)+4),dble(2*(i+j+(j-1)*DIM2+5)+4)) ) error stop 190
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6)+5 ) error stop 192
       if ( dtb(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(2*(i+j+(j-1)*DIM2+7)+6),real(2*(i+j+(j-1)*DIM2+7)+6)) ) error stop 194
...
function fun1(dtk) bind(c)
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dtk(l,k)%var_a(i,j) /= (real(i+j+(j-1)*DIM2-1),real(i+j+(j-1)*DIM2-1)) ) error stop 212
       if ( dtk(l,k)%var_b(i,j) /= i+j+(j-1)*DIM2 ) error stop 214
       if ( dtk(l,k)%var_c(i,j) /= (dble(i+j+(j-1)*DIM2+1),dble(i+j+(j-1)*DIM2+1)) ) error stop 216
       if ( dtk(l,k)%var_d(i,j) /= (dble(i+j+(j-1)*DIM2+2),dble(i+j+(j-1)*DIM2+2)) ) error stop 218
       if ( dtk(l,k)%var_e(i,j) /= (real(i+j+(j-1)*DIM2+3),real(i+j+(j-1)*DIM2+3)) ) error stop 220
       if ( dtk(l,k)%var_f(i,j) /= i+j+(j-1)*DIM2+4 ) error stop 222
       if ( dtk(l,k)%var_g(i,j) /= (dble(i+j+(j-1)*DIM2+5),dble(i+j+(j-1)*DIM2+5)) ) error stop 224
       if ( dtk(l,k)%var_h(i,j) /= i+j+(j-1)*DIM2+6 ) error stop 226

       if ( dtk(l,k)%vdt2(j,i)%var_a(i,j) /= (dble(i+j+(j-1)*DIM2-1),dble(i+j+(j-1)*DIM2-1)) ) error stop 228
       if ( dtk(l,k)%vdt2(j,i)%var_b(i,j) /= i+j+(j-1)*DIM2 ) error stop 230
       if ( dtk(l,k)%vdt2(j,i)%var_c(i,j) /= (dble(i+j+(j-1)*DIM2+1),dble(i+j+(j-1)*DIM2+1)) ) error stop 232
       if ( dtk(l,k)%vdt2(j,i)%var_d(i,j) /= i+j+(j-1)*DIM2+2 ) error stop 234
       if ( dtk(l,k)%vdt2(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 236
       if ( dtk(l,k)%vdt2(j,i)%var_f(i,j) /= (real(i+j+(j-1)*DIM2+4),real(i+j+(j-1)*DIM2+4)) ) error stop 238
       if ( dtk(l,k)%vdt2(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+5 ) error stop 240
       if ( dtk(l,k)%vdt2(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 242

       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) /= i+j+(j-1)*DIM2-1 ) error stop 244
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= (dble(i+j+(j-1)*DIM2),dble(i+j+(j-1)*DIM2)) ) error stop 246
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+1 ) error stop 248
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= (real(i+j+(j-1)*DIM2+2),real(i+j+(j-1)*DIM2+2)) ) error stop 250
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= (dble(i+j+(j-1)*DIM2+3),dble(i+j+(j-1)*DIM2+3)) ) error stop 252
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= (dble(i+j+(j-1)*DIM2+4),dble(i+j+(j-1)*DIM2+4)) ) error stop 254
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+5 ) error stop 256
       if ( dtk(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= (real(i+j+(j-1)*DIM2+6),real(i+j+(j-1)*DIM2+6)) ) error stop 258
      end do
     end do
    end do
   end do
