...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya00.presh fxdtyra30 cxdtyra28
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( dtb(l,k)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2+1)) ) error stop 70
       if ( dtb(l,k)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+2) ) error stop 72
       if ( dtb(l,k)%vdt1(j,i)%var_d(i,j) /= real(2*(i+j+(j-1)*DIM2+3)) ) error stop 74
       if ( dtb(l,k)%vdt1(j,i)%var_e(i,j) /= dble(2*(i+j+(j-1)*DIM2+4)) ) error stop 76
       if ( dtb(l,k)%vdt1(j,i)%var_f(i,j) /= dble(2*(i+j+(j-1)*DIM2+5)) ) error stop 78
       if ( dtb(l,k)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6) ) error stop 80
       if ( dtb(l,k)%vdt1(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+7)) ) error stop 82
      end do
     end do
    end do
   end do
