...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya00.presh fxdtyra31 cxdtyra29
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( dta(l,k)%vdt1(j,i)%var_b(i,j) /= dble(i+j+(j-1)*DIM2+2) ) error stop 54
       if ( dta(l,k)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+3 ) error stop 56
       if ( dta(l,k)%vdt1(j,i)%var_d(i,j) /= real(i+j+(j-1)*DIM2+4) ) error stop 58
       if ( dta(l,k)%vdt1(j,i)%var_e(i,j) /= dble(i+j+(j-1)*DIM2+5) ) error stop 60
       if ( dta(l,k)%vdt1(j,i)%var_f(i,j) /= dble(i+j+(j-1)*DIM2+6) ) error stop 62
       if ( dta(l,k)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+7 ) error stop 64
       if ( dta(l,k)%vdt1(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+8) ) error stop 66

       if ( dtr(l,k)%vdt1(j,i)%var_a(i,j) /= i+j+(j-1)*DIM2+1 ) error stop 68
       if ( dtr(l,k)%vdt1(j,i)%var_b(i,j) /= dble(i+j+(j-1)*DIM2+2) ) error stop 70
       if ( dtr(l,k)%vdt1(j,i)%var_c(i,j) /= i+j+(j-1)*DIM2+3 ) error stop 72
       if ( dtr(l,k)%vdt1(j,i)%var_d(i,j) /= real(i+j+(j-1)*DIM2+4) ) error stop 74
       if ( dtr(l,k)%vdt1(j,i)%var_e(i,j) /= dble(i+j+(j-1)*DIM2+5) ) error stop 76
       if ( dtr(l,k)%vdt1(j,i)%var_f(i,j) /= dble(i+j+(j-1)*DIM2+6) ) error stop 78
       if ( dtr(l,k)%vdt1(j,i)%var_g(i,j) /= i+j+(j-1)*DIM2+7 ) error stop 80
       if ( dtr(l,k)%vdt1(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+8) ) error stop 82
      end do
     end do
    end do
   end do
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( dtb(l,k)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2+1)) ) error stop 118
       if ( dtb(l,k)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+2) ) error stop 120
       if ( dtb(l,k)%vdt1(j,i)%var_d(i,j) /= real(2*(i+j+(j-1)*DIM2+3)) ) error stop 122
       if ( dtb(l,k)%vdt1(j,i)%var_e(i,j) /= dble(2*(i+j+(j-1)*DIM2+4)) ) error stop 124
       if ( dtb(l,k)%vdt1(j,i)%var_f(i,j) /= dble(2*(i+j+(j-1)*DIM2+5)) ) error stop 126
       if ( dtb(l,k)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6) ) error stop 128
       if ( dtb(l,k)%vdt1(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+7)) ) error stop 130

       if ( dtr(l,k)%vdt1(j,i)%var_a(i,j) /= 2*(i+j+(j-1)*DIM2) ) error stop 132
       if ( dtr(l,k)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2+1)) ) error stop 134
       if ( dtr(l,k)%vdt1(j,i)%var_c(i,j) /= 2*(i+j+(j-1)*DIM2+2) ) error stop 136
       if ( dtr(l,k)%vdt1(j,i)%var_d(i,j) /= real(2*(i+j+(j-1)*DIM2+3)) ) error stop 138
       if ( dtr(l,k)%vdt1(j,i)%var_e(i,j) /= dble(2*(i+j+(j-1)*DIM2+4)) ) error stop 140
       if ( dtr(l,k)%vdt1(j,i)%var_f(i,j) /= dble(2*(i+j+(j-1)*DIM2+5)) ) error stop 142
       if ( dtr(l,k)%vdt1(j,i)%var_g(i,j) /= 2*(i+j+(j-1)*DIM2+6) ) error stop 144
       if ( dtr(l,k)%vdt1(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+7)) ) error stop 146
      end do
     end do
    end do
   end do
