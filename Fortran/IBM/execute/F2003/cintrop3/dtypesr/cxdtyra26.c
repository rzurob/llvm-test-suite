   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
      ...
      if ( (dta+k)->vdt2[j][i].var_a[i][j] != (double)(i*DIM2+j+2) ) exit(37);
      if ( (dta+k)->vdt2[j][i].var_b[i][j] != i*DIM2+j+3 ) exit(39);
      if ( (dta+k)->vdt2[j][i].var_c[i][j] != (double)(i*DIM2+j+4) ) exit(41);
      if ( (dta+k)->vdt2[j][i].var_d[i][j] != i*DIM2+j+5 ) exit(43);
      if ( (dta+k)->vdt2[j][i].var_e[i][j] != (long double)(i*DIM2+j+6) ) exit(45);
      if ( (dta+k)->vdt2[j][i].var_f[i][j] != (float)(i*DIM2+j+7) ) exit(47);
      if ( (dta+k)->vdt2[j][i].var_g[i][j] != i*DIM2+j+8 ) exit(49);
      if ( (dta+k)->vdt2[j][i].var_h[i][j] != (float)(i*DIM2+j+9) ) exit(51);

      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_a[i][j] != i*DIM2+j+2 ) exit(53);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != (double)(i*DIM2+j+3) ) exit(55);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_c[i][j] != i*DIM2+j+4 ) exit(57);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != (float)(i*DIM2+j+5) ) exit(59);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != (long double)(i*DIM2+j+6) ) exit(61);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != (double)(i*DIM2+j+7) ) exit(63);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_g[i][j] != i*DIM2+j+8 ) exit(65);
      if ( (dta+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != (float)(i*DIM2+j+9) ) exit(67);
     }
    }
   }
   free(dta);
...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
      if ( (dtb+k)->var_a[i][j] != (float)(2*(i*DIM2+j+2)-1) ) exit(69);
      if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(71);
      if ( (dtb+k)->var_c[i][j] != (long double)(2*(i*DIM2+j+4)+1) ) exit(73);
      if ( (dtb+k)->var_d[i][j] != (double)(2*(i*DIM2+j+5)+2) ) exit(75);
      if ( (dtb+k)->var_e[i][j] != (float)(2*(i*DIM2+j+6)+3) ) exit(77);
      if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(79);
      if ( (dtb+k)->var_g[i][j] != (double)(2*(i*DIM2+j+8)+5) ) exit(81);
      if ( (dtb+k)->var_h[i][j] != 2*(i*DIM2+j+9)+6 ) exit(83);

      if ( (dtb+k)->vdt2[j][i].var_a[i][j] != (double)(2*(i*DIM2+j+2)-1) ) exit(85);
      if ( (dtb+k)->vdt2[j][i].var_b[i][j] != 2*(i*DIM2+j+3) ) exit(87);
      if ( (dtb+k)->vdt2[j][i].var_c[i][j] != (double)(2*(i*DIM2+j+4)+1) ) exit(89);
      if ( (dtb+k)->vdt2[j][i].var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(91);
      if ( (dtb+k)->vdt2[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+6)+3) ) exit(93);
      if ( (dtb+k)->vdt2[j][i].var_f[i][j] != (float)(2*(i*DIM2+j+7)+4) ) exit(95);
      if ( (dtb+k)->vdt2[j][i].var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(97);
      if ( (dtb+k)->vdt2[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+9)+6) ) exit(99);

      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(101);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_b[i][j] != (double)(2*(i*DIM2+j+3)) ) exit(103);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(105);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_d[i][j] != (float)(2*(i*DIM2+j+5)+2) ) exit(107);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+6)+3) ) exit(109);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_f[i][j] != (double)(2*(i*DIM2+j+7)+4) ) exit(111);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(113);
      if ( (dtb+k)->vdt2[j][i].vdt1[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+9)+6) ) exit(115);
     }
    }
   }
