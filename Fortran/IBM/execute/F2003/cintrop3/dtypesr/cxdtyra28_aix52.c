   ...
   for ( k = 0; k < DIM1*DIM2; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
       ...
       if ( (dtx+k)->vdt1[j][i].var_b[i][j] != (double)(2*(i*DIM2+j+2)) ) exit(71);
       if ( (dtx+k)->vdt1[j][i].var_c[i][j] != 2*(i*DIM2+j+3) ) exit(73);
       if ( (dtx+k)->vdt1[j][i].var_d[i][j] != (float)(2*(i*DIM2+j+4)) ) exit(75);
       if ( (dtx+k)->vdt1[j][i].var_e[i][j] != (long double)(2*(i*DIM2+j+5)) ) exit(77);
       if ( (dtx+k)->vdt1[j][i].var_f[i][j] != (double)(2*(i*DIM2+j+6)) ) exit(79);
       if ( (dtx+k)->vdt1[j][i].var_g[i][j] != 2*(i*DIM2+j+7) ) exit(81);
       if ( (dtx+k)->vdt1[j][i].var_h[i][j] != (float)(2*(i*DIM2+j+8)) ) exit(83);
     }
    }
   }
   ...
   for ( k = 0; k < DIM1*DIM2; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
       ...
       (dty+k)->vdt1[j][i].var_b[i][j] = (dtx+k)->vdt1[j][i].var_b[i][j] + 2.0;
       (dty+k)->vdt1[j][i].var_c[i][j] = (dtx+k)->vdt1[j][i].var_c[i][j] + 2;
       (dty+k)->vdt1[j][i].var_d[i][j] = (dtx+k)->vdt1[j][i].var_d[i][j] + 2.0f;
       (dty+k)->vdt1[j][i].var_e[i][j] = (dtx+k)->vdt1[j][i].var_e[i][j] + 2.0l;
       (dty+k)->vdt1[j][i].var_f[i][j] = (dtx+k)->vdt1[j][i].var_f[i][j] + 2.0;
       (dty+k)->vdt1[j][i].var_g[i][j] = (dtx+k)->vdt1[j][i].var_g[i][j] + 2;
       (dty+k)->vdt1[j][i].var_h[i][j] = (dtx+k)->vdt1[j][i].var_h[i][j] + 2.0f;
     }
    }
   }
