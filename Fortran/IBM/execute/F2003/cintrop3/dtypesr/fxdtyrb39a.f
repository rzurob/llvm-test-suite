...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya01.presh fxdtyrb39a
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY:
! %STDIN:
...
function fun1(dtk) bind(c)
   ...
   if ( dtk%var_d /= 8.0d0 .or. dtk%vdt2%var_d /= 8 .or. &
   ...
   if ( dtk%var_f /= 12 .or. dtk%vdt2%var_f /= 12.0e0 .or. &
                           dtk%vdt2%vdt1%var_f /= 12.0d0 ) error stop 222
   if ( dtk%var_g /= 14.0d0 .or. dtk%vdt2%var_g /= 14 .or. &
                           dtk%vdt2%vdt1%var_h /= 16.0e0 ) error stop 224
   if ( dtk%var_h /= 16 .or. dtk%vdt2%var_h /= 16.0e0 .or. &
