...
int main() {
   ...
   for ( k = 0; k < DIM2; k++ ) {
    for ( l = 0; l < DIM1; l++ ) {
     for ( i = 0; i < DIM1; i++ ) {
      for ( j = 0; j < DIM2; j++ ) {
       if ( dtb[k][l].var_a[i][j] != (float)(2*(i*DIM2+j+1)+1) ) exit(53);
       if ( dtb[k][l].var_b[i][j] != 1 ) exit(55);
       if ( dtb[k][l].var_c[i][j] != (long double)(2*(i*DIM2+j+3)+1) ) exit(57);
       if ( dtb[k][l].var_d[i][j] != (double)(2*(i*DIM2+j+4)+1) ) exit(59);
       if ( dtb[k][l].var_e[i][j] != (float)(2*(i*DIM2+j+5)+1) ) exit(61);
       if ( dtb[k][l].var_f[i][j] != 1 ) exit(63);
       if ( dtb[k][l].var_g[i][j] != (double)(2*(i*DIM2+j+7)+1) ) exit(65);
