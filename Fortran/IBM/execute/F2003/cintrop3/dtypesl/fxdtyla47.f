...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya01.presh fxdtyla47
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
...
!*      - Testing FORTRAN module functions
...
function fun1()
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dta(l,k)%var_a(i,j) /= real(i+j+(j-1)*DIM2-1) ) error stop 212
       if ( (dta(l,k)%var_b(i,j) .neqv. .true.) ) error stop 214
       if ( dta(l,k)%var_c(i,j) /= dble(i+j+(j-1)*DIM2+1) ) error stop 216
       if ( dta(l,k)%var_d(i,j) /= dble(i+j+(j-1)*DIM2+2) ) error stop 218
       if ( dta(l,k)%var_e(i,j) /= real(i+j+(j-1)*DIM2+3) ) error stop 220
       if ( (dta(l,k)%var_f(i,j) .neqv. .true.) ) error stop 222
       if ( dta(l,k)%var_g(i,j) /= dble(i+j+(j-1)*DIM2+5) ) error stop 224
       if ( (dta(l,k)%var_h(i,j) /= 'A') ) error stop 226

       if ( dta(l,k)%vdt2(j,i)%var_a(i,j) /= dble(i+j+(j-1)*DIM2-1) ) error stop 228
       if ( (dta(l,k)%vdt2(j,i)%var_b(i,j) .neqv. .true.) ) error stop 230
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= dble(i+j+(j-1)*DIM2+1) ) error stop 232
       if ( (dta(l,k)%vdt2(j,i)%var_d(i,j) /= 'A') ) error stop 234
       if ( dta(l,k)%vdt2(j,i)%var_e(i,j) /= dble(i+j+(j-1)*DIM2+3) ) error stop 236
       if ( dta(l,k)%vdt2(j,i)%var_f(i,j) /= real(i+j+(j-1)*DIM2+4) ) error stop 238
       if ( (dta(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .true.) ) error stop 240
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+6) ) error stop 242

       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .true.) ) error stop 244
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(i+j+(j-1)*DIM2) ) error stop 246
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'A') ) error stop 248
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= real(i+j+(j-1)*DIM2+2) ) error stop 250
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= dble(i+j+(j-1)*DIM2+3) ) error stop 252
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= dble(i+j+(j-1)*DIM2+4) ) error stop 254
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .true.) ) error stop 256
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+6) ) error stop 258
      end do
     end do
    end do
   end do
...
!! Test 3
   ...
   do k = 1, DIM1
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       if ( dta(l,k)%var_a(i,j) /= real(2*(i+j+(j-1)*DIM2-1)+1) ) error stop 116
       if ( (dta(l,k)%var_b(i,j) .neqv. .false.) ) error stop 118
       if ( dta(l,k)%var_c(i,j) /= dble(2*(i+j+(j-1)*DIM2+2)+1) ) error stop 120
       if ( dta(l,k)%var_d(i,j) /= dble(2*(i+j+(j-1)*DIM2+3)+2) ) error stop 122
       if ( dta(l,k)%var_e(i,j) /= real(2*(i+j+(j-1)*DIM2+4)+3) ) error stop 124
       if ( (dta(l,k)%var_f(i,j) .neqv. .false.) ) error stop 126
       if ( dta(l,k)%var_g(i,j) /= dble(2*(i+j+(j-1)*DIM2+6)+5) ) error stop 128
       ...
       if ( dta(l,k)%vdt2(j,i)%var_a(i,j) /= dble(2*(i+j+(j-1)*DIM2-1)+1) ) error stop 148
       if ( (dta(l,k)%vdt2(j,i)%var_b(i,j) .neqv. .false.) ) error stop 150
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= dble(2*(i+j+(j-1)*DIM2+2)+1) ) error stop 152
       if ( (dta(l,k)%vdt2(j,i)%var_d(i,j) /= 'B') ) error stop 154
       if ( dta(l,k)%vdt2(j,i)%var_e(i,j) /= dble(2*(i+j+(j-1)*DIM2+4)+3) ) error stop 156
       if ( dta(l,k)%vdt2(j,i)%var_f(i,j) /= real(2*(i+j+(j-1)*DIM2+5)+4) ) error stop 158
       if ( (dta(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .false.) ) error stop 160
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+7)+6) ) error stop 162
       ...
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .false.) ) error stop 180
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2+1)) ) error stop 182
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'B') ) error stop 184
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_d(i,j) /= real(2*(i+j+(j-1)*DIM2+3)+2) ) error stop 186
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_e(i,j) /= dble(2*(i+j+(j-1)*DIM2+4)+3) ) error stop 188
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_f(i,j) /= dble(2*(i+j+(j-1)*DIM2+5)+4) ) error stop 190
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .false.) ) error stop 192
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+7)+6) ) error stop 194
