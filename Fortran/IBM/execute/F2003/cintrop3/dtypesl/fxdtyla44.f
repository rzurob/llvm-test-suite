...
! %START
! %MAIN:
! %PRECMD: ${TR_SRC}/scrdtya01.presh fxdtyla44
! %COMPOPTS:
! %GROUP: redherring.f
! %VERIFY: 
! %STDIN:
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( (dta(l,k)%var_b(i,j) .neqv. .true.) ) error stop 22
       ...
       if ( (dta(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .true.) ) error stop 48
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+6) ) error stop 50

       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .true.) ) error stop 52
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(i+j+(j-1)*DIM2) ) error stop 54
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'A') ) error stop 56
       ...
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .true.) ) error stop 64
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( (dtu(l,k)%var_b(i,j) .neqv. .true.) ) error stop 70
       ...
       if ( (dtu(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .true.) ) error stop 96
       if ( dtu(l,k)%vdt2(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+6)) ) error stop 98

       if ( (dtu(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .true.) ) error stop 100
       if ( dtu(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2)) ) error stop 102
       if ( (dtu(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'A') ) error stop 104
       ...
       if ( (dtu(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .true.) ) error stop 112
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( (dta(l,k)%var_b(i,j) .neqv. .false.) ) error stop 118
       ...
       if ( (dta(l,k)%var_f(i,j) .neqv. .false.) ) error stop 126
       if ( dta(l,k)%var_g(i,j) /= dble(i+j+(j-1)*DIM2+6) ) error stop 128
       if ( (dta(l,k)%var_h(i,j) /= 'B') ) error stop 130

       if ( dta(l,k)%vdt2(j,i)%var_a(i,j) /= dble(i+j+(j-1)*DIM2) ) error stop 132
       if ( (dta(l,k)%vdt2(j,i)%var_b(i,j) .neqv. .false.) ) error stop 134
       if ( dta(l,k)%vdt2(j,i)%var_c(i,j) /= dble(i+j+(j-1)*DIM2+2) ) error stop 136
       if ( (dta(l,k)%vdt2(j,i)%var_d(i,j) /= 'B') ) error stop 138
       ...
       if ( (dta(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .false.) ) error stop 144
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= real(i+j+(j-1)*DIM2+7) ) error stop 146

       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .false.) ) error stop 148
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(i+j+(j-1)*DIM2+1) ) error stop 150
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'B') ) error stop 152
       ...
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .false.) ) error stop 160
    ...
    do l = 1, DIM2
     do j = 1, DIM2
      do i = 1, DIM1
       ...
       if ( (dta(l,k)%var_b(i,j) .neqv. .false.) ) error stop 166
       ...
       if ( (dta(l,k)%vdt2(j,i)%var_g(i,j) .neqv. .false.) ) error stop 192
       if ( dta(l,k)%vdt2(j,i)%var_h(i,j) /= real(2*(i+j+(j-1)*DIM2+6)+8) ) error stop 194

       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_a(i,j) .neqv. .false.) ) error stop 196
       if ( dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_b(i,j) /= dble(2*(i+j+(j-1)*DIM2)+2) ) error stop 198
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_c(i,j) /= 'B') ) error stop 200
       ...
       if ( (dta(l,k)%vdt2(j,i)%vdt1(j,i)%var_g(i,j) .neqv. .false.) ) error stop 208
