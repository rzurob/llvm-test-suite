...
DT1 *fun2(DT1 *dtx, DT1 *dty) {
   ...
   for ( k = 0; k < DIM1*DIM2; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
       if ( (dtx+k)->var_a[i][j] != 2*(i*DIM2+j+1) ) exit(37);
       if ( (dtx+k)->var_b[i][j] != 2*(i*DIM2+j+2) ) exit(39);
       if ( (dtx+k)->var_c[i][j] != 2*(i*DIM2+j+3) ) exit(41);
       if ( (dtx+k)->var_d[i][j] != 2*(i*DIM2+j+4) ) exit(43);
       if ( (dtx+k)->var_e[i][j] != 2*(i*DIM2+j+5) ) exit(45);
       if ( (dtx+k)->var_f[i][j] != 2*(i*DIM2+j+6) ) exit(47);
       if ( (dtx+k)->var_g[i][j] != 2*(i*DIM2+j+7) ) exit(49);
