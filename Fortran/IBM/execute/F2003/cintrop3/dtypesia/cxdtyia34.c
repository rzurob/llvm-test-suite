...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
    for ( i = 0; i < DIM1; i++ ) {
     for ( j = 0; j < DIM2; j++ ) {
      if ( (dtb+k)->var_a[i][j] != 2*(i*DIM2+j+2)-1 ) exit(53);
      if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+3) ) exit(55);
      if ( (dtb+k)->var_c[i][j] != 2*(i*DIM2+j+4)+1 ) exit(57);
      if ( (dtb+k)->var_d[i][j] != 2*(i*DIM2+j+5)+2 ) exit(59);
      if ( (dtb+k)->var_e[i][j] != 2*(i*DIM2+j+6)+3 ) exit(61);
      if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+7)+4 ) exit(63);
      if ( (dtb+k)->var_g[i][j] != 2*(i*DIM2+j+8)+5 ) exit(65);
