...
/* Test 2 */
   ...
   for ( k = 0; k < DIM2*DIM1; k++ ) {
     for ( i = 0; i < DIM1; i++ ) {
      for ( j = 0; j < DIM2; j++ ) {
       if ( (dtb+k)->var_a[i][j] != 2*(i*DIM2+j+1)+1 ) exit(37);
       if ( (dtb+k)->var_b[i][j] != 2*(i*DIM2+j+2)+1 ) exit(39);
       if ( (dtb+k)->var_c[i][j] != 2*(i*DIM2+j+3)+1 ) exit(41);
       if ( (dtb+k)->var_d[i][j] != 2*(i*DIM2+j+4)+1 ) exit(43);
       if ( (dtb+k)->var_e[i][j] != 2*(i*DIM2+j+5)+1 ) exit(45);
       if ( (dtb+k)->var_f[i][j] != 2*(i*DIM2+j+6)+1 ) exit(47);
       if ( (dtb+k)->var_g[i][j] != 2*(i*DIM2+j+7)+1 ) exit(49);
